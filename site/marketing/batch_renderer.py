    ...
    def render_version(self, version_config):
        ...
        try:
            ...
            self.video_renderer.output_path = os.path.join(
                self.exports_dir,
                f"Njanja_Intro_{version_config['name']}.mp4"
            )
            
            # Render video with effects
            video_config = {
                'title': version_config['title'],
                'features': [(f, i * 5) for i, f in enumerate(version_config['features'])],
    ...
    def generate_report(self, results):
        ...
        with open(report_path, 'w') as f:
            ...
            for result in results:
                ...
                # Load version config
                version_config = next(
                    (v for v in self.load_config()['versions'] if v['name'] == result['name']),
                    None
                )
                ...
                if version_config:
                    ...
                    for effect_type, effects in version_config['effects'].items():
                        ...
                        for effect, enabled in effects.items():
                            f.write(f"  - {effect}: {'Enabled' if enabled else 'Disabled'}\n")
