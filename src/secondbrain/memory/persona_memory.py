...
class MemoryEngine:
    ...
    def store(self, context: Dict[str, Any], feedback: Dict[str, Any]):
        ...
        if not self.persona:
            raise ValueError("Memory engine must be linked to a persona before storing memories")
    ...
    def retrieve(self, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
    ...
    def get_emotional_history(self, timespan_hours: Optional[float] = None) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "most_common_emotion": max(emotions.items(), key=lambda x: x[1])[0]
    ...
    def _save_memories(self):
        ...
        filepath = os.path.join(self.storage_dir, f"{self.persona.lower()}_memories.json")
        try:
