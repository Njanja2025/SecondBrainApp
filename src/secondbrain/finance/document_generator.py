...
class DocumentGenerator:
    ...
    def __init__(self, config: DocumentConfig):
        ...
        """
        self.config = config
        self.output_dir = Path(config.output_dir) if config.output_dir else Path.home() / ".secondbrain" / "reports"
...
class WordDocumentGenerator(DocumentGenerator):
    ...
    def generate_income_report(self, data: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            # Save document
            output_path = self.output_dir / f"income_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    ...
    def generate_income_report(self, data: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            if self.config.include_summary:
                ...
                pdf.set_font("Arial", "", 12)
                pdf.cell(0, 10, f"Total Revenue: {self._format_currency(total)}", ln=True)
                pdf.cell(0, 10, f"Number of Sources: {len(data)}", ln=True)
                pdf.cell(0, 10, f"Average Revenue per Source: {self._format_currency(total / len(data))}", ln=True)
            ...
            # Save document
            output_path = self.output_dir / f"income_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
