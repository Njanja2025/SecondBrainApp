...
pytree.register_pytree_node(
    torch.Size, lambda x: (list(x), None), lambda xs, _: tuple(xs)
)
...
def track_tensor(tensor, proxy, *, constant, tracer):
    ...
    for i, s in enumerate(tensor.shape):
        try_set_proxy_slot(
            s, lambda x, i: set_meta(torch.ops.aten.sym_size.int(proxy, i), x), i
        )

    for i, s in enumerate(tensor.stride()):
        try_set_proxy_slot(
            s, lambda x, i: set_meta(torch.ops.aten.sym_stride.int(proxy, i), x), i
        )

    try_set_proxy_slot(
        tensor.numel(), lambda x: set_meta(torch.ops.aten.sym_numel.default(proxy), x)
    )
    try_set_proxy_slot(
        tensor.storage_offset(),
        lambda x: set_meta(torch.ops.aten.sym_storage_offset.default(proxy), x),
    )
...
def proxy_call(proxy_mode, func, pre_dispatch, args, kwargs):
    ...
    f_args, f_kwargs = pytree.tree_map_only(
        torch.Tensor, fetch_tensor_proxy(tracer), (args, kwargs)
    )
...
def wrap_key(f, tensors, tracer, pre_dispatch: bool):
    ...
    def wrapped(*proxies):
        ...
        out = pytree.tree_map_only(
            (SymInt, SymFloat, SymBool), lambda t: get_proxy_slot(t.node, tracer)(), out
        )
        return out
...
class ProxySymDispatchMode(SymDispatchMode):
    ...
    def _compute_proxy(self, func, args, out: Union[SymInt, SymFloat, SymBool]):
        n_args = tuple(
            (
                get_proxy_slot(a.node, self.tracer)().node
