...
class StencilPass(object):
    def __init__(
        self, func_ir, typemap, calltypes, array_analysis, typingctx, targetctx, flags
    ):
    ...
    def replace_return_with_setitem(
        self, blocks, exit_value_var, parfor_body_exit_label
    ):
        """
    ...
    def _mk_stencil_parfor(
        ...
        for i in range(ndims):
            ...
            loopnests.append(
                numba.parfors.parfor.LoopNest(parfor_vars[i], start_ind, last_ind, 1)
            )
        ...
        if ndims == 1:
            ...
            parfor_ind_var = ir.Var(
                scope, mk_unique_var("$parfor_index_tuple_var"), loc
            )
        ...
        if out_arr is None:
            ...
            slice_callexpr = ir.Expr.call(
                func=slice_func_var, args=(none_var, none_var), kws=(), loc=loc
            )
            ...
            def handle_border(
                ...
                border_ind_var = ir.Var(
                    scope, mk_unique_var("$border_index_tuple_var"), loc
                )
            ...
            if "cval" in stencil_func.options:  # do out[:] = cval
                ...
                setitemexpr = ir.StaticSetItem(
                    out_arr, slice(None, None), slice_inst_var, cval_const_var, loc
                )
        ...
        cval_const_assign = ir.Assign(
            ir.Const(0, loc=dummy_loc), ret_const_var, dummy_loc
        )
    ...
    def _replace_stencil_accesses(
        ...
        if "standard_indexing" in stencil_func.options:
            for x in stencil_func.options["standard_indexing"]:
                if x not in arg_to_arr_dict:
                    raise NumbaValueError(
                        "Standard indexing requested for "
                        "an array name not present in the "
                        "stencil kernel definition."
                    )
            standard_indexed = [
                arg_to_arr_dict[x] for x in stencil_func.options["standard_indexing"]
            ]
        else:
            standard_indexed = []
        ...
        if in_arr.name in standard_indexed:
            raise NumbaValueError(
                "The first argument to a stencil kernel "
        ...
        for label, block in stencil_blocks.items():
            ...
            for stmt in block.body:
                ...
                if (
                    ...
                    (isinstance(stmt, ir.SetItem) or isinstance(stmt, ir.StaticSetItem))
                    and stmt.target.name in in_arg_names
                ):
                    raise NumbaValueError(
                        "Assignments to arrays passed to "
                ...
                if (
                    ...
                    if index_offsets:
                        index_list = self._add_index_offsets(
                            index_list, list(index_offsets), new_body, scope, loc
                        )
        ...
        for i in range(len(index_list)):
            ...
            if isinstance(old_index_var, int):
                ...
                const_assign = ir.Assign(
                    ir.Const(index_list[i], loc), old_index_var, loc
                )
            ...
            if isinstance(offset_var, int):
                ...
                const_assign = ir.Assign(
                    ir.Const(index_offsets[i], loc), offset_var, loc
                )
                out_nodes.append(const_assign)
            ...
            if isinstance(old_index_var, slice) or isinstance(
                ...
                index_var = self._add_offset_to_slice(
                    old_index_var, offset_var, out_nodes, scope, loc
                )
            ...
            if isinstance(offset_var, slice) or isinstance(
                ...
                index_var = self._add_offset_to_slice(
                    offset_var, old_index_var, out_nodes, scope, loc
                )
                index_vars.append(index_var)
                continue
...
def get_stencil_ir(sf, typingctx, args, scope, loc, input_dict, typemap, calltypes):
    ...
    stencil_blocks = ir_utils.add_offset_to_labels(
        stencil_blocks, ir_utils.next_label()
    )
