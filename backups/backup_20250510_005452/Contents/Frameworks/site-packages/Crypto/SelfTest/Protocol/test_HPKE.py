...
class HPKE_Tests(unittest.TestCase):
    ...
    def test_psk(self):
        ...
        HPKE.new(
            receiver_key=self.key1.public_key(), aead_id=aead_id, psk=(b"a", b"c" * 32)
        )
    ...
    def test_neg_wrong_enc(self):
        ...
        with self.assertRaises(DeserializeError):
            HPKE.new(
                receiver_key=self.key1, aead_id=HPKE.AEAD.AES128_GCM, enc=wrong_enc
            )
    ...
    def test_x25519_mode_1(self):
        ...
        decryptor = HPKE.new(
            receiver_key=keyR, aead_id=aead_id, info=info, psk=(psk_id, psk), enc=enc
        )
    ...
    def setUp(self):
        ...
        try:
            ...
            full_file_name = os.path.join(
                init_dir, "Protocol", "HPKE-test-vectors.json"
            )
    ...
    def import_private_key(self, key_hex, kem_id):
        ...
        if kem_id == 0x0010:
            return ECC.construct(
                curve="p256", d=int.from_bytes(key_bin, byteorder="big")
            )
        elif kem_id == 0x0011:
            return ECC.construct(
                curve="p384", d=int.from_bytes(key_bin, byteorder="big")
            )
        elif kem_id == 0x0012:
            return ECC.construct(
                curve="p521", d=int.from_bytes(key_bin, byteorder="big")
            )
        elif kem_id == 0x0020:
    ...
    def test_hpke_encap(self):
        ...
        for idx, vector in enumerate(self.vectors):
            ...
            with self.subTest(idx=idx, kem_id=kem_id, aead_id=aead_id):
                ...
                receiver_pub = self.import_private_key(
                    vector["skRm"], kem_id
                ).public_key()
