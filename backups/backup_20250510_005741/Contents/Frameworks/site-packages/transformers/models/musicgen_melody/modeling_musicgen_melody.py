...
class MusicgenMelodyDecoder(MusicgenMelodyPreTrainedModel):
    ...
    def __init__(self, config: MusicgenMelodyDecoderConfig):
        ...
        self.layers = nn.ModuleList(
            [
                MusicgenMelodyDecoderLayer(config)
            ...
            if attention_mask is not None:
                if encoder_attention_mask is None:
                    encoder_attention_mask = torch.ones(
                        encoder_hidden_states.shape[:2], device=attention_mask.device
                    )
                attention_mask = torch.cat(
                    [encoder_attention_mask, attention_mask], dim=1
                )
...
class MusicgenMelodyForCausalLM(MusicgenMelodyPreTrainedModel, GenerationMixin):
    ...
    @add_start_docstrings_to_model_forward(MUSICGEN_MELODY_DECODER_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=MusicgenMelodyOutputWithPast, config_class=_CONFIG_FOR_DOC
    )
    # Ignore copy
    def forward(
        self,
...
class MusicgenMelodyForConditionalGeneration(PreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        if self.config.num_chroma != self.decoder.config.hidden_size:
            self.audio_enc_to_dec_proj = nn.Linear(
                self.config.num_chroma, self.decoder.config.hidden_size
            )
        ...
        **kwargs,
        ...
        if encoder_hidden_states is None:
            ...
            if input_features is not None:
                ...
                if self.config.num_chroma != self.decoder.config.hidden_size:
                    audio_hidden_states = self.audio_enc_to_dec_proj(
                        audio_hidden_states
                    )
                ...
                if audio_hidden_states.shape[1] < self.config.chroma_length:
                    n_repeat = int(
                        math.ceil(
                            self.config.chroma_length / audio_hidden_states.shape[1]
                        )
                    )
                ...
                if encoder_hidden_states is not None:
                    encoder_hidden_states = torch.cat(
                        [audio_hidden_states, encoder_hidden_states], dim=1
                    )
                else:
    ...
    def _prepare_encoder_hidden_states_kwargs_for_generation(
        ...
        if inputs_tensor is not None:
            ...
            encoder_kwargs["output_hidden_states"] = (
        ...
        if inputs_tensor is not None:
            if audio_hidden_states is not None:
                ...
                null_audio_hidden_states = torch.zeros(
                    (inputs_tensor.shape[0], 1, self.config.num_chroma),
                    device=self.device,
                    dtype=self.dtype,
                )
        ...
        if audio_hidden_states is not None:
            ...
            if audio_hidden_states.shape[1] < self.config.chroma_length:
                n_repeat = int(
                    math.ceil(self.config.chroma_length / audio_hidden_states.shape[1])
                )
                audio_hidden_states = audio_hidden_states.repeat(1, n_repeat, 1)
