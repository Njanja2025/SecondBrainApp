...
def create_vault_files(temp_dir: Path) -> Dict[str, str]:
    ...
    try:
        ...
        for prompt in prompts:
            ...
            prompt_text.append(f"Monetization Potential: {prompt['monetization_potential']}")
            prompt_text.append(f"Target Audience: {', '.join(prompt['target_audience'])}")
            prompt_text.append(f"Required Skills: {', '.join(prompt['required_skills'])}")
