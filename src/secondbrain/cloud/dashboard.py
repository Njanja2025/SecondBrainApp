...
class ReportDashboard:
    ...
    def _setup_ui(self):
        ...
        # Title
        title = ttk.Label(
            main_frame, text="Cloud Operations Dashboard", font=("Arial", 20)
        )
        title.grid(row=0, column=0, columnspan=2, pady=20)

        # Report list
        self.report_list = tk.Listbox(main_frame, width=70, height=20)
        self.report_list.grid(row=1, column=0, columnspan=2, pady=10)

        # Buttons
        refresh_btn = ttk.Button(
            main_frame, text="Refresh Reports", command=self.refresh_reports
        )
        refresh_btn.grid(row=2, column=0, pady=10, padx=5)

        view_btn = ttk.Button(
            main_frame, text="View Selected Report", command=self.view_selected_report
        )
    ...
    def refresh_reports(self):
        ...
        try:
            ...
            for report in reports:
                ...
                display_name = (
                    f"{report.name} - {timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
                )
