...
class Baddy(Character):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.name = "Baddy"
        self.description = (
            "A mischievous character that creates challenges and obstacles"
        )
        self.personality = {
            "traits": [
                "mischievous",
        ...
        try:
            ...
            if event in mood_changes:
                self.personality["mood"] = (
                    "playful" if mood_changes[event] > 0 else "calm"
                )
                self.personality["energy"] = max(
                    0, min(100, self.personality["energy"] + mood_changes[event])
                )
                self.save_state()
    ...
    def generate_challenge(self, difficulty: str = "medium") -> Dict[str, Any]:
        ...
        try:
            challenge_types = {
                ...
                "medium": ["Navigate a maze", "Decode a message", "Find a pattern"],
                "hard": [
    ...
    def generate_reward(self, difficulty: str) -> Dict[str, Any]:
        ...
        try:
            rewards = {
                "easy": ["Small hint", "Bonus point", "Extra time"],
                "medium": ["Medium hint", "Bonus points", "Special ability"],
                "hard": ["Big hint", "Many bonus points", "Special power"],
            }
