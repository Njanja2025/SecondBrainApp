...
def run_benchmark(
    ...
    cuda = device == "cuda"
    spectral_fuzzer = SpectralOpFuzzer(
        seed=seed, dtype=dtype, cuda=cuda, probability_regular=probability_regular
    )
    results = []
...
def _output_csv(file, results):
    file.write(
        "benchmark,device,num_threads,numel,shape,contiguous,dim,mean (us),median (us),iqr (us)\n"
    )
    for measurement in results:
        ...
        print(
            name,
            device,
            measurement.task_spec.num_threads,
            numel,
            shape_str,
            contiguous,
            dim_str,
            measurement.mean * 1e6,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", type=str, choices=DEVICE_NAMES, nargs="+", default=DEVICE_NAMES
    )
    parser.add_argument(
        "--bench", type=str, choices=BENCHMARK_NAMES, nargs="+", default=BENCHMARK_NAMES
    )
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--samples", type=int, default=10)
    parser.add_argument(
        "--probability-regular", "--probability_regular", type=float, default=1.0
    )
    ...
    for device in args.device:
        for bench in (BENCHMARK_MAP[b] for b in args.bench):
            ...
            i += 1
            print(
                f"Completed {bench.name} benchmark on {device} ({i} of {num_benchmarks})"
            )
