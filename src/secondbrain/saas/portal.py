...
class Portal:
    ...
    def create_subscription(
        self, user: User, plan_id: str, payment_method: str
    ) -> bool:
        """
        ...
        try:
            ...
            if payment_method == "stripe":
                customer_id = self.payment_gateway.create_stripe_customer(
                    user.email, user.name
                )
            else:
                customer_id = self.payment_gateway.create_paypal_customer(
                    user.email, user.name
                )
            ...
            subscription_id = self.payment_gateway.create_subscription(
                customer_id, plan_id, payment_method
            )
