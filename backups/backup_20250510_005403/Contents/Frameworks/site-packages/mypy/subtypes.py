    ...
    def visit_instance(self, left: Instance) -> bool:
            ...
            if left.type.has_base(right.partial_fallback.type.fullname):
                if not self.proper_subtype:
                    ...
                    mapped = map_instance_to_supertype(
                        left, right.partial_fallback.type
                    )
                    for arg in map(get_proper_type, mapped.args):
                        if isinstance(arg, UnpackType):
                            ...
                            if not isinstance(
                                get_proper_type(unpacked.args[0]), AnyType
                            ):
                                break
        ...
        if isinstance(right, Instance):
            ...
            if type_state.is_cached_negative_subtype_check(
                ...
                return False
            if (
                not self.subtype_context.ignore_promotions
                and not right.type.is_protocol
            ):
            ...
            if (
                ...
                if right.type.has_type_var_tuple_type:
                    ...
                    left_prefix, left_middle, left_suffix = (
                        split_with_prefix_and_suffix(t.args, prefix, suffix)
                    )
                    right_prefix, right_middle, right_suffix = (
                        split_with_prefix_and_suffix(right.args, prefix, suffix)
                    )
                    left_args = (
                        left_prefix
                        + (TupleType(list(left_middle), fallback),)
                        + left_suffix
                    )
                    right_args = (
                        right_prefix
                        + (TupleType(list(right_middle), fallback),)
                        + right_suffix
                    )
    ...
    def variadic_tuple_subtype(self, left: TupleType, right: TupleType) -> bool:
        ...
        if left_unpack_index is None:
            ...
            if not all(
                ...
                for li, ri in zip(prefix, right.items[:right_prefix])
            ):
                return False
            if right_suffix and not all(
                self._is_subtype(li, ri)
                for li, ri in zip(suffix, right.items[-right_suffix:])
            ):
                return False
    ...
    def visit_typeddict_type(self, left: TypedDictType) -> bool:
        ...
        if isinstance(right, Instance):
            ...
            for name, l, r in left.zip(right):
                ...
                required_differ = (name in left.required_keys) != (
                    name in right.required_keys
                )
            ...
            for right_item in right.items:
                ...
                for left_index, left_item in enumerate(left.items):
                    ...
                    if subtype_match and previous_match_left_index <= left_index:
                        ...
                        strict_concat = (
                            (
                                self.options.extra_checks
                                or self.options.strict_concatenate
                            )
                            if self.options
                            else False
                        )
...
def are_args_compatible(
    ...
    *,
    ...
    if (
        is_different(left.pos, right.pos, allow_overlap=False)
        and not allow_imprecise_kinds
    ):
        return False
...
def unify_generic_callable(
    ...
    if set(type.type_var_ids()) & {
        v.id for v in mypy.typeops.get_all_type_vars(target)
    }:
