...
class BackupManager:
        ...
        try:
            ...
            if config_file.exists():
                with open(config_file, 'r') as f:
                    self.configs = {name: BackupConfig(**config)
    ...
    def create_config(self, config: BackupConfig) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to create backup configuration {config.name}: {str(e)}")
            return False
    ...
    def _upload_to_s3(self, backup_file: Path, config: BackupConfig):
        ...
        try:
            ...
            # Generate S3 key
            s3_key = f"{config.s3_prefix}/{backup_file.name}" if config.s3_prefix else backup_file.name
