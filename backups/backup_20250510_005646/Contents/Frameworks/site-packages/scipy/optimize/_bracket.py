...
def _bracket_root_iv(func, xl0, xr0, xmin, xmax, factor, args, maxiter):
    ...
    xl0 = xp.asarray(xl0)[()]
    if not xp.isdtype(xl0.dtype, "numeric") or xp.isdtype(
        xl0.dtype, "complex floating"
    ):
    ...
    if not xp.isdtype(xr0.dtype, "numeric") or xp.isdtype(
        xr0.dtype, "complex floating"
    ):
        raise ValueError("`xr0` must be numeric and real.")

    if not xp.isdtype(xmin.dtype, "numeric") or xp.isdtype(
        xmin.dtype, "complex floating"
    ):
        raise ValueError("`xmin` must be numeric and real.")

    if not xp.isdtype(xmax.dtype, "numeric") or xp.isdtype(
        xmax.dtype, "complex floating"
    ):
        raise ValueError("`xmax` must be numeric and real.")

    if not xp.isdtype(factor.dtype, "numeric") or xp.isdtype(
        factor.dtype, "complex floating"
    ):
    ...
    if (
        not xp.isdtype(maxiter.dtype, "numeric")
        or maxiter.shape != tuple()
...
def _bracket_root(
    func, xl0, xr0=None, *, xmin=None, xmax=None, factor=None, args=(), maxiter=1000
):
...
def _bracket_minimum_iv(func, xm0, xl0, xr0, xmin, xmax, factor, args, maxiter):
    ...
    xm0 = xp.asarray(xm0)[()]
    if not xp.isdtype(xm0.dtype, "numeric") or xp.isdtype(
        xm0.dtype, "complex floating"
    ):
