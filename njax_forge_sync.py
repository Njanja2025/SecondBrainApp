...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('njax_sync.log'),
    ...
    def sync_components(self, component_list: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            if component_list is None:
                component_list = [d.name for d in self.components_dir.iterdir() if d.is_dir()]
            ...
            for component in component_list:
                ...
                if sync_script.exists():
                    ...
                    if result.returncode != 0:
                        logger.error(f"Failed to sync component {component}: {result.stderr}")
                        continue
        ...
        try:
            ...
            for component_path in self.components_dir.iterdir():
                ...
                if not manifest_file.exists():
                    logger.error(f"Missing manifest for component {component_path.name}")
                    return False
                ...
                for file_info in manifest.get('files', []):
                    ...
                    if not file_path.exists():
                        logger.error(f"Missing file {file_path} in component {component_path.name}")
                        return False
