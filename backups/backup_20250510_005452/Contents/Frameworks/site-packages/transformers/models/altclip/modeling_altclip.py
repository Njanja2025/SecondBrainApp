...
class AltRobertaLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = AltRobertaAttention(
                config, position_embedding_type="absolute"
            )
...
class AltRobertaEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [AltRobertaLayer(config) for _ in range(config.num_hidden_layers)]
        )
...
class AltCLIPEncoder(nn.Module):
    ...
    def __init__(self, config: AltCLIPConfig):
        ...
        self.layers = nn.ModuleList(
            [AltCLIPEncoderLayer(config) for _ in range(config.num_hidden_layers)]
        )
        self.gradient_checkpointing = False
...
class AltCLIPVisionTransformer(nn.Module):
    ...
    @replace_return_docstrings(
        output_type=BaseModelOutputWithPooling, config_class=AltCLIPVisionConfig
    )
    def forward(
        self,
...
class AltCLIPTextModel(AltCLIPPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=BaseModelOutputWithPoolingAndProjection,
        config_class=AltCLIPTextConfig,
    )
    def forward(
        self,
