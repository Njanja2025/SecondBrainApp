...
class ViTHybridPatchEmbeddings(nn.Module):
    ...
    def __init__(self, config, feature_size=None):
        ...
        if self.backbone.config.model_type != "bit":
            raise ValueError(
                f"Backbone model type {self.backbone.model_type} is not supported."
            )
        ...
        if feature_size is None:
            ...
            feature_size = (
                feature_size
                if isinstance(feature_size, collections.abc.Iterable)
                else (feature_size, feature_size)
            )
            feature_dim = self.backbone.channels[-1]
        ...
        self.projection = nn.Conv2d(
            feature_dim, hidden_size, kernel_size=patch_size, stride=patch_size
        )
...
class ViTHybridLayer(nn.Module):
    ...
    def __init__(self, config: ViTHybridConfig) -> None:
        ...
        self.seq_len_dim = 1
        self.attention = VIT_HYBRID_ATTENTION_CLASSES[config._attn_implementation](
            config
        )
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(
            self.layernorm_before(
                hidden_states
            ),  # in ViTHybrid, layernorm is applied before self-attention
            head_mask,
...
class ViTHybridEncoder(nn.Module):
    def __init__(self, config: ViTHybridConfig) -> None:
        ...
        self.layer = nn.ModuleList(
            [ViTHybridLayer(config) for _ in range(config.num_hidden_layers)]
        )
