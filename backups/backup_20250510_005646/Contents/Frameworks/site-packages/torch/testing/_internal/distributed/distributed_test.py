...
def get_profiling_event(event_name, profiler):
    ...
    return [
        ...
        if (event.name.endswith(event_name) or event.name.startswith(event_name))
    ]
...
class DistributedTest:
    class _DistTestBase:
        ...
        @unittest.skipIf(
            BACKEND == "ucc",
            "broken, see https://github.com/pytorch/pytorch/pull/113620",
        )
        ...
        @skip_if_lt_x_gpu(2)
        @skip_but_pass_in_sandcastle_if(
            BACKEND != "nccl", "TORCH_NCCL_USE_COMM_NONBLOCKING only applies to NCCL"
        )
