...
def numpy_cube_backward(ctx, saved, grad_out, grad_dx):
    x, dx = saved
    grad_x = torch.ops._torch_testing.numpy_mul(
        grad_out, dx
    ) + 6 * torch.ops._torch_testing.numpy_mul(grad_dx, x)
    return {"x": grad_x}
...
def numpy_sort_abstract(x, dim):
    return (
        torch.empty_like(x),
        torch.empty_like(x, dtype=torch.long),
        torch.empty_like(x, dtype=torch.long),
    )
...
@custom_ops.impl_backward(
    "_torch_testing::numpy_sort", output_differentiability=[True, False, False]
)
...
def numpy_take_backward(ctx, saved, grad_out):
    return {
        "x": torch.ops._torch_testing.numpy_take(
            grad_out, saved["ind_inv"], saved["ind"], saved["dim"]
        ),
        "ind": None,
        "ind_inv": None,
    }
...
def numpy_split_copy_with_int(
    x: Tensor, sections: Sequence[int], dim: int
...
def numpy_split_copy_with_int_impl(x, splits, dim):
    x_np = to_numpy(x)
    arrs = np.split(x_np, splits, axis=dim)
    return [torch.tensor(arr, device=x.device, dtype=x.dtype) for arr in arrs], len(
        splits
    )
...
@custom_ops.impl_backward(
    "_torch_testing::numpy_split_copy_with_int", output_differentiability=[True, False]
)
