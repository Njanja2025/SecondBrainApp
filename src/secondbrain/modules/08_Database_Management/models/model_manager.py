...
class ModelManager:
    ...
    def create_config(self, config: ModelConfig) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to create model configuration {config.name}: {str(e)}"
            )
            return False
            ...
            for field in config.fields:
                ...
                if field_type:
                    setattr(
                        model_class,
                        field["name"],
                        Column(field_type, **field.get("options", {})),
                    )
                ...
                for rel in config.relationships:
                    ...
                    if relationship_type:
                        setattr(
                            model_class,
                            rel["name"],
                            relationship_type(**rel.get("options", {})),
                        )
    ...
    def validate_schema(self, model_class: Type) -> bool:
        ...
        try:
            ...
            for column in model_class.__table__.columns:
                if not isinstance(
                    column.type, (String, Integer, DateTime, Boolean, JSON)
                ):
