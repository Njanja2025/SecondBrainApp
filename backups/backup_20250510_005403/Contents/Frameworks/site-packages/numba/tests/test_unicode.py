...
from numba.core.errors import TypingError, UnsupportedError, UnsupportedBytecodeError
...
class TestUnicode(BaseTest):
    ...
    def test_expandtabs_with_tabsize(self):
        fns = [
            njit(expandtabs_with_tabsize_usecase),
            njit(expandtabs_with_tabsize_kwarg_usecase),
        ]
        messages = [
            'Results of "{}".expandtabs({}) must be equal',
            ...
            for prefix in cpython_subs + default_subs + extra_subs:
                for start in list(range(-20, 20)) + [None]:
                    for end in list(range(-20, 20)) + [None]:
                        self.assertEqual(
                            pyfunc(s, prefix, start, end), cfunc(s, prefix, start, end)
                        )
    ...
    def test_endswith_default(self):
        ...
        for s in cpython_str + UNICODE_EXAMPLES:
            ...
            for sub_str in cpython_subs + default_subs + extra_subs:
                ...
                self.assertEqual(
                    pyfunc(s, sub_str), cfunc(s, sub_str), msg=msg.format(s, sub_str)
                )
            ...
            for sub_str in cpython_subs + default_subs + extra_subs:
                for start in list(range(-20, 20)) + [None]:
                    for end in list(range(-20, 20)) + [None]:
                        ...
                        tuple_subs = (sub_str, "lo")
                        self.assertEqual(
                            pyfunc(s, tuple_subs, start, end),
                            cfunc(s, tuple_subs, start, end),
                            msg=msg.format(s, tuple_subs, start, end),
                        )
    ...
    def test_in(self, flags=no_pyobj_flags):
        ...
        for a in UNICODE_EXAMPLES:
            ...
            for substr in extras:
                self.assertEqual(
                    pyfunc(substr, a), cfunc(substr, a), "'%s' in '%s'?" % (substr, a)
                )
    ...
    def test_find(self, flags=no_pyobj_flags):
        ...
        cpython_subs = [
            ("a" * 100 + "\u0102", ["\u0102", "\u0201", "\u0120", "\u0220"]),
            ("a" * 100 + "\U00100304", ["\U00100304", "\U00100204", "\U00102004"]),
            ("\u0102" * 100 + "a", ["a"]),
            ("\U00100304" * 100 + "a", ["a"]),
            ("\U00100304" * 100 + "\u0102", ["\u0102"]),
    ...
    def test_count(self):
        ...
        for s, sub in UNICODE_COUNT_EXAMPLES:
            ...
            self.assertEqual(
                py_result, c_result, error_msg.format(s, sub, py_result, c_result)
            )
    ...
    def test_count_with_start(self):
        ...
        for s, sub in UNICODE_COUNT_EXAMPLES:
            ...
            self.assertEqual(
                py_result, c_result, error_msg.format(s, sub, None, py_result, c_result)
            )
    ...
    def test_count_arg_type_check(self):
        ...
        with self.assertRaises(TypingError) as raises:
            cfunc("ascii", "c", 1, 0.5)
        self.assertIn(
            "The slice indices must be an Integer or None", str(raises.exception)
        )
    ...
    def test_rfind(self):
        ...
        cpython_subs = [
            ("\u0102" + "a" * 100, ["\u0102", "\u0201", "\u0120", "\u0220"]),
            ("\U00100304" + "a" * 100, ["\U00100304", "\U00100204", "\U00102004"]),
            ("abcdefghiabc", ["abc", ""]),
            ("a" + "\u0102" * 100, ["a"]),
            ("a" + "\U00100304" * 100, ["a"]),
            ("\u0102" + "\U00100304" * 100, ["\u0102"]),
    ...
    def test_rfind_wrong_start_end_optional(self):
        ...
        def try_compile_wrong_start_optional(*args):
            wrong_sig_optional = types.int64(
                types.unicode_type,
                types.unicode_type,
                types.Optional(types.float64),
    ...
    def test_index_rindex_with_start_end(self):
        ...
        messages = [
            'Results of "{}".index("{}", {}, {}) must be equal',
        ...
        for pyfunc, msg in zip(pyfuncs, messages):
            ...
            for s in unicode_examples:
                ...
                cases = [
                    ...
                    (s[3:], list(range(4)) + list(range(-10, -1)), list(range(l, 10))),
        ...
        for s in UNICODE_EXAMPLES:
            for i in list(range(-len(s), len(s))):
                for j in list(range(-len(s), len(s))):
                    sl = slice(i, j)
                    self.assertEqual(
                        pyfunc(s, sl), cfunc(s, sl), "'%s'[%d:%d]?" % (s, i, j)
                    )
    ...
    def test_repeat_exception_float(self):
        ...
        with self.assertRaises(TypingError) as raises:
            cfunc("hi", 2.5)
        self.assertIn(
            _header_lead + " Function(<built-in function mul>)", str(raises.exception)
        )
    ...
    def test_split_with_maxsplit(self):
        ...
        for pyfunc, fmt_str in [
            ...
            for test_str, splitter, maxsplit in CASES:
                self.assertEqual(
                    pyfunc(test_str, splitter, maxsplit),
    ...
    def test_split_whitespace(self):
        ...
        for test_str in CASES:
            self.assertEqual(
                pyfunc(test_str), cfunc(test_str), "'%s'.split()?" % (test_str,)
            )
    ...
    def test_splitlines_with_keepends(self):
        ...
        for pyfunc, msg in zip(pyfuncs, messages):
            ...
            for s, keepends in product(cases, all_keepends):
                self.assertEqual(
                    pyfunc(s, keepends), cfunc(s, keepends), msg=msg.format(s, keepends)
                )
    ...
    def test_join(self):
        ...
        for sep, parts in CASES:
            self.assertEqual(
                pyfunc(sep, parts), cfunc(sep, parts), "'%s'.join('%s')?" % (sep, parts)
            )
    ...
    def test_justification(self):
        for pyfunc, case_name in [
            (center_usecase, "center"),
            (ljust_usecase, "ljust"),
            (rjust_usecase, "rjust"),
        ]:
            cfunc = njit(pyfunc)
    ...
    def test_justification_fillchar(self):
        for pyfunc, case_name in [
            (center_usecase_fillchar, "center"),
    ...
    def test_justification_fillchar_exception(self):
        ...
        for pyfunc in [
            ...
            for fillchar in ["", "+0", "quién", "处着"]:
                ...
                self.assertIn(
                    "The fill character must be exactly one", str(raises.exception)
                )
            ...
            for fillchar in [1, 1.1]:
                with self.assertRaises(TypingError) as raises:
                    cfunc(UNICODE_EXAMPLES[0], 20, fillchar)
                self.assertIn(
                    "The fillchar must be a UnicodeType", str(raises.exception)
                )
    ...
    def test_strip(self):
        ...
        for pyfunc, case_name in [
            ...
            for string, chars in STRIP_CASES:
                self.assertEqual(
                    pyfunc(string), cfunc(string), "'%s'.%s()?" % (string, case_name)
                )
        # parametrized form
        for pyfunc, case_name in [
            (strip_usecase_chars, "strip"),
            (lstrip_usecase_chars, "lstrip"),
            (rstrip_usecase_chars, "rstrip"),
        ]:
            cfunc = njit(pyfunc)

            sig1 = types.unicode_type(
                types.unicode_type, types.Optional(types.unicode_type)
            )
            cfunc_optional = njit([sig1])(pyfunc)

            def try_compile_bad_optional(*args):
                bad = types.unicode_type(
                    types.unicode_type, types.Optional(types.float64)
                )
                njit([bad])(pyfunc)
            ...
            for fn in cfunc, try_compile_bad_optional:
                with self.assertRaises(TypingError) as raises:
                    fn("tú quis?", 1.1)
                self.assertIn(
                    "The arg must be a UnicodeType or None", str(raises.exception)
                )
    ...
    def test_istitle(self):
        ...
        for s in ISTITLE_EXAMPLES:
            ...
            c_result = cfunc(s)
            self.assertEqual(
                py_result, c_result, error_msg.format(s, py_result, c_result)
            )
    ...
    def test_comparison(self):
        ...
        for x, y in permutations(UNICODE_ORDERING_EXAMPLES, r=2):
            for cmpop in ["==", "!=", "<", ">", "<=", ">=", ""]:
                args = [cmpop, x, y]
                self.assertEqual(
                    pyfunc(*args), cfunc(*args), msg="failed on {}".format(args)
                )
    ...
    def test_replace(self):
        ...
        for test_str, old_str, new_str in CASES:
            self.assertEqual(
                ...
                "'%s'.replace('%s', '%s')?" % (test_str, old_str, new_str),
            )
    ...
    def test_replace_with_count(self):
        ...
        for test_str, old_str, new_str in CASES:
            for count in count_test:
                self.assertEqual(
                    pyfunc(test_str, old_str, new_str, count),
