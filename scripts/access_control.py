...
class AccessControl:
    ...
    def create_user(
        self, admin_token: str, username: str, password: str, role: Role
    ) -> bool:
        ...
        self.audit_log(
            admin_username,
            "create_user",
            f"Created user: {username} with role: {role.name}",
        )
        return True
    ...
    def change_user_role(self, admin_token: str, username: str, new_role: Role) -> bool:
        ...
        if (
            new_role.name
            not in self.roles["roles"][admin_role.name]["can_assign_roles"]
        ):
        ...
        self.audit_log(
            admin_username,
            "change_role",
            f"Changed role for {username} to {new_role.name}",
        )
        return True
