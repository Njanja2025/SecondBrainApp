...
class GumroadAssetManager:
    ...
    def copy_mockup_image(
        self, source_path: str, destination_name: str = "cover_mockup.jpg"
    ) -> str:
        ...
        try:
            ...
            # Copy the file
            logger.info(
                f"Copying mockup image from {source_path} to {destination_path}"
            )
    ...
    def setup_product_assets(self):
        ...
        try:
            ...
            if os.path.exists(voice_over_path):
                shutil.copy2(
                    voice_over_path, os.path.join(product_dir, "product_intro.mp3")
                )

            # Copy product files
            product_files = ["NjanjaStorefront_Package.zip", "README.md", "LICENSE"]

            for file in product_files:
                if os.path.exists(file):
                    shutil.copy2(file, os.path.join(product_dir, file))
