...
class ReportGenerator:
    ...
    def create_config(self, config: ReportConfig) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to create report configuration {config.name}: {str(e)}"
            )
            return False
    ...
    def generate_report(
        self, config_name: str, test_results: Dict[str, Any]
    ) -> Optional[str]:
        ...
        try:
            ...
            report_file = (
                export_dir / f"report_{config_name}_{timestamp}.{config.format}"
            )
            ...
            logger.error(
                f"Failed to generate report for configuration {config_name}: {str(e)}"
            )
            return None
