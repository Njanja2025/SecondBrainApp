...
class PerformanceAnalyzer:
        ...
        try:
            ...
            if config_file.exists():
                with open(config_file, 'r') as f:
                    self.configs = {name: PerformanceConfig(**config)
    ...
    def create_config(self, config: PerformanceConfig) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to create performance test configuration {config.name}: {str(e)}")
            return False
    ...
    def update_config(self, name: str, config: PerformanceConfig) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to update performance test configuration {name}: {str(e)}")
            return False
    ...
    def delete_config(self, name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to delete performance test configuration {name}: {str(e)}")
            return False
    ...
    def run_tests(self, config_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to run performance tests for configuration {config_name}: {str(e)}")
    ...
    def _run_benchmark(self, benchmark: Dict[str, Any], config: PerformanceConfig) -> Dict[str, Any]:
                        ...
                        for metric, threshold in config.threshold.items():
                            if metric in result["metrics"]:
                                if result["metrics"][metric]["avg"] > threshold:
                                    ...
                                    result["error"] = f"Metric {metric} exceeded threshold"
                                    break
    ...
    def add_benchmark(self, config_name: str, benchmark: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to add benchmark to configuration {config_name}: {str(e)}")
            return False
    ...
    def remove_benchmark(self, config_name: str, benchmark_name: str) -> bool:
        ...
        try:
            ...
            config.benchmarks = [b for b in config.benchmarks if b.get("name") != benchmark_name]
            self._save_configs()
            ...
            logger.error(f"Failed to remove benchmark from configuration {config_name}: {str(e)}")
            return False
