...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/payments.log"), logging.StreamHandler()],
)
...
class PaymentDashboard:
    ...
    def _create_widgets(self):
        ...
        plans_frame = ttk.LabelFrame(
            self.plans_tab, text="Available Plans", padding="5"
        )
        ...
        stats_frame = ttk.LabelFrame(
            self.stats_tab, text="Payment Statistics", padding="5"
        )
        ...
        trans_frame = ttk.LabelFrame(
            self.stats_tab, text="Recent Transactions", padding="5"
        )
        ...
        scrollbar = ttk.Scrollbar(
            trans_frame, orient=tk.VERTICAL, command=self.trans_tree.yview
        )
        ...
        self.log_text = scrolledtext.ScrolledText(
            logs_frame, wrap=tk.WORD, width=80, height=30
        )
        self.log_text.pack(fill=tk.BOTH, expand=True)

        # Add refresh button
        refresh_btn = ttk.Button(
            logs_frame, text="Refresh Logs", command=self._refresh_logs
        )
    ...
    def _update_statistics(self):
        ...
        try:
            ...
            avg_revenue = (
                total_revenue / active_subscriptions if active_subscriptions > 0 else 0
            )

            # Update labels
            self.stats_labels["Total Revenue"].config(text=f"${total_revenue:.2f}")
            self.stats_labels["Active Subscriptions"].config(
                text=str(active_subscriptions)
            )
            self.stats_labels["Average Revenue per User"].config(
