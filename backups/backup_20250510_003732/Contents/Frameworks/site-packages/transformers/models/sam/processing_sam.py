...
class SamProcessor(ProcessorMixin):
    ...
    def _normalize_and_convert(
            ...
            if len(original_sizes) != len(input_points):
                input_points = [
                    self._normalize_coordinates(
                        self.target_size, point, original_sizes[0]
                    )
        ...
        if input_boxes is not None:
            if len(original_sizes) != len(input_boxes):
                input_boxes = [
                    self._normalize_coordinates(
                        self.target_size, box, original_sizes[0], is_bounding_box=True
                    )
                ...
                input_boxes = [
                    self._normalize_coordinates(
                        self.target_size, box, original_size, is_bounding_box=True
                    )
        ...
        if input_labels is not None:
            if return_tensors == "pt":
                ...
                # point batch size of 1 by default
                input_labels = (
                    input_labels.unsqueeze(1)
                    if len(input_labels.shape) != 3
                    else input_labels
                )
            elif return_tensors == "tf":
        ...
        for i, point in enumerate(input_points):
            if point.shape[0] != expected_nb_points:
                point = np.concatenate(
                    [
                        point,
                        np.zeros((expected_nb_points - point.shape[0], 2))
                        + point_pad_value,
                    ],
                    axis=0,
                )
    ...
    def _normalize_coordinates(
        ...
        new_h, new_w = self.image_processor._get_preprocess_shape(
            original_size, longest_edge=target_size
        )
    ...
    def _check_and_preprocess_points(
        ...
        if input_points is not None:
            ...
            if not isinstance(input_points, list) or not isinstance(
                input_points[0], list
            ):
                raise ValueError(
        ...
        if input_labels is not None:
            ...
            if not isinstance(input_labels, list) or not isinstance(
                input_labels[0], list
            ):
