...
class DNSManager:
        ...
        async with self._lock:
            try:
                ...
                if self.system == "darwin":
                    await self._configure_dns_macos(
                        vpn_dns_servers, interface, backup_dns
                    )
                elif self.system == "linux":
                    await self._configure_dns_linux(vpn_dns_servers, backup_dns)
                else:
                    raise NotImplementedError(
                        f"DNS configuration not implemented for {self.system}"
                    )
    ...
    async def _configure_dns_macos(
        self, vpn_dns_servers: List[str], interface: str, backup_dns: List[str]
    ):
    ...
    async def _configure_dns_linux(
        self, vpn_dns_servers: List[str], backup_dns: List[str]
    ):
        ...
        async with self._lock:
            try:
                if self.system == "darwin":
                    ...
                    raise NotImplementedError(
                        f"Split tunneling not implemented for {self.system}"
                    )
