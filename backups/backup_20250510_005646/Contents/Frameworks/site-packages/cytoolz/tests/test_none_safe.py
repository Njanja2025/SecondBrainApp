...
def test_dicttoolz():
    ...
    # XXX
    assert raises(TypeError, lambda: get_in(None, {})) or get_in(None, {}) is None
    ...
    # XXX
    assert raises(TypeError, lambda: keymap(None, {1: 2})) or keymap(None, {1: 2}) == {
        (1,): 2
    }
    ...
    assert raises((TypeError, AttributeError), lambda: merge_with(identity, None, None))
    ...
    assert raises(
        (TypeError, AttributeError), lambda: update_in(None, [1, 2], identity)
    )
    ...
    # XXX
    assert raises(TypeError, lambda: valmap(None, {1: 2})) or valmap(None, {1: 2}) == {
        1: (2,)
    }
    assert raises((AttributeError, TypeError), lambda: valmap(identity, None))
    tested.append("valmap")

    assert raises(TypeError, lambda: itemmap(None, {1: 2})) or itemmap(
        None, {1: 2}
    ) == {1: (2,)}
...
def test_itertoolz():
    ...
    # XXX
    assert raises(TypeError, lambda: list(drop(None, [1, 2]))) or list(
        drop(None, [1, 2])
    ) == [1, 2]
    ...
    assert raises(
        TypeError,
        lambda: list(
            interleave([[1, 2], GenException(ValueError)], pass_exceptions=None)
        ),
    )
    ...
    # XXX
    assert raises(TypeError, lambda: list(mapcat(None, [[1], [2]]))) or list(
        mapcat(None, [[1], [2]])
    ) == [[1], [2]]
    ...
    # XXX
    assert (
        raises(TypeError, lambda: list(sliding_window(None, [1, 2, 3])))
        or list(sliding_window(None, [1, 2, 3])) == []
    )
    assert raises(TypeError, lambda: list(sliding_window(1, None)))
    tested.append("sliding_window")

    # XXX
    assert raises(TypeError, lambda: list(take(None, [1, 2])) == [1, 2]) or list(
        take(None, [1, 2])
    ) == [1, 2]
    ...
    # XXX
    assert raises(TypeError, lambda: list(tail(None, [1, 2])) == [1, 2]) or list(
        tail(None, [1, 2])
    ) == [1, 2]
    ...
    # XXX
    assert raises(TypeError, lambda: list(take_nth(None, [1, 2]))) or list(
        take_nth(None, [1, 2])
    ) == [1, 2]
...
def test_recipes():
    tested = []
    # XXX
    assert raises(TypeError, lambda: countby(None, [1, 2])) or countby(
        None, [1, 2]
    ) == {(1,): 1, (2,): 1}
    ...
    # XXX
    assert raises(TypeError, lambda: list(partitionby(None, [1, 2]))) or list(
        partitionby(None, [1, 2])
    ) == [(1,), (2,)]
