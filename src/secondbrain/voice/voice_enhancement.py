...
class VoiceEnhancer:
    ...
    def enhance_voice(
        ...
        try:
            ...
            profile = self.persona_manager.get_profile(
                name=persona_name, emotion=emotion, context=context
            )
    ...
    def rate_response(
        self, text: str, rating: float, persona_name: Optional[str] = None
    ) -> None:
        """
    ...
    def get_persona_stats(self) -> Dict[str, Any]:
        ...
        for name, persona in self.persona_manager.personas.items():
            ...
            rated_interactions = sum(
                1 for h in persona.interaction_history if h.response_rating is not None
            )
            avg_rating = 0.0
            if rated_interactions > 0:
                ratings = [
                    h.response_rating
                    for h in persona.interaction_history
                    if h.response_rating is not None
                ]
