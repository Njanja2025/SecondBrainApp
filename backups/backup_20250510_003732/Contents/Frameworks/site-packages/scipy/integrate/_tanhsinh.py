...
def tanhsinh(
    ...
    *,
    ...
    with np.errstate(over="ignore", invalid="ignore", divide="ignore"):
        ...
        c[inf_a] = b[inf_a] - 1.0  # takes care of infinite a
        c[inf_b] = a[inf_b] + 1.0  # takes care of infinite b
        c[inf_a & inf_b] = 0.0  # takes care of infinite a and b
        temp = eim._initialize(
            f, (c,), args, complex_ok=True, preserve_shape=preserve_shape, xp=xp
        )
    ...
    work = _RichResult(
        ...
        eps=eps,
        a=xp.reshape(a, (-1, 1)),
        b=xp.reshape(b, (-1, 1)),  # integration limits
        n=minlevel,
        nit=nit,
        nfev=nfev,
        status=status,  # iter/eval counts
        xr0=xr0,
        fr0=fr0,
        wr0=wr0,
        xl0=xl0,
        ...
        binf=binf,
        abinf=abinf,
        a0=xp.reshape(a0, (-1, 1)),  # transforms
        ...
        pair_cache=_RichResult(xjc=None, wj=None, indices=[0], h0=None),
    )  # pair cache
    ...
    def post_func_eval(x, fj, work):
        ...
        if work.log:
            fj[work.abinf] += xp.log(1 + work.xj[work.abinf] ** 2) - 2 * xp.log(
                1 - work.xj[work.abinf] ** 2
            )
        ...
        if work.Sk.shape[-1]:
            Snm1 = work.Sk[:, -1]
            Sn = (
                special.logsumexp(xp.stack([Snm1 - math.log(2), Sn]), axis=0)
                if log
                else Snm1 / 2 + Sn
            )
    ...
    def check_termination(work):
        ...
        if work.nit == 0:
            ...
            i = (
                (work.rerr < rtol) | (work.rerr + xp_real(work.Sn) < atol)
                if log
...
def _get_pairs(k, h0, inclusive, dtype, xp, work):
    ...
    if (
        len(work.pair_cache.indices) <= k + 2
...
def _euler_maclaurin_sum(fj, work, xp):
    ...
    Sn = (
        special.logsumexp(fjwj + xp.log(work.h), axis=-1)
        if work.log
...
def _estimate_error(work, xp):
    ...
    if work.Sk.shape[-1] == 0:
        ...
        Snm1 = (
            special.logsumexp(fjwj, **axis_kwargs) + xp.log(h)
            if work.log
    ...
    if work.log:
        ...
        d1 = xp_real(
            special.logsumexp(xp.stack([work.Sn, Snm1 + work.pi * 1j]), axis=0)
        )
        d2 = xp_real(
            special.logsumexp(xp.stack([work.Sn, Snm2 + work.pi * 1j]), axis=0)
        )
...
def _tanhsinh_iv(
    f, a, b, log, maxfun, maxlevel, minlevel, atol, rtol, args, preserve_shape, callback
):
    ...
    if xp.isdtype(a.dtype, "complex floating") or xp.isdtype(
        b.dtype, "complex floating"
    ):
        raise ValueError(message)
...
def nsum(f, a, b, *, step=1, args=(), log=False, maxterms=int(2**20), tolerances=None):
    ...
    if xp.any(i3):
        ...
        tmp = _integral_bound(
            _f, -b[i3], -a[i3], step[i3], args_indirect, constants, xp
        )
    ...
    return _RichResult(sum=S, error=E, status=status, success=status == 0, nfev=nfev)
...
def _integral_bound(f, a, b, step, args, constants, xp):
    ...
    k = a + n_steps * step
    left, left_error, left_nfev = _direct(
        f, a, k, step, args, constants, xp, inclusive=False
    )
