...
def beat_track(
    *,
    ...
    bpm_expanded = util.expand_to(_bpm, ndim=onset_envelope.ndim, axes=range(_bpm.ndim))

    # Then, run the tracker
    beats = __beat_tracker(
        onset_envelope, bpm_expanded, float(sr) / hop_length, tightness, trim
    )
...
def __beat_tracker(
    ...
    if bpm.shape[-1] not in (1, onset_envelope.shape[-1]):
        raise ParameterError(
            f"Invalid bpm shape={bpm.shape} does not match onset envelope shape={onset_envelope.shape}"
        )
...
def __beat_local_score(onset_envelope, frames_per_beat, localscore):
    ...
    if len(frames_per_beat) == 1:
        ...
        window = np.exp(
            -0.5
            * (
                np.arange(-frames_per_beat[0], frames_per_beat[0] + 1)
                * 32.0
                / frames_per_beat[0]
            )
            ** 2
        ...
        for i in range(len(onset_envelope)):
            window = np.exp(
                -0.5
                * (
                    np.arange(-frames_per_beat[i], frames_per_beat[i] + 1)
                    * 32.0
                    / frames_per_beat[i]
                )
                ** 2
...
def __beat_track_dp(localscore, frames_per_beat, tightness, backlink, cumscore):
    ...
    for i, score_i in enumerate(localscore):
        ...
        for loc in range(
            i - np.round(frames_per_beat[tv * i] / 2),
            i - 2 * frames_per_beat[tv * i] - 1,
            -1,
        ):
            ...
            score = (
                cumscore[loc]
                - tightness * (np.log(i - loc) - np.log(frames_per_beat[tv * i])) ** 2
            )
            if score > best_score:
                best_score = score
                beat_location = loc
...
def __trim_beats(localscore, beats, trim, beats_trimmed):
    ...
    smooth_boe = np.convolve(localscore[beats], w)[
...
@numba.guvectorize(
    ["void(int32[:], int32, bool_[:])", "void(int64[:], int64, bool_[:])"],
    "(t),()->(t)",
    nopython=True,
    cache=True,
)
