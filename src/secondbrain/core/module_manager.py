...
class ModuleManager:
    ...
    def backup_module(self, module_id: str, backup_dir: str = "backups") -> bool:
        ...
        try:
            module_path = self.system_map.get_module_path(module_id)
            backup_path = Path(backup_dir) / f"{module_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def validate_module(self, module_id: str) -> List[str]:
        ...
        if not config_path.exists():
            ...
            try:
                ...
                required_fields = ['id', 'name', 'description', 'dependencies', 'status', 'version']
                for field in required_fields:
                    if field not in config:
                        issues.append(f"Missing required field in configuration: {field}")
    ...
    def get_module_status(self, module_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Get directory sizes
            src_size = sum(f.stat().st_size for f in (module_path / "src").rglob("*") if f.is_file())
            test_size = sum(f.stat().st_size for f in (module_path / "tests").rglob("*") if f.is_file())
            doc_size = sum(f.stat().st_size for f in (module_path / "docs").rglob("*") if f.is_file())
