    ...
    if out is not None and arr.dtype.kind == "c":
        ...
        if out is None:
            ...
            if isinstance(arr, (complex, np.complexfloating)):
                res = complex(
                    fixup_signed_zero(arr.real, res.real),
...
class TestArrayMethods(MemoryLeakMixin, TestCase):
    ...
    def test_array_view(self):
        ...
        def check_err(arr, dtype):
            with self.assertRaises(ValueError) as raises:
                run(arr, dtype)
            self.assertEqual(
                str(raises.exception), "new type not compatible with array"
            )
    ...
    def check_np_frombuffer(self, pyfunc):
        ...
        self.assertEqual(
            "buffer size must be a multiple of element size", str(raises.exception)
        )
    ...
    def check_nonzero(self, pyfunc):
        ...
        for v in (0.0, 1.5, float("nan")):
            ...
            if numpy_version < (2, 1):
                ...
                msg = (
                    "Calling nonzero on 0d arrays is not allowed. Use "
            ...
            for inputs in [(1, np.int16(2), 0), (1, 2, 0)]:
                ...
                with self.assertRaises(permitted_errors) as raises:
                    ...
                    with warnings.catch_warnings():
                        ...
                        f(*inputs)
                    self.assertIn(
                        "Maximum allowed size exceeded", str(raises.exception)
                    )
    ...
    def test_itemset(self):
        ...
        def check_err(a):
            with self.assertRaises(ValueError) as raises:
                cfunc(a, 42)
            self.assertIn(
                "itemset(): can only write to an array of size 1", str(raises.exception)
            )
            ...
            for arr in arr_list:
                for axis in (0, 1, 2):
                    if axis > len(arr.shape) - 1:
                        continue
                    with self.subTest(
                        "Testing np.sum(axis) with {} " "input ".format(arr.dtype)
                    ):
                        self.assertPreciseEqual(
                            pyfunc(arr, axis=axis), cfunc(arr, axis=axis)
                        )
        ...
        for arr_list in all_test_arrays:
            for arr in arr_list:
                for out_dtype in out_dtypes[arr.dtype]:
                    subtest_str = "Testing np.sum with {} input and {} output".format(
                        arr.dtype, out_dtype
                    )
                    with self.subTest(subtest_str):
                        self.assertPreciseEqual(
                            pyfunc(arr, dtype=out_dtype), cfunc(arr, dtype=out_dtype)
                        )
    ...
    def _lower_clip_result_test_util(self, func, a, a_min, a_max):
        ...
        np.testing.assert_almost_equal(
            lower_clip_result(a), jit(nopython=True)(lower_clip_result)(a)
        )
            ...
            for pyfunc in has_out + has_no_out:
                ...
                if pyfunc in has_out:
                    ...
                    np.testing.assert_equal(
                        pyfunc(a, -5, 5, pyout), cfunc(a, -5, 5, cout)
                    )
            ...
            for a_min in mins:
                for a_max in maxs:
                    ...
                    if pyfunc in has_out:
                        ...
                        cout = np.empty_like(a)
                        np.testing.assert_equal(
                            pyfunc(a, a_min, a_max, pyout), cfunc(a, a_min, a_max, cout)
                        )
                        np.testing.assert_equal(pyout, cout)
