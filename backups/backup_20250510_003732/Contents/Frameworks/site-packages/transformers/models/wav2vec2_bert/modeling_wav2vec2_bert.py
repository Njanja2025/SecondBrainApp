...
class Wav2Vec2BertSelfAttention(nn.Module):
    ...
    def __init__(self, config, is_adapter_attention=False):
        ...
        hidden_size = (
            config.hidden_size
            if not is_adapter_attention
            else config.output_hidden_size
        )
...
class Wav2Vec2BertAdapterLayer(nn.Module):
    def __init__(self, config):
        ...
        self.ffn = Wav2Vec2BertFeedForward(
            config, act_fn=config.adapter_act, hidden_size=embed_dim
        )
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Wav2Vec2BertSelfAttention):
            ...
            if hasattr(module, "layer_weights"):
                module.layer_weights.data.fill_(
                    1.0 / (self.config.num_hidden_layers + 1)
                )
