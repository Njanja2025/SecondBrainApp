...
class TestHelperFuncs(TestCase):
    def test_proper_func_provider(self):
        ...
        self.assertEqual(
            _get_proper_func(test_32bit_func, test_64bit_func, np.float64)[0](), 64
        )
        self.assertEqual(
            _get_proper_func(test_32bit_func, test_64bit_func, np.float32)[0](), 32
        )
        ...
        self.assertIn(
            "Argument dtype is not one of the expected type(s)", str(raises.exception)
        )
        with self.assertRaises(TypingError) as raises:
    ...
    def test_integers_arg_check(self):
        rng = np.random.default_rng(1)
        py_func = lambda x, low, high, dtype: x.integers(
            low=low, high=high, dtype=dtype, endpoint=True
        )
        numba_func = numba.njit()(py_func)
        numba_func_low = numba.njit()(py_func)

        py_func = lambda x, low, high, dtype: x.integers(
            low=low, high=high, dtype=dtype, endpoint=False
        )
        ...
        self.assertIn(
            "low is greater than high in given interval", str(raises.exception)
        )
...
class TestRandomGenerators(MemoryLeakMixin, TestCase):
    def check_numpy_parity(
        ...
        if (
            isinstance(numba_res, np.ndarray)
            and np.issubdtype(numba_res.dtype, np.floating)
        ) or isinstance(numba_res, float):
            # Float scalars and arrays
            np.testing.assert_array_max_ulp(
                numpy_res, numba_res, maxulp=ulp_prec, dtype=test_dtype
            )
        else:
        ...
        for _state_key in numpy_gen_state:
            self.assertPreciseEqual(
                numba_gen_state[_state_key], numpy_gen_state[_state_key]
            )
    ...
    def test_integers(self):
        ...
        with self.subTest():
            self.check_numpy_parity(
                dist_func, test_size=None, test_dtype=None, ulp_prec=0
            )
            ...
            for _dtype in test_dtypes:
                for _bitgen in bitgen_types:
                    with self.subTest(_size=_size, _dtype=_dtype, _bitgen=_bitgen):
                        self.check_numpy_parity(
                            dist_func, _bitgen, None, _size, _dtype, 0
                        )
        ...
        dist_func = lambda x, size, dtype: x.integers(
            False, True, size=size, dtype=np.bool_
        )
        for _size in test_sizes:
            for _bitgen in bitgen_types:
                with self.subTest(_size=_size, _bitgen=_bitgen):
                    self.check_numpy_parity(
                        dist_func, _bitgen, None, _size, np.bool_, 0
                    )
        ...
        dist_func = lambda x, low, high, size, dtype, endpoint: x.integers(
    ...
    def test_random(self):
        ...
        dist_func = lambda x, size, dtype: x.random(size=size, dtype=dtype)
        self._check_invalid_types(
            dist_func, ["size", "dtype"], [(1,), np.float64], [("x",), 0.0]
        )
    ...
    def test_standard_normal(self):
        ...
        dist_func = lambda x, size, dtype: x.standard_normal(size=size, dtype=dtype)
        self._check_invalid_types(
            dist_func, ["size", "dtype"], [(1,), np.float32], [("x",), 0]
        )
    ...
    def test_standard_exponential(self):
        ...
        dist_func = lambda x, method, size, dtype: x.standard_exponential(
            method=method, size=size, dtype=dtype
        )
        self._check_invalid_types(
            dist_func,
            ["method", "size", "dtype"],
    ...
    def test_standard_exponential_inv(self):
        ...
        dist_func = lambda x, size, dtype: x.standard_exponential(
            size=size, dtype=dtype, method="inv"
        )
        for _size in test_sizes:
            for _dtype in test_dtypes:
                for _bitgen in bitgen_types:
    ...
    def test_standard_gamma(self):
        ...
        dist_func = lambda x, size, dtype: x.standard_gamma(
            shape=5.0, size=size, dtype=dtype
        )
        for _size in test_sizes:
            for _dtype in test_dtypes:
                for _bitgen in bitgen_types:
                    with self.subTest(_size=_size, _dtype=_dtype, _bitgen=_bitgen):
                        self.check_numpy_parity(
                            dist_func, _bitgen, None, _size, _dtype, adjusted_ulp_prec
                        )
        dist_func = lambda x, shape, size, dtype: x.standard_gamma(
    ...
    def test_normal(self):
        ...
        with self.subTest():
            self.check_numpy_parity(
                dist_func, test_size=None, test_dtype=None, ulp_prec=adjusted_ulp_prec
            )
        ...
        for _size in test_sizes:
            for _bitgen in bitgen_types:
                with self.subTest(_size=_size, _bitgen=_bitgen):
                    self.check_numpy_parity(
                        dist_func, _bitgen, None, _size, None, adjusted_ulp_prec
                    )
        dist_func = lambda x, loc, scale, size: x.normal(
            loc=loc, scale=scale, size=size
        )
        self._check_invalid_types(
            dist_func, ["loc", "scale", "size"], [1.5, 3, (1,)], ["x", "x", ("x",)]
        )
    ...
    def test_uniform(self):
        ...
        self._check_invalid_types(
            dist_func, ["low", "high", "size"], [1.5, 3, (1,)], ["x", "x", ("x",)]
        )
    ...
    def test_exponential(self):
        ...
        self._check_invalid_types(
            dist_func, ["scale", "size"], [1.5, (1,)], ["x", ("x",)]
        )
    ...
    def test_gamma(self):
        ...
        dist_func = lambda x, shape, scale, size: x.gamma(
            shape=shape, scale=scale, size=size
        )
        self._check_invalid_types(
            dist_func, ["shape", "scale", "size"], [5.0, 1.5, (1,)], ["x", "x", ("x",)]
        )
    ...
    def test_beta(self):
        ...
        self._check_invalid_types(
            dist_func, ["a", "b", "size"], [5.0, 1.5, (1,)], ["x", "x", ("x",)]
        )
    ...
    def test_f(self):
        ...
        dist_func = lambda x, dfnum, dfden, size: x.f(
            dfnum=dfnum, dfden=dfden, size=size
        )
        self._check_invalid_types(
            dist_func, ["dfnum", "dfden", "size"], [5, 1, (1,)], ["x", "x", ("x",)]
        )
    ...
    def test_laplace(self):
        ...
        dist_func = lambda x, loc, scale, size: x.laplace(
            loc=loc, scale=scale, size=size
        )
        self._check_invalid_types(
            dist_func, ["loc", "scale", "size"], [1.0, 1.5, (1,)], ["x", "x", ("x",)]
        )
    ...
    def test_logistic(self):
        ...
        dist_func = lambda x, loc, scale, size: x.logistic(
            loc=loc, scale=scale, size=size
        )
        self._check_invalid_types(
            dist_func, ["loc", "scale", "size"], [1.0, 1.5, (1,)], ["x", "x", ("x",)]
    ...
    def test_lognormal(self):
        ...
        dist_func = lambda x, mean, sigma, size: x.lognormal(
            mean=mean, sigma=sigma, size=size
        )
        self._check_invalid_types(
            dist_func, ["mean", "sigma", "size"], [1.0, 1.5, (1,)], ["x", "x", ("x",)]
        )
    ...
    def test_wald(self):
        ...
        dist_func = lambda x, mean, scale, size: x.wald(
            mean=mean, scale=scale, size=size
        )
        self._check_invalid_types(
            dist_func, ["mean", "scale", "size"], [1.0, 1.5, (1,)], ["x", "x", ("x",)]
        )
    ...
    def test_triangular(self):
        ...
        dist_func = lambda x, size, dtype: x.triangular(
            left=0, mode=3, right=5, size=size
        )
        for _size in test_sizes:
            for _bitgen in bitgen_types:
                with self.subTest(_size=_size, _bitgen=_bitgen):
        ...
        dist_func = lambda x, left, mode, right, size: x.triangular(
    ...
    def test_negative_binomial(self):
        ...
        self._check_invalid_types(
            dist_func, ["n", "p", "size"], [1, 0.75, (1,)], ["x", "x", ("x",)]
        )
    ...
    def test_shuffle_check(self):
        ...
        self._check_invalid_types(
            dist_func, ["x", "axis"], [np.array([3, 4, 5]), 0], ["x", "x"]
        )
        ...
        self.assertIn(
            "Axis is out of bounds for the given array", str(raises.exception)
        )
    ...
    def test_noncentral_chisquare(self):
        ...
        dist_func = lambda x, df, nonc, size: x.noncentral_chisquare(
            df=df, nonc=nonc, size=size
        )
        valid_args = [3.0, 5.0, (1,)]
        self._check_invalid_types(
            dist_func, ["df", "nonc", "size"], valid_args, ["x", "x", ("x",)]
        )
    ...
    def test_noncentral_f(self):
        ...
        dist_func = lambda x, dfnum, dfden, nonc, size: x.noncentral_f(
