...
class SecurityMonitor:
    ...
    def _check_rapid_failed_attempts(self, user_id: str) -> bool:
        ...
        return (
            metrics.failed_attempts >= self.thresholds["failed_logins_per_minute"]
            and time_diff.total_seconds() < 60
        )
    ...
    def _is_unusual_time(self, timestamp: datetime) -> bool:
        ...
        hour = timestamp.hour
        return (
            hour >= self.time_windows["unusual_hours"][0]
            or hour < self.time_windows["unusual_hours"][1]
        )
    ...
    def _check_ip_reputation(self, ip_address: str, user_id: str, username: str):
        ...
        if (
            len(rep["associated_users"]) > 3
            or rep["attempts"] > self.thresholds["suspicious_ip_requests"]
        ):
            self._trigger_alert(
                "suspicious_ip",
                "high",
    ...
    def _trigger_alert(
        ...
        alert = SecurityAlert(
            alert_type=alert_type, severity=severity, message=message, details=details
        )
    ...
    def get_user_metrics(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_attempt": (
                metrics.last_attempt.isoformat() if metrics.last_attempt else None
            ),
