...
def _chandrupatla(
    ...
    *,
    ...
    f1, f2 = fs
    status = xp.full_like(
        x1, xp.asarray(eim._EINPROGRESS), dtype=xp.int32
    )  # in progress
    ...
    maxiter = (
        math.log2(finfo.max) - math.log2(finfo.smallest_normal)
        if maxiter is None
        else maxiter
    )
    work = _RichResult(
    ...
    def post_termination_check(work):
        ...
        t[j] = f1j / (f1j - f2j) * f3j / (f3j - f2j) - alphaj * f1j / (
            f3j - f1j
        ) * f2j / (f2j - f3j)
...
def _chandrupatla_iv(func, args, xatol, xrtol, fatol, frtol, maxiter, callback):
    ...
    tols = np.asarray(
        [
            xatol if xatol is not None else 1,
...
def _chandrupatla_minimize(
    ...
    *,
    ...
    def post_func_eval(x, f, work):
        ...
        work.x1[ni], work.x2[ni], work.x3[ni] = (
            x1ni,
            x2ni,
            x3ni,
        )
