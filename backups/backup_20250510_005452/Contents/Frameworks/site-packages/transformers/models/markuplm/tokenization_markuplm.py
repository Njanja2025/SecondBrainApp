    ...
    def prepare_for_model(
        ...
        *truncation_strategy = longest_first* or `True`, it is not possible to return overflowing tokens. Such a
            ...
            if node_labels is None:
                ...
                for word, xpath, label in zip(text, xpaths, node_labels):
                    ...
                    if self.only_label_first_subword:
                        ...
                        labels.extend(
                            [label] + [self.pad_token_label] * (len(word_tokens) - 1)
                        )
                    else:
        ...
        if add_special_tokens:
            ...
            xpath_tags_ids = self.build_xpath_tags_with_special_tokens(
                xpath_tags_seq, pair_xpath_tags_seq
            )
            xpath_subs_ids = self.build_xpath_subs_with_special_tokens(
                xpath_subs_seq, pair_xpath_subs_seq
            )
            if labels:
            ...
            xpath_tags_ids = (
                xpath_tags_seq + pair_xpath_tags_seq if pair else xpath_tags_seq
            )
            xpath_subs_ids = (
                xpath_subs_seq + pair_xpath_subs_seq if pair else xpath_subs_seq
            )
