...
def create_payment_processor(config: dict) -> PaymentProcessor:
    ...
    try:
        return PaymentProcessor(config_path=config.get("payment_config_path", "config/payment_config.json"))
...
def handle_create_subscription(args, processor: PaymentProcessor):
    ...
    try:
        ...
        with open(log_file, "a") as f:
            f.write(f"[{datetime.now()}] Subscription started for {args.email} (Tier: {args.tier})\n")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Payment and subscription management CLI")
    parser.add_argument("--config", default="config/payment.json", help="Path to configuration file")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    
    subparsers = parser.add_subparsers(dest="command", help="Command to execute")
    
    # Create subscription command
    create_subscription = subparsers.add_parser("create-subscription", help="Create a subscription")
    create_subscription.add_argument("--email", required=True, help="Customer email")
    create_subscription.add_argument("--tier", required=True, choices=["basic", "premium", "enterprise"], help="Subscription tier")
    
    # Start webhook server command
    webhook_server = subparsers.add_parser("start-webhook-server", help="Start webhook server")
    ...
    webhook_server.add_argument("--debug", action="store_true", help="Enable debug mode")
