    ...
    def _save_services(self):
        ...
        try:
            ...
            with open(config_file, "w") as f:
                json.dump(
                    {name: vars(config) for name, config in self.services.items()},
                    f,
                    indent=2,
                )
...
if __name__ == "__main__":
    ...
    config = ServiceConfig(
        name="example_service", api_key="test_key", base_url="https://api.example.com"
    )
    manager.add_service(config)

    # Make a request
    response = manager.make_request(
        service_name="example_service", endpoint="data", method="GET"
    )
