...
class CallInstr(Instruction):
    ...
    def _descr(self, buf, add_metadata):
        ...
        fn_attrs = (
            " " + " ".join(self.attributes._to_list(fnty.return_type))
            if self.attributes
            else ""
        )

        fm_attrs = (
            " " + " ".join(self.fastmath._to_list(fnty.return_type))
            if self.fastmath
            else ""
        )
...
class InvokeInstr(CallInstr):
    def __init__(
        ...
        super(InvokeInstr, self).__init__(
            parent,
            func,
            args,
            name,
...
class Terminator(Instruction):
    def __init__(self, parent, opname, operands):
        super(Terminator, self).__init__(parent, types.VoidType(), opname, operands)
...
class SwitchInstr(PredictableInstr, Terminator):
    ...
    def descr(self, buf):
        cases = [
            "{0} {1}, label {2}".format(
                val.type, val.get_reference(), blk.get_reference()
            )
...
class SelectInstr(Instruction):
    def __init__(self, parent, cond, lhs, rhs, name="", flags=()):
        ...
        super(SelectInstr, self).__init__(
...
class CompareInstr(Instruction):
    ...
    def __init__(self, parent, op, lhs, rhs, name="", flags=[]):
        ...
        if isinstance(lhs.type, types.VectorType):
            ...
            typ = types.IntType(1)
        super(CompareInstr, self).__init__(
            parent, typ, opname, [lhs, rhs], flags=flags, name=name
        )
        self.op = op
...
class LoadAtomicInstr(Instruction):
    def __init__(self, parent, ptr, ordering, align, name="", typ=None):
        ...
        super(LoadAtomicInstr, self).__init__(
            parent, typ, "load atomic", [ptr], name=name
        )
...
class StoreAtomicInstr(Instruction):
    def __init__(self, parent, val, ptr, ordering, align):
        super(StoreAtomicInstr, self).__init__(
            parent, types.VoidType(), "store atomic", [val, ptr]
        )
        self.ordering = ordering
        self.align = align
...
class AllocaInstr(Instruction):
    def __init__(self, parent, typ, count, name):
        ...
        super(AllocaInstr, self).__init__(
            parent, typ.as_pointer(), "alloca", operands, name
        )
...
class GEPInstr(Instruction):
    def __init__(self, parent, ptr, indices, inbounds, name, source_etype=None):
        ...
        super(GEPInstr, self).__init__(
            parent, typ, "getelementptr", [ptr] + list(indices), name=name
        )
        self.pointer = ptr
...
class ExtractElement(Instruction):
    def __init__(self, parent, vector, index, name=""):
        ...
        super(ExtractElement, self).__init__(
            parent, typ, "extractelement", [vector, index], name=name
        )
...
class InsertElement(Instruction):
    def __init__(self, parent, vector, value, index, name=""):
        ...
        if not isinstance(index.type, types.IntType):
            raise TypeError("index needs to be of IntType.")
        typ = vector.type
        super(InsertElement, self).__init__(
...
class ShuffleVector(Instruction):
    ...
    def descr(self, buf):
        buf.append(
            "shufflevector {0} {1}\n".format(
                ", ".join(
                    "{0} {1}".format(op.type, op.get_reference())
                    for op in self.operands
                ),
                self._stringify_metadata(leading_comma=True),
            )
...
class ExtractValue(Instruction):
    def __init__(self, parent, agg, indices, name=""):
        ...
        super(ExtractValue, self).__init__(
            parent, typ, "extractvalue", [agg], name=name
        )
...
class InsertValue(Instruction):
    def __init__(self, parent, agg, elem, indices, name=""):
        ...
        super(InsertValue, self).__init__(
            parent, agg.type, "insertvalue", [agg, elem], name=name
        )
...
class Unreachable(Instruction):
    def __init__(self, parent):
        super(Unreachable, self).__init__(
            parent, types.VoidType(), "unreachable", (), name=""
        )
...
class AtomicRMW(Instruction):
    def __init__(self, parent, op, ptr, val, ordering, name):
        super(AtomicRMW, self).__init__(
            parent, val.type, "atomicrmw", (ptr, val), name=name
        )
        self.operation = op
        self.ordering = ordering
...
class _LandingPadClause(object):
    ...
    def __str__(self):
        return "{kind} {type} {value}".format(
            kind=self.kind, type=self.value.type, value=self.value.get_reference()
        )
...
class Comment(Instruction):
    ...
    def __init__(self, parent, text):
        super(Comment, self).__init__(parent, types.VoidType(), ";", (), name="")
