...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
        ...
        for item, typ in expanded:
            ...
            with self.binder.top_frame_context():
                ...
                if typ.type_is:
                    ...
                    if ref_type is not None and (
                        not defn.is_static or defn.name == "__new__"
                    ):
                        arg_index = 1
                    ...
                    if (
                        ...
                        if not is_same_type(arg_type, ref_type):
                            ...
                            erased = get_proper_type(
                                erase_typevars(erase_to_bound(arg_type))
                            )
                            if not is_subtype(
                        ...
                        if (
                            ...
                            and defn.name
                            not in ("__init__", "__new__", "__post_init__")
                            and not is_private(
                                defn.name
    ...
    def check_method_override(
        ...
        check_override_compatibility = (
            defn.name
            not in ("__init__", "__new__", "__init_subclass__", "__post_init__")
            ...
            if is_property(defn):
                ...
                if inner is not None:
                    ...
                    if (
                        ...
                        and (
                            not original_node.is_property
                            or original_node.is_settable_property
                        )
    ...
    def bind_and_map_method(
        ...
        """
        if isinstance(
            sym.node, (FuncDef, OverloadedFuncDef, Decorator)
    ...
    def check_override(
        ...
        if fail:
            ...
            offset_arguments = (
                isinstance(override, CallableType) and override.unpack_kwargs
            )
            ...
            if (
                ...
                for i, (sub_kind, super_kind) in enumerate(
                    ...
                    if sub_kind.is_positional() and super_kind.is_positional():
                        ...
                        sub_kind.is_named()
                        and super_kind.is_named()
                        and not offset_arguments
                    ):
    ...
    def visit_class_def(self, defn: ClassDef) -> None:
        ...
        for base in typ.mro[1:]:
            if base.is_final:
                self.fail(
                    message_registry.CANNOT_INHERIT_FROM_FINAL.format(base.name), defn
                )
        with (
    ...
    def check_enum(self, defn: ClassDef) -> None:
        ...
        if defn.info.fullname not in ENUM_BASES and "__members__" in defn.info.names:
            ...
            if isinstance(sym.node, Var) and sym.node.has_explicit_value:
                ...
                self.fail(
                    message_registry.ENUM_MEMBERS_ATTR_WILL_BE_OVERRIDDEN, sym.node
                )
    ...
    def check_multiple_inheritance(self, typ: TypeInfo) -> None:
        ...
        for name in sorted(all_names - typ.names.keys()):
            ...
            # the used definition.
            i, base = next(
                (i, base) for i, base in enumerate(mro) if name in base.names
            )
    ...
    def check_compatibility(
        ...
        if call and isinstance(second_type, FunctionLike):
            ...
            if isinstance(first.node, Var):
                first_type = get_proper_type(
                    map_type_from_supertype(first_type, ctx, base1)
                )
                first_type = expand_self_type(
                    first.node, first_type, fill_typevars(ctx)
                )
            if isinstance(second.node, Var):
                second_type = get_proper_type(
                    map_type_from_supertype(second_type, ctx, base2)
                )
        ...
        if isinstance(lvalue, (TupleExpr, ListExpr)):
            ...
            if lvalue_type:
                if isinstance(lvalue_type, PartialType) and lvalue_type.type is None:
                    ...
                    rvalue_type, lvalue_type, infer_lvalue_type = (
                ...
                if (
                    ...
                    if not (isinstance(lvalue, NameExpr) and lvalue.is_special_form):
                        ...
                        if (
                            ...
                            lvalue.node.type = remove_instance_last_known_values(
                                lvalue_type
                            )
        ...
        if self.check_rvalue_count_in_assignment(
            ...
            if not undefined_rvalue:
                ...
                if isinstance(reinferred_rvalue_type, TypeVarLikeType):
                    reinferred_rvalue_type = get_proper_type(
                        reinferred_rvalue_type.upper_bound
                    )
        ...
        *,
        ...
        if self.is_stub and isinstance(rvalue, EllipsisExpr):
            ...
            if (
                ...
                with (
                    self.msg.filter_errors() as local_errors,
    ...
    def check_member_assignment(
        ...
        mx.chk.warn_deprecated_overload_item(
            dunder_set,
            mx.context,
            target=inferred_dunder_set_type,
            selftype=attribute_type,
        )
    ...
    def visit_operator_assignment_stmt(self, s: OperatorAssignmentStmt) -> None:
        ...
        if inplace:
            ...
            if isinstance(
                inst := get_proper_type(lvalue_type), Instance
    ...
    def visit_match_stmt(self, s: MatchStmt) -> None:
        ...
        if isinstance(s.subject, CallExpr):
            ...
            # This is hack!
            id = (
                s.subject.callee.fullname
                if isinstance(s.subject.callee, RefExpr)
                else ""
            )
            name = "dummy-match-" + id
            v = Var(name)
    ...
    def _get_recursive_sub_patterns_map(
        ...
        if isinstance(expr, TupleExpr) and isinstance(typ_, TupleType):
            ...
            for item_expr, item_typ in zip(expr.items, typ_.items):
                ...
                sub_patterns_map.update(
                    self._get_recursive_sub_patterns_map(item_expr, item_typ)
                )
    ...
    def conditional_callable_type_map(
        ...
        if callables and uncallables:
            callable_map = (
                {expr: UnionType.make_union(callables)} if callables else None
            )
            uncallable_map = (
    ...
    def find_type_equals_check(
        ...
        for expr in exprs_in_type_calls:
            current_if_type, current_else_type = (
    ...
    def find_isinstance_check_helper(
        ...
        if isinstance(node, CallExpr) and len(node.args) != 0:
            ...
            if refers_to_fullname(node.callee, "builtins.isinstance"):
                ...
                if (
                    node.callee.type_guard is not None
                    or node.callee.type_is is not None
                ):
                        ...
                        if node.callee.type_guard is not None:
                            ...
                            return conditional_types_to_typemaps(
                                ...
                                *self.conditional_types_with_intersection(
                                    ...
                                    [
                                        TypeRange(
                                            node.callee.type_is, is_upper_bound=False
                                        )
                                    ],
                                    expr,
                                ),
            ...
            if_map = (
                {node: yes_type} if not isinstance(yes_type, UninhabitedType) else None
            )
            else_map = (
    ...
    def comparison_type_narrowing_helper(
        ...
        if not non_optional_types or (len(non_optional_types) != len(chain_indices)):
            ...
            for i in narrowable_operand_indices:
                ...
                if not is_overlapping_none(expr_type):
                    continue
                if any(
                    is_overlapping_erased_types(expr_type, t)
                    for t in non_optional_types
                ):
    ...
    def get_isinstance_type(self, expr: Expression) -> list[TypeRange] | None:
        ...
        for typ in all_types:
            if isinstance(typ, FunctionLike) and typ.is_type_obj():
                ...
                isinstance(typ, Instance)
                and typ.type.fullname == "types.UnionType"
                and typ.args
            ):
    ...
    def check_deprecated(self, node: Node | None, context: Context) -> None:
        ...
        if isinstance(node, (FuncDef, OverloadedFuncDef, TypeInfo)) and (
            ...
            for imp in self.tree.imports:
                if isinstance(imp, ImportFrom) and any(
                    node.name == n[0] for n in imp.names
                ):
                    break
            else:
    ...
    def warn_deprecated(self, node: Node | None, context: Context) -> None:
        ...
        if (
            ...
            warn = (
                self.msg.note
                if self.options.report_deprecated_as_note
                else self.msg.fail
            )
...
def builtin_item_type(tp: Type) -> Type | None:
    ...
    if isinstance(tp, Instance):
        ...
        if all(
            not isinstance(it, AnyType) for it in get_proper_types(normalized_items)
        ):
            return make_simplified_union(
                normalized_items
