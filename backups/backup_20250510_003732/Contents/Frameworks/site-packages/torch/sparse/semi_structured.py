...
class SparseSemiStructuredTensor(torch.Tensor):
    ...
    def __new__(
        ...
        """
        assert compressed_tensor_cusparselt is None or (
            sparse_tensor_cutlass is None and meta_tensor_cutlass is None
        )
    ...
    def __init__(
        ...
        if original_tensor is not None:
            ...
            if self._FORCE_CUTLASS:
                ...
                sparse_tensor_cutlass, meta_tensor_cutlass = (
    ...
    def __tensor_unflatten__(inner_tensors, meta):
        ...
        if len(inner_tensors) == 2:
            ...
            raise RuntimeError(
                f"Expected 1 or 2 inner tensors but got {len(inner_tensors)}"
            )
    ...
    def __torch_dispatch__(cls, func, types, args, kwargs) -> Any:
        ...
        if func is torch.ops.aten.addmm.default:
            ...
            if isinstance(input_B, cls) and input_B.transposed:
                ...
                if input_B.compressed_tensor_cusparselt is None:
                    assert (
                        input_B.sparse_tensor_cutlass is not None
                        and input_B.meta_tensor_cutlass is not None
                    )
        ...
        if func is torch.ops.aten.mm.default:
            ...
            if isinstance(input_A, cls) and not input_A.transposed:
                ...
                if input_A.compressed_tensor_cusparselt is None:
                    assert (
                        input_A.sparse_tensor_cutlass is not None
                        and input_A.meta_tensor_cutlass is not None
                    )
                    res = torch._sparse_semi_structured_linear(
                        input_B_padded.t(),
        ...
        if func is torch.ops.aten.linear.default:
            ...
            if isinstance(weight, cls):
                if weight.compressed_tensor_cusparselt is None:
                    assert (
                        weight.sparse_tensor_cutlass is not None
                        and weight.meta_tensor_cutlass is not None
                    )
                    res = torch._sparse_semi_structured_linear(
        ...
        if func is torch.ops.aten.values.default:
            if args[0].compressed_tensor_cusparselt is None:
                ...
                return (
                    args[0]
                    .compressed_tensor_cusparselt[:num_kept_elements]
                    .view(m, k // 2)
                )
    ...
    def to_dense(self):
        if self.compressed_tensor_cusparselt is not None:
            raise RuntimeError(
                "Converting to dense is not yet supported by cuSPARSELt backend!"
            )
