...
class BackupTestRunner:
    ...
    async def run_memory_backup_test(self):
        ...
        try:
            ...
            dropbox_result = await self.cloud_sync.upload_to_dropbox(
                str(memory_path), self.env_vars["cloud"]["DROPBOX_ACCESS_TOKEN"]
            )
            ...
            if success:
                ...
                self.log_reporter.log_event(
                    ...
                    "SUCCESS",
                    {"dropbox": dropbox_result, "drive": drive_result, "s3": s3_result},
                )
            else:
    ...
    async def run_dns_health_test(self):
        ...
        try:
            ...
            if not verify_result:
                self.log_reporter.log_event(
                    "dns", "DNS verification failed, attempting update"
                )
    ...
    async def run_all_tests(self):
        ...
        await self.voice.speak(
            ...
            if success
            else "Some tests failed. Please check the report."
        )
