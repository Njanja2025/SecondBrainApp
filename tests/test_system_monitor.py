...
class TestSystemMonitorPlugin:
    ...
    def test_get_system_metrics(
        ...
        mock_temp.return_value = {
            "coretemp": [MagicMock(current=mock_metrics.temperature)]
        }
        ...
        assert (
            abs(metrics.uptime - mock_metrics.uptime) < 1
    ...
    def test_processes_command(self, plugin):
        ...
        with patch(
            "psutil.process_iter",
            return_value=[MagicMock(info=proc) for proc in mock_processes],
        ):
