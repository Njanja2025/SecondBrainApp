...
def _lower_parfor_parallel_std(lowerer, parfor):
    ...
    if nredvars > 0:
        ...
        for i in range(nredvars):
            ...
            glbl_np_empty = pfbdr.bind_global_function(
                ...
                args=(types.UniTuple(types.intp, redarrdim),),
            ...
            if init_val is not None:
                if isinstance(redvar_typ, types.npytypes.Array):
                    ...
                    full_call = pfbdr.call(
                        full_func_node,
                        args=[redshape_var, init_val_var, dt],
                    )
                ...
                if config.DEBUG_ARRAY_OPT_RUNTIME:
                    ...
                    lowerer.fndesc.calltypes[res_print] = signature(
                        types.none, typemap[lhs.name], typemap[redvar.name]
                    )
            ...
            with cgutils.for_range(
                builder, lowerer.loadvar(num_threads_var.name), intp=ntllvm_type
            ) as loop:
                ...
                pfbdr.setitem(
                    obj=redarr_var, index=numba_ir_loop_index_var, val=redtoset
                )
    ...
    try:
        (func, func_args, func_sig, func_arg_types, exp_name_to_tuple_var) = (
...
def _lower_trivial_inplace_binops(parfor, lowerer, thread_count_var, reduce_info):
    ...
    for inst in reduce_info.redvar_info.reduce_nodes:
        ...
        elif _is_right_op_and_rhs_is_init(
            inst, reduce_info.redvar_name, "inplace_binop"
        ):
            fn = inst.value.fn
        ...
        if _fix_redvar_name_ssa_mismatch(
            parfor, lowerer, inst, reduce_info.redvar_name
        ):
            break
    if config.DEBUG_ARRAY_OPT_RUNTIME:
...
def _lower_non_trivial_reduce(parfor, lowerer, thread_count_var, reduce_info):
    ...
    with cgutils.for_range(lowerer.builder, num_thread_llval) as loop:
        ...
        for inst in reduce_info.redvar_info.reduce_nodes:
            ...
            elif isinstance(inst, ir.Assign) and any(
...
def _emit_binop_reduce_call(binop, lowerer, thread_count_var, reduce_info):
    ...
    if config.DEBUG_ARRAY_OPT_RUNTIME:
        ...
        res_print = ir.Print(
            args=[reduce_info.redarr_var, init_var],
            vararg=None,
            loc=lowerer.loc,
        )
        typemap = lowerer.fndesc.typemap
        lowerer.fndesc.calltypes[res_print] = signature(
            types.none,
            typemap[reduce_info.redarr_var.name],
...
def _create_shape_signature(
    get_shape_classes, num_inputs, num_reductions, args, func_sig, races, typemap
):
...
def compute_def_once_block(
    block, def_once, def_more, getattr_taken, typemap, module_assigns
):
    ...
    for one_assign in assignments:
        ...
        if (
            ...
            and rhs.op == "getattr"
            and rhs.value.name in def_once
        ):
        ...
        if (
            isinstance(rhs, ir.Expr)
            and rhs.op == "call"
            and rhs.func.name in getattr_taken
        ):
...
def _hoist_internal(
    inst, dep_on_param, call_table, hoisted, not_hoisted, typemap, stored_arrays
):
    if inst.target.name in stored_arrays:
        ...
        if config.DEBUG_ARRAY_OPT >= 1:
            print(
                "Instruction",
                inst,
                "could not be hoisted because the created array is stored.",
            )
        return False
    ...
    if len(diff) == 0 and is_pure(inst.value, None, call_table):
        ...
        if len(diff) > 0:
            ...
            if config.DEBUG_ARRAY_OPT >= 1:
                print(
                    "Instruction", inst, "could not be hoisted because of a dependency."
                )
        else:
            not_hoisted.append((inst, "not pure"))
            if config.DEBUG_ARRAY_OPT >= 1:
                print(
                    "Instruction", inst, "could not be hoisted because it isn't pure."
                )
    return False
...
def empty_container_allocator_hoist(
    inst, dep_on_param, call_table, hoisted, not_hoisted, typemap, stored_arrays
):
    if (
...
def hoist(parfor_params, loop_body, typemap, wrapped_blocks):
    ...
    for label, block in loop_body.items():
        new_block = []
        for inst in block.body:
            if empty_container_allocator_hoist(
                inst, dep_on_param, call_table, hoisted, not_hoisted, typemap, itemsset
            ):
                continue
...
def _create_gufunc_for_parfor_body(
    ...
    for pi in parfor_inputs:
        ...
        if isinstance(pi_type, types.UniTuple) or isinstance(
            pi_type, types.NamedUniTuple
        ):
    ...
    if config.DEBUG_ARRAY_OPT >= 1:
        print(
            "parfor_inputs post tuple handling = ",
            parfor_inputs,
            " ",
            type(parfor_inputs),
        )
    ...
    param_dict = legalize_names_with_typemap(
        parfor_params + parfor_redvars + parfor_tuple_params, typemap
    )
    if config.DEBUG_ARRAY_OPT >= 1:
    ...
    for tup_var, exp_names in tuple_var_to_expanded_names.items():
        ...
        if isinstance(tup_type, types.NamedTuple) or isinstance(
            tup_type, types.NamedUniTuple
        ):
            named_tup = True
        else:
            named_tup = False
        ...
        if named_tup:
            ...
            if func_def is not None:
                if isinstance(func_def, ir.Expr) and func_def.op == "call":
                    named_tuple_def = guard(
                        get_definition, lowerer.func_ir, func_def.func
                    )
                    if config.DEBUG_ARRAY_OPT:
                        print(
                            "named_tuple_def:", named_tuple_def, type(named_tuple_def)
                        )
                    ...
                    if config.DEBUG_ARRAY_OPT:
                        print(
                            "named_tuple_def:",
                            named_tuple_def,
                            type(named_tuple_def),
                            named_tuple_def.name,
                        )
            if named_tuple_def is not None:
                if isinstance(named_tuple_def, ir.Global) or isinstance(
                    named_tuple_def, ir.FreeVar
                ):
                ...
                raise CompilerError(
                    "Could not find definition of " + str(tup_var), tup_var.loc
                )
    ...
    for arr, var in zip(parfor_redarrs, parfor_redvars):
        ...
        if config.DEBUG_ARRAY_OPT_RUNTIME:
            gufunc_txt += (
                '    print("thread id =", ParallelAcceleratorGufuncThreadId)\n'
            )
            gufunc_txt += (
                '    print("initial reduction value",ParallelAcceleratorGufuncThreadId,'
                + param_dict[var]
                + ","
                + param_dict[var]
                + ".shape)\n"
            ...
            gufunc_txt += (
                '    print("reduction array",ParallelAcceleratorGufuncThreadId,'
                + param_dict[arr]
                + ","
                + param_dict[arr]
                + ".shape)\n"
    ...
    for arr, var in zip(parfor_redarrs, parfor_redvars):
        if config.DEBUG_ARRAY_OPT_RUNTIME:
            gufunc_txt += (
                '    print("final reduction value",ParallelAcceleratorGufuncThreadId,'
                + param_dict[var]
                + ")\n"
            )
            gufunc_txt += (
                '    print("final reduction array",ParallelAcceleratorGufuncThreadId,'
                + param_dict[arr]
                + ")\n"
            )
    ...
    class ParforGufuncCompiler(compiler.CompilerBase):
        def define_pipelines(self):
            ...
            lowering_passes = dpb.define_parfor_gufunc_nopython_lowering_pipeline(
                self.state
            )
...
def call_parallel_gufunc(
    ...
    info = build_gufunc_wrapper(
        llvm_func, cres, sin, sout, cache=False, is_parfors=True
    )
    ...
    with cgutils.if_unlikely(
        builder, builder.icmp_signed("<=", num_threads, num_threads.type(0))
    ):
    ...
    get_sched_size_fnty = llvmlite.ir.FunctionType(
        uintp_t, [uintp_t, uintp_t, intp_ptr_t, intp_ptr_t]
    )
    get_sched_size = cgutils.get_or_insert_function(
        builder.module, get_sched_size_fnty, name="get_sched_size"
    )
    num_divisions = builder.call(
        get_sched_size,
        [
            num_threads,
    ...
    alloc_sched_func = cgutils.get_or_insert_function(
        builder.module, alloc_sched_fnty, name="allocate_sched"
    )
    ...
    if index_var_typ.signed:
        do_scheduling = cgutils.get_or_insert_function(
            builder.module, scheduling_fnty, name="do_scheduling_signed"
        )
    else:
        do_scheduling = cgutils.get_or_insert_function(
            builder.module, scheduling_fnty, name="do_scheduling_unsigned"
        )
        ...
        if i >= ninouts:  # reduction variables
            ...
            if i < num_inps:
                ...
                if isinstance(aty, types.Optional):
                    ...
                    unpacked_aty = aty
                typ = (
                    context.get_data_type(unpacked_aty)
    ...
    fnty = llvmlite.ir.FunctionType(
        llvmlite.ir.VoidType(), [byte_ptr_ptr_t, intp_ptr_t, intp_ptr_t, byte_ptr_t]
    )
    ...
    dealloc_sched_fnty = llvmlite.ir.FunctionType(
        llvmlite.ir.VoidType(), [sched_ptr_type]
    )
    dealloc_sched_func = cgutils.get_or_insert_function(
        builder.module, dealloc_sched_fnty, name="deallocate_sched"
    )
