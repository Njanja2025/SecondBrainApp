...
class AccessControl:
    ...
    def _initialize_roles(self):
        ...
        self.roles = {
            ...
            "admin": Role("admin", "System administrator", {"read", "write", "delete", "admin"}, 3),
            "superuser": Role("superuser", "Superuser", {"read", "write", "delete", "admin", "superuser"}, 4)
        }
    ...
    def _load_users(self):
        ...
        if self.users_file.exists():
            with open(self.users_file, 'r') as f:
                self.users = {username: User(**data) for username, data in json.load(f).items()}
        else:
            self.users = {}
            self._save_users()
    ...
    def _save_users(self):
        ...
        with open(self.users_file, 'w') as f:
            json.dump({username: user.__dict__ for username, user in self.users.items()}, f, indent=2)
    ...
    def check_permission(self, username: str, permission: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to check permission {permission} for user {username}: {str(e)}")
            return False
