...
class SocialMediaDashboard:
    ...
    def generate_engagement_chart(self):
        ...
        for platform in ['linkedin', 'twitter', 'tiktok']:
            ...
            for post_id, post_data in platform_analytics.items():
                data.append({
                    'platform': platform,
                    'engagement_rate': post_data.get('metrics', {}).get('engagement_rate', 0),
                    'date': post_data.get('timestamp', '')
                })
                
        df = pd.DataFrame(data)
        fig = px.line(df, x='date', y='engagement_rate', color='platform',
    ...
    def generate_reach_chart(self):
        ...
        df = pd.DataFrame(data)
        fig = px.bar(df, x='date', y='reach', color='platform',
...
def get_audio(filename):
    ...
    try:
        return send_file(
            os.path.join('site/marketing/assets/audio', filename),
            mimetype='audio/mpeg'
        )
    except Exception as e:
