...
class XmodOutput(nn.Module):
    def __init__(self, config):
        ...
        if config.adapter_layer_norm:
            self.adapter_layer_norm = nn.LayerNorm(
    ...
    def lang_adapter(self, lang_ids: torch.Tensor, hidden_states: torch.Tensor):
        ...
        lang_wise_outputs = []
        for i, (lang_id, split_hidden_state) in enumerate(
            zip(lang_ids, split_hidden_states)
        ):
...
class XmodEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [XmodLayer(config) for _ in range(config.num_hidden_layers)]
        )
...
class XmodModel(XmodPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        XMOD_INPUTS_DOCSTRING.format("batch_size, sequence_length")
    )
    def forward(
        self,
        ...
        if lang_ids is None:
            if self.config.default_language is None:
                raise ValueError(
                    "Input language unknown. Please call `XmodPreTrainedModel.set_default_language()`"
                )
            adapter_languages = list(
                self.encoder.layer[0].output.adapter_modules.keys()
            )
...
class XmodForMultipleChoice(XmodPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        XMOD_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length")
    )
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        ...
        flat_lang_ids = (
            lang_ids.repeat(input_ids.size(0) * input_ids.size(1))
