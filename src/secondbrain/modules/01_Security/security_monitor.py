...
class SecurityMonitor:
    ...
    def _create_alert(self, alert_type: str, severity: str, description: str,
                     source: str, details: Dict[str, Any]) -> Optional[str]:
    ...
    def record_login_attempt(self, user_id: str, success: bool, ip_address: Optional[str] = None):
        ...
        try:
            ...
            if not success:
                ...
                self.metrics.failed_attempts[user_id] = self.metrics.failed_attempts.get(user_id, 0) + 1
    ...
    def record_access(self, user_id: str, is_unusual: bool = False):
        ...
        try:
            if is_unusual:
                self.metrics.unusual_access[user_id] = self.metrics.unusual_access.get(user_id, 0) + 1
            else:
    ...
    def record_session(self, user_id: str, action: str):
        ...
        try:
            if action == "start":
                self.metrics.concurrent_sessions[user_id] = self.metrics.concurrent_sessions.get(user_id, 0) + 1
            elif action == "end":
                self.metrics.concurrent_sessions[user_id] = max(0, self.metrics.concurrent_sessions.get(user_id, 0) - 1)
