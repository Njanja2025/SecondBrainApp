...
class LocationManager:
    ...
    async def test_server(self, server: str, port: int) -> Tuple[Optional[float], Optional[float]]:
    ...
    async def find_best_server(self, max_ping: float = 300.0, max_packet_loss: float = 10.0) -> Optional[str]:
        """Find the best available server based on performance metrics."""
        candidates = []
        
        async with self._lock:
            for server, location in self.locations.items():
                if (location.ping is not None and 
                    location.packet_loss is not None and
                    location.ping <= max_ping and 
                    location.packet_loss <= max_packet_loss):
...
class ConnectionQualityMonitor:
    ...
    def __init__(self, threshold_ping: float = 300.0, threshold_packet_loss: float = 10.0):
    ...
    async def add_measurement(self, ping: Optional[float], packet_loss: Optional[float]):
    ...
    def connection_quality(self) -> str:
        ...
        if avg_ping <= 100 and avg_packet_loss <= 1:
            ...
            return "good"
        elif avg_ping <= self.threshold_ping and avg_packet_loss <= self.threshold_packet_loss:
            return "fair"
        else:
            return "poor"
    ...
    def should_switch_server(self) -> bool:
        ...
        return (
            len(recent_ping) == 3 and
            len(recent_packet_loss) == 3 and
            all(p > self.threshold_ping for p in recent_ping) or
            all(pl > self.threshold_packet_loss for pl in recent_packet_loss)
        ) 