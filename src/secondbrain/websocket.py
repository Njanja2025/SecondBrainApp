...
class ConnectionManager:
    ...
    async def broadcast(self, message: dict):
        ...
        for connection in self.active_connections:
            try:
                ...
                logger.error(
                    f"Error broadcasting to {self.connection_ids.get(connection)}: {e}"
                )
                disconnected.append(connection)
    ...
    async def send_personal_message(self, message: dict, websocket: WebSocket):
        ...
        try:
            ...
            logger.error(
                f"Error sending personal message to {self.connection_ids.get(websocket)}: {e}"
            )
        ...
        try:
            ...
            while True:
                ...
                await self.send_personal_message(
                    {"type": "audio_processed", "status": "success"}, websocket
                )
...
class NotificationManager(ConnectionManager):
    ...
    async def send_notification(
        self, notification_type: str, message: str, data: dict = None
    ):
