...
class MigrationManager:
    ...
    def __init__(
        ...
        config_dir: str = "config/migrations",
    ):
...
"""
    ...
    def create_config(self, config: MigrationConfig) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to create migration configuration {config.name}: {str(e)}"
            )
            return False
        ...
        try:
            ...
            if not migration_file:
                migration_file = self.script.generate_revision(
                    config.version, config.description or "", config.author or ""
                )
...
"""
    ...
    {', '.join(f"sa.Column('{col['name']}', {col['type']}, {col.get('options', {})})" for col in op["columns"])}
                    ...
                    formatted.append(
                        f"""op.add_column('{op["table"]}', sa.Column('{op["column"]["name"]}', {op["column"]["type"]}, {op["column"].get("options", {})}))"""
                    )
                elif op["type"] == "drop_column":
                    formatted.append(
                        f"op.drop_column('{op['table']}', '{op['column']}')"
                    )
                elif op["type"] == "create_index":
                    formatted.append(
                        f"""op.create_index('{op["name"]}', '{op["table"]}', [{', '.join(f"'{col}'" for col in op["columns"])}], {op.get("options", {})})"""
                    )
                elif op["type"] == "drop_index":
                    formatted.append(
                        f"op.drop_index('{op['name']}', table_name='{op['table']}')"
                    )
...
if __name__ == "__main__":
    ...
    config = MigrationConfig(
        ...
        down_operations=[{"type": "drop_table", "name": "users"}],
    )
