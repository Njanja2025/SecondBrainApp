...
def _fuse_conv_bn_qat(m: GraphModule) -> GraphModule:
    ...
    if torch.cuda.is_available():
        m = _fuse_conv_bn_qat_helper(
            m, F.conv1d, _conv1d_bn_example_inputs, is_cuda=True
        )
        m = _fuse_conv_bn_qat_helper(
            m, F.conv2d, _conv2d_bn_example_inputs, is_cuda=True
        )
    return m
...
def _copy_over_q_dq_args(original_node: Node, replacement_node: Node):
    ...
    if original_node.target in (
        ...
        raise ValueError(
            "Expected quantize/dequantize nodes, got '%s'" % original_node.target
        )
...
def _fold_conv_bn_qat(m: GraphModule) -> GraphModule:
    m = _fold_conv_bn_qat_helper(
        m, F.conv1d, _quantized_conv1d_bn_example_inputs, is_cuda=False
    )
    m = _fold_conv_bn_qat_helper(
        m, F.conv2d, _quantized_conv2d_bn_example_inputs, is_cuda=False
    )
    if torch.cuda.is_available():
        m = _fold_conv_bn_qat_helper(
            m, F.conv1d, _quantized_conv1d_bn_example_inputs, is_cuda=True
        )
        m = _fold_conv_bn_qat_helper(
            m, F.conv2d, _quantized_conv2d_bn_example_inputs, is_cuda=True
        )
    return m
...
def _fold_conv_bn_qat_helper(
    ...
    for (
        ...
        kwargs = _get_quantized_conv_bn_example_inputs_kwargs(
            is_per_channel, has_bias, is_cuda
        )
