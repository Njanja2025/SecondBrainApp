...
class ConversationLoop:
            ...
            while self.is_listening:
                if not self.audio_queue.empty():
                    ...
                    if command:
                        ...
                        recommendation = await self.recommendation_engine.get_recommendation(context)
                        
                        # Generate and enhance response
                        response = await self._generate_response(command, recommendation)
                        enhanced_response = await self.voice_enhancer.enhance_voice_output(
                            response,
                            recommendation.get("delivery", {})
                        )
    ...
    async def _generate_response(self, command: str, recommendation: Dict[str, Any]) -> str:
