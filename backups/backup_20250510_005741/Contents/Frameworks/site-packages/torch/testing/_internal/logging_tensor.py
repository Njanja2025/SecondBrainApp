    ...
    def _fmt(self, a: object, with_type: bool = False) -> str:
        ...
        if isinstance(a, cond_cls):
            ...
            if with_type and self.with_type:
                maybe_type = (
                    f": {_dtype_abbrs[a.dtype]}[{', '.join(map(str, a.shape))}]"
                )
    ...
    def emit(self, record):
        fmt_args = ", ".join(
            itertools.chain(
                ...
                (
                    f"{k}={str(tree_map(self._fmt, v))}"
                    for k, v in record.args[1].items()
                ),
            )
        )
        fmt_rets = tree_map(
            functools.partial(self._fmt, with_type=True), record.args[2]
        )
...
class GatherTraceback(logging.Filter):
    ...
    def filter(self, record):
        record.traceback = gather_traceback(
            python=self.python, script=self.script, cpp=self.cpp
        )
        return True
...
def capture_logs(
    is_mode=False, python_tb=False, script_tb=False, cpp_tb=False
) -> Iterator[List[str]]:
    ...
    if collect_traceback:
        logger.addFilter(
            GatherTraceback(python=python_tb, script=script_tb, cpp=cpp_tb)
        )
    try:
        if collect_traceback:
...
def capture_logs_with_logging_tensor_mode(
    python_tb=False, script_tb=False, cpp_tb=False
):
