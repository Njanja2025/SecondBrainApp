...
class ScenarioSimulator:
    ...
    async def run_morning_checkin_scenario(self):
        ...
        for interaction in interactions:
            ...
            emotion_score = self._validate_emotion_adaptation(
                result["enhanced"],
                interaction["emotion"],
                interaction["expected_tone"]
            )
            
            # Validate personality traits
            trait_score = self._validate_personality_traits(
                result["enhanced"],
                interaction["expected_traits"]
            )
    ...
    async def run_error_recovery_scenario(self):
            ...
            if past_frustrations:
                ...
                if successful_responses:
                    ...
                    for response in successful_responses[:3]:  # Show top 3
                        ...
                        logger.info(f"  Effectiveness: {response['feedback']['effectiveness']:.2f}")
        ...
        for interaction in interactions:
            ...
            combined_score = (emotion_score + trait_score + transition_effectiveness) / 3
            self.enhancer.rate_response(
                interaction["message"],
                combined_score,
                "Samantha"
    ...
    def _calculate_emotional_transition_score(self, from_state: str, to_state: str, 
        ...
        if to_value > from_value:
            # Positive transition
            transition_score = 0.7 + (0.3 * ((to_value - from_value) / len(emotional_progression)))
        else:
    ...
    def _validate_emotion_adaptation(self, enhanced_text: str, 
                                  expected_emotion: EmotionType,
                                  expected_tone: str) -> float:
        """
        ...
        if markers:
            score += sum(1 for marker in markers if marker in enhanced_text.lower()) / len(markers)
        ...
        if expected_tone in tone_markers:
            ...
            tone_score = sum(1 for marker in markers if marker in enhanced_text.lower()) / len(markers)
        
        # Combine scores with weights
        final_score = (score * 0.6) + (tone_score * 0.4)
        self.emotion_accuracy[expected_emotion] = self.emotion_accuracy.get(expected_emotion, []) + [final_score]
        
        return min(1.0, final_score)

    def _validate_personality_traits(self, enhanced_text: str, expected_traits: list) -> float:
        """
        ...
        for trait in expected_traits:
            if trait in trait_markers:
                ...
                trait_score = sum(1 for marker in markers if marker in enhanced_text.lower()) / len(markers)
                total_score += trait_score
                
                # Track trait effectiveness
                self.trait_effectiveness[trait] = self.trait_effectiveness.get(trait, []) + [trait_score]
    ...
    def get_adaptation_report(self) -> Dict[str, Any]:
        ...
        if self.emotion_transitions:
            effectiveness_scores = [t["effectiveness"] for t in self.emotion_transitions]
            report["emotional_transitions"]["average_effectiveness"] = sum(effectiveness_scores) / len(effectiveness_scores)
        
        # Calculate overall performance including emotional transitions
        all_emotion_scores = [score for scores in self.emotion_accuracy.values() for score in scores]
        all_trait_scores = [score for scores in self.trait_effectiveness.values() for score in scores]
        ...
        report["overall_performance"] = {
            "average_emotion_accuracy": sum(all_emotion_scores) / len(all_emotion_scores),
            "average_trait_effectiveness": sum(all_trait_scores) / len(all_trait_scores),
            "average_transition_effectiveness": sum(all_transition_scores) / len(all_transition_scores),
    ...
    def _get_memory_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "learning_effectiveness": len(patterns["successful_responses"]) / max(1, emotional_history["total_interactions"])
        }
