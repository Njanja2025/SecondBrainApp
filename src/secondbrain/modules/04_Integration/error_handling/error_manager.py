...
class ErrorManager:
    ...
    def retry_operation(
        self, operation: Callable, config: RetryConfig, *args, **kwargs
    ) -> Any:
            ...
            try:
                ...
                if attempt < config.max_retries:
                    if config.retry_on_exceptions and not isinstance(
                        e, tuple(config.retry_on_exceptions)
                    ):
                        raise e
                    ...
                    # Log retry attempt
                    logger.warning(
                        f"Operation failed, retrying in {delay:.1f} seconds "
    ...
    def get_error_stats(
        ...
        """
        try:
            stats = {"total_errors": 0, "error_types": {}, "services": {}}
