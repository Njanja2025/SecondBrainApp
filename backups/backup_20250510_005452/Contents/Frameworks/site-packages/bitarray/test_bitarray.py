...
class Util(object):
    ...
    def check_obj(self, a):
        ...
        ptr, nbytes, endian, padbits, alloc, readonly, buf, exports = a.buffer_info()
...
class CreateObjectTests(unittest.TestCase, Util):
    ...
    def test_endian_wrong(self):
        ...
        self.assertRaisesMessage(
            TypeError, "'ellipsis' object is not iterable", bitarray, Ellipsis
        )
...
class InternalTests(unittest.TestCase, Util):
    ...
    def test_copy_n_example(self):
        ...
        y = bitarray("00101110 11111001 01011101 11001011 10110000 01011110 011")
        x = bitarray("01011101 11100101 01110101 01011001 01110100 10001010 01111011")
        x._copy_n(21, y, 6, 31)
        self.assertEqual(
            x,
...
class SliceTests(unittest.TestCase, Util):
    ...
    def test_setslice_random(self):
        for a in self.randombitarrays(start=1):
            ...
            for _ in range(10):
                step = self.rndsliceidx(la) or None
                s = slice(self.rndsliceidx(la), self.rndsliceidx(la), step)
                lb = randrange(10) if step is None else self.calc_slicelength(s, la)
                b = bitarray(lb)
                c = bitarray(a)
                c[s] = b
                self.check_obj(c)
    ...
    def test_setslice_bitarray_random(self):
        for _ in range(100):
            ...
            lst_b = b.tolist()
            s = slice(self.rndsliceidx(n), self.rndsliceidx(n), randint(-3, 3) or None)
            try:
                a[s] = b
            except ValueError:
                a = None
...
class SequenceIndexTests(unittest.TestCase, Util):
    ...
    def test_set_bitarray_basic(self):
        ...
        self.assertRaisesMessage(
            ValueError, msg, a.__setitem__, [1, 2], bitarray("001")
        )
    ...
    def test_type_messages(self):
        for item, msg in [
            ...
            (
                None,
                "bitarray indices must be integers, slices or "
...
class PickleTests(unittest.TestCase, Util):
    ...
    def test_reconstructor_invalid_args(self):
        ...
        self.assertRaises(
            TypeError, _bitarray_reconstructor, bitarray, b"\x0f", 123, 1, 0
        )
        self.assertRaisesMessage(
            ValueError,
        ...
        self.assertRaises(
            TypeError, _bitarray_reconstructor, bitarray, b"\x0f", "big", 0.0, 0
        )
        self.assertRaisesMessage(
            ValueError,
            "invalid number of pad bits: 8",
            _bitarray_reconstructor,
            bitarray,
            b"A",
            "big",
        ...
        # argument 5 - readonly
        self.assertRaises(
            TypeError, _bitarray_reconstructor, bitarray, b"\x0f", "big", 1, "foo"
        )
...
class RichCompareTests(unittest.TestCase, Util):
    ...
    def test_eq_ne(self):
        for _ in range(10):
            self.assertTrue(
                bitarray(0, self.random_endian()) == bitarray(0, self.random_endian())
            )
            self.assertFalse(
                bitarray(0, self.random_endian()) != bitarray(0, self.random_endian())
            )
...
class SequenceTests(unittest.TestCase, Util):
    ...
    def test_contains_explicit(self):
        ...
        for s, r in [
            ("", True),  # every bitarray contains an empty one
            ("1", True),
            ("11", True),
            ("111", False),
            ("011", True),
...
class NumberTests(unittest.TestCase, Util):
    ...
    def test_size_error(self):
        ...
        self.assertRaises(ValueError, lambda: a | b)
        self.assertRaises(ValueError, lambda: a ^ b)
        for x in (a.__and__, a.__or__, a.__xor__, a.__iand__, a.__ior__, a.__ixor__):
...
class PackTests(unittest.TestCase, Util):
    ...
    def test_pack_self(self):
        a = bitarray()
        self.assertRaisesMessage(
            BufferError, "cannot resize bitarray that is exporting buffers", a.pack, a
        )
...
class PopTests(unittest.TestCase, Util):
    ...
    def test_basic(self):
        a = bitarray("01")
        self.assertRaisesMessage(IndexError, "pop index out of range", a.pop, 2)
...
class To01Tests(unittest.TestCase, Util):
    ...
    def test_sep(self):
        for a in self.randombitarrays():
            sep = "".join(chr(randint(32, 126)) for _ in range(randrange(10)))
    ...
    def test_random(self):
        for a in self.randombitarrays():
            ...
            self.assertEqual(nspace, nsep * ((n - 1) // group) if group and n else 0)
...
class ByteReverseTests(unittest.TestCase, Util):
    ...
    def test_explicit_all(self):
        for x, y in [
            ...
            ("11101101", "10110111"),
            ("00000001", "10000000"),
            ("11011111 00100000 00011111", "11111011 00000100 11111000"),
        ]:
            a = bitarray(x)
            a.bytereverse()
            self.assertEqual(a, bitarray(y))
...
class IndexTests(unittest.TestCase, Util):
    ...
    def test_find_empty(self):
        ...
        for x in bytearray([0]), b"\0", "A":
            ...
            for _ in range(50):
                ...
                start = randint(-5, 5)
                self.assertEqual(z.find(empty, start), self.find_empty(n, start))
                self.assertEqual(
                    z.rfind(empty, start), self.find_empty(n, start, right=1)
                )

                stop = randint(-5, 5)
                self.assertEqual(
                    z.find(empty, start, stop), self.find_empty(n, start, stop)
                )
                self.assertEqual(
                    z.rfind(empty, start, stop), self.find_empty(n, start, stop, 1)
                )
    ...
    def test_empty(self):
        ...
        for _ in range(50):
            ...
            z = bitarray(n)
            r = getrandbits(1)
            self.assertEqual(z.find(empty, right=r), self.find_empty(n, right=r))

            start = randint(-5, 5)
            self.assertEqual(
                z.find(empty, start, right=r), self.find_empty(n, start, right=r)
            )

            stop = randint(-5, 5)
            self.assertEqual(
                z.find(empty, start, stop, r), self.find_empty(n, start, stop, r)
            )
...
class SearchTests(unittest.TestCase, Util):
    ...
    def test_search_empty(self):
        ...
        for start, stop, right, res in [
            ...
            self.assertEqual(list(a.search(empty, start, stop, right)), res)
    ...
    def test_search_random(self):
        for _ in range(500):
            ...
            # list of positions
            if b:
                plst = [i + k for k in range(len(aa)) if aa[k : k + len(b)] == b]
...
class BytesTests(unittest.TestCase, Util):
    ...
    def test_frombytes_simple(self):
        ...
        a.frombytes(b"BC")
        self.assertEQUAL(a, bitarray("110 01000001 01000010 01000011", endian="big"))
...
class FileTests(unittest.TestCase, Util):
    ...
    def test_mmap_readonly(self):
        ...
        with open(self.tmpfname, "rb") as fi:  # readonly
            ...
            self.assertTrue(info["imported"])
            self.assertRaisesMessage(
                TypeError, "cannot modify read-only memory", a.__setitem__, 0, 1
            )
...
class DecodeTreeTests(unittest.TestCase, Util):
    ...
    def test_complete(self):
        ...
        dt = decodetree({"a": bitarray("0"), "b": bitarray("1")})
        self.assertTrue(dt.complete())

        dt = decodetree({"a": bitarray("0"), "b": bitarray("11")})
        self.assertFalse(dt.complete())

        dt = decodetree({"a": bitarray("0"), "b": bitarray("11"), "c": bitarray("10")})
        self.assertTrue(dt.complete())
    ...
    @skipIf(is_pypy)
    def test_large(self):
        d = {i: bitarray(bool((1 << j) & i) for j in range(10)) for i in range(1024)}
        t = decodetree(d)
...
class PrefixCodeTests(unittest.TestCase, Util):
    ...
    def test_decode_incomplete_2(self):
        ...
        self.assertRaisesMessage(ValueError, msg, list, a.decode(alphabet_code))
    ...
    def test_decode_no_term(self):
        ...
        self.assertRaisesMessage(
            ValueError, "incomplete prefix code at position 1", next, it
        )
...
class BufferImportTests(unittest.TestCase, Util):
    ...
    def test_bytearray(self):
        ...
        for n in 7, 9:
            self.assertRaises(BufferError, a.__setitem__, slice(8, 16), bitarray(n))
        b[1] = b[2] = 255
        self.assertEqual(b, bytearray(100 * [255]))
    ...
    def test_bitarray(self):
        ...
        self.assertRaisesMessage(
            BufferError, "cannot resize bitarray that is exporting buffers", a.pop
        )
        self.assertRaisesMessage(BufferError, "cannot resize imported buffer", b.pop)
        self.check_obj(a)
        self.check_obj(b)
    ...
    def test_bitarray_shared_sections(self):
        ...
        self.assertEqual(buffer_info(b, "address"), buffer_info(a, "address") + 0x100)
        c = bitarray(buffer=memoryview(a)[0x200:0x800])
        self.assertEqual(buffer_info(c, "address"), buffer_info(a, "address") + 0x200)
