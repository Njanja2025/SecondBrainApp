    ...
    * a -l option to (only) list tests
    ...
    def _getParentArgParser(self):
        ...
        parser.add_argument(
            ...
            dest="tags",
            type=str,
            help="Comma-separated list of tags to select " "a subset of the test suite",
        )
        parser.add_argument(
            "--exclude-tags",
            dest="exclude_tags",
        ...
        parser.add_argument(
            ...
            nargs="?",
            help=(
                "Run tests from changes made against "
    ...
    def parseArgs(self, argv):
        ...
        if self.random_select:
            self.test = _choose_random_tests(
                self.test, self.random_select, self.random_seed
            )
    ...
    def runTests(self):
        if self.refleak:
            ...
            self.testRunner = BasicTestRunner(
                self.useslice,
        ...
        if self.multiprocess and not self.nomultiproc:
            if self.multiprocess < 1:
                msg = (
                    "Value specified for the number of processes to use in "
