...
class EmailMetricsManager:
    ...
    def create_config(self, config: MonitorConfig) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to create email configuration {config.name}: {str(e)}")
            return False
    ...
    def collect_metrics(self, config_name: str, imap_server: str, email_address: str, 
                       password: str) -> Dict[str, float]:
                ...
                if metric.type == MetricType.VOLUME:
                    ...
                    for msg_id in messages[0].split()[-10:]:  # Last 10 messages
                        ...
                        if email_message["In-Reply-To"]:
                            ...
                            _, reply_data = mail.search(None, f'HEADER "Message-ID" "{reply_id}"')
                            if reply_data[0]:
                                reply_time = email.utils.parsedate_to_datetime(
                                    email.message_from_bytes(
                                        mail.fetch(reply_data[0].split()[0], "(RFC822)")[1][0][1]
                                    )["Date"]
                                )
                    ...
                    metrics[metric.name] = np.mean(response_times) if response_times else 0
                    ...
                    for msg_id in messages[0].split()[-50:]:  # Last 50 messages
                        ...
                        if re.search(r"newsletter|update|digest", subject.lower()):
                            categories["newsletter"] = categories.get("newsletter", 0) + 1
                        elif re.search(r"alert|warning|error", subject.lower()):
                            categories["alert"] = categories.get("alert", 0) + 1
                        else:
                            categories["other"] = categories.get("other", 0) + 1
                    
                    metrics[metric.name] = max(categories.items(), key=lambda x: x[1])[0]
    ...
    def analyze_metrics(self, metrics_data: List[Dict[str, float]], 
                       config: MonitorConfig) -> Dict[str, Any]:
    ...
    def start_monitoring(self, config_name: str, imap_server: str, 
    ...
    def _monitoring_loop(self, config_name: str, imap_server: str, 
                        email_address: str, password: str):
