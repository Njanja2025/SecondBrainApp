...
class SplinterLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = SplinterAttention(
                config, position_embedding_type="absolute"
            )
...
class QuestionAwareSpanSelectionHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.query_start_transform = SplinterFullyConnectedLayer(
            config.hidden_size, config.hidden_size
        )
        self.query_end_transform = SplinterFullyConnectedLayer(
            config.hidden_size, config.hidden_size
        )
        self.start_transform = SplinterFullyConnectedLayer(
            config.hidden_size, config.hidden_size
        )
        self.end_transform = SplinterFullyConnectedLayer(
            config.hidden_size, config.hidden_size
        )
    ...
    def forward(self, inputs, positions):
        _, _, dim = inputs.size()
        index = positions.unsqueeze(-1).repeat(
            1, 1, dim
        )  # [batch_size, num_positions, dim]
        gathered_reps = torch.gather(
        ...
        query_end_reps = self.query_end_transform(
            gathered_reps
        )  # [batch_size, num_positions, dim]
        ...
        hidden_states = self.start_classifier(
            query_start_reps
        )  # [batch_size, num_positions, dim]
...
class SplinterForQuestionAnswering(SplinterPreTrainedModel):
    ...
    def forward(
        ...
        start_logits, end_logits = self.splinter_qass(
            sequence_output, question_positions
        )
        ...
        if attention_mask is not None:
            start_logits = (
                start_logits
                + (1 - attention_mask) * torch.finfo(start_logits.dtype).min
            )
            end_logits = (
                end_logits + (1 - attention_mask) * torch.finfo(end_logits.dtype).min
            )
...
class SplinterForPreTraining(SplinterPreTrainedModel):
    ...
    def forward(
        ...
        if (
            question_positions is None
            and start_positions is not None
            and end_positions is not None
        ):
            raise TypeError(
                "question_positions must be specified in order to calculate the loss"
            )

        elif question_positions is None and input_ids is None:
            raise TypeError(
                "question_positions must be specified when input_embeds is used"
            )
