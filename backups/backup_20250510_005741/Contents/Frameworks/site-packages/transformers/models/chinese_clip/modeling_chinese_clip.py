...
class ChineseCLIPTextAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = CHINESE_CLIP_TEXT_SELF_ATTENTION_CLASSES[
...
class ChineseCLIPTextLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = ChineseCLIPTextAttention(
                config, position_embedding_type="absolute"
            )
...
class ChineseCLIPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, ChineseCLIPVisionEmbeddings):
            ...
            nn.init.normal_(
                module.token_type_embeddings.weight,
                mean=0.0,
                std=self.config.initializer_range,
            )
            for embedding in [
...
class ChineseCLIPTextEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [ChineseCLIPTextLayer(config) for _ in range(config.num_hidden_layers)]
        )
...
class ChineseCLIPVisionEncoder(nn.Module):
    ...
    def __init__(self, config: ChineseCLIPConfig):
        ...
        self.layers = nn.ModuleList(
            [ChineseCLIPVisionLayer(config) for _ in range(config.num_hidden_layers)]
        )
        self.gradient_checkpointing = False
...
class ChineseCLIPVisionTransformer(nn.Module):
    ...
    @replace_return_docstrings(
        output_type=BaseModelOutputWithPooling, config_class=ChineseCLIPVisionConfig
    )
    def forward(
        self,
...
class ChineseCLIPModel(ChineseCLIPPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=ChineseCLIPOutput, config_class=ChineseCLIPConfig
    )
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
