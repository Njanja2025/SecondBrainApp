...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    def __init__(
        ...
        self.assistant_model.generation_config.eos_token_id = (
        ...
        if (
            "logits_to_keep" in assistant_kwargs.keys()
            and not assistant_model._supports_logits_to_keep()
        ):
        ...
        if assistant_model.config.is_encoder_decoder:
            inputs_tensor, model_input_name, assistant_kwargs = (
            ...
            assistant_kwargs = (
                assistant_model._prepare_encoder_decoder_kwargs_for_generation(
                    ...
                    assistant_kwargs,
                    model_input_name,
                    assistant_model.generation_config,
                )
            )
    ...
    def get_candidates(
        ...
        generation_args = self._prepare_generation_args(
            input_ids, min_new_tokens, max_new_tokens
        )
    ...
    def _prepare_generation_args(
        self, input_ids: torch.LongTensor, min_new_tokens: int, max_new_tokens: int
    ) -> Dict:
    ...
    def _generate_candidates(
        ...
        assistant_output = self.assistant_model.generate(
            **generation_args, **self.assistant_kwargs
        )
...
class AssistedCandidateGeneratorDifferentTokenizers(AssistedCandidateGenerator):
    ...
    def _get_longest_diag_dict(input_matrix, nonzero_idx):
        ...
        for idx in nonzero_idx:
            ...
            start_idx += 1
            while (
                start_idx[0] < input_matrix.shape[0]
                and start_idx[1] < input_matrix.shape[1]
            ):
                tuple_start_idx = tuple(start_idx.tolist())
                visited.add(tuple_start_idx)
    ...
    def _get_tokens_diag(prompt, prompt_plus_new_tokens):
        ...
        longest_location, longest_diag_length = (
        ...
        new_tokens_only = prompt_plus_new_tokens[
            :, new_token_start_index + discrepancy_length :
        ]
    ...
    def convert_source_tokens_to_target_tokens(
        ...
        """
        text = source_tokenizer.batch_decode(
            input_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True
        )
        dest_ids = destination_tokenizer(
    ...
    def get_candidates(
        ...
        min_new_tokens = max(
            min(
                max_new_tokens,
                self.main_model_min_length - assistant_input_ids.shape[-1],
            ),
            0,
        )
        ...
        generation_args = self._prepare_generation_args(
            assistant_input_ids, min_new_tokens, max_new_tokens
        )
        ...
        new_target_ids = self._process_assistant_outputs(
            input_ids, assistant_output.sequences, assistant_input_ids
        )
    ...
    def _prepare_assistant_input_ids(
        self, input_ids: torch.LongTensor
        ...
        if (
            ...
            discrepancy_length, new_tokens_only, discrepancy_only = (
                self._get_tokens_diag(prompt_use, new_assistant_ids)
            )
            ...
            if new_tokens_only is not None:
                if discrepancy_length > 0 and discrepancy_only.shape[1] > 0:
                    if discrepancy_length == discrepancy_only.shape[1]:
                        ...
                        discrepancy_length_diff = (
                            discrepancy_length - discrepancy_only.shape[1]
                        )
                        assistant_input_ids = assistant_input_ids[
                            :, :-discrepancy_length_diff
                        ]
                        assistant_input_ids[:, -discrepancy_only.shape[1] :] = (
                            discrepancy_only
                        )
                ...
                if new_tokens_only.shape[1] > 0:
                    assistant_input_ids = torch.cat(
                        [assistant_input_ids, new_tokens_only], dim=-1
                    )
            else:
                # edge case: in case of no intersection between prompt and new_assistant_ids
                assistant_input_ids = torch.cat(
                    [assistant_input_ids, new_assistant_ids], dim=-1
                )
        else:
    ...
    def _process_assistant_outputs(
        self,
        input_ids: torch.LongTensor,
        assistant_sequences: torch.LongTensor,
        ...
        _, target_new_tokens_only, _ = self._get_tokens_diag(
        ...
        if target_new_tokens_only is not None:
            if target_new_tokens_only.shape[1] > 0:
                new_target_ids = torch.cat(
                    [new_target_ids, target_new_tokens_only], dim=-1
                )
        else:
            # edge case: in case of no intersection between prompt and new_target_ids
            new_target_ids = torch.cat(
                [new_target_ids, new_target_ids_from_window], dim=-1
            )
...
class AssistantToTargetTranslator:
    ...
    def __init__(
        ...
        if len(self._suppress_input_ids) > 0:
            ...
            self.logits_processors = LogitsProcessorList(
                [
                    SuppressTokensLogitsProcessor(
                        self._get_suppress_input_ids(), self._assistant_model_device
                    )
                ]
            )
    ...
    def _get_assistant_to_target_input_ids(self):
        ...
        space_str = " "
        target_space_ids = self._target_tokenizer(space_str, add_special_tokens=False)[
            "input_ids"
        ]
        if len(target_space_ids) > 0:
            target_space_sign = self._target_tokenizer.convert_ids_to_tokens(
                target_space_ids
            )[0][0]

            assistant_space_ids = self._assistant_tokenizer(
                space_str, add_special_tokens=False
            )["input_ids"]
            if len(assistant_space_ids) > 0:
                assistant_space_sign = self._assistant_tokenizer.convert_ids_to_tokens(
                    assistant_space_ids
                )[0][0]
        ...
        assistant_to_target_input_ids = torch.full(
            (max_assistant_index + 1,), self.SUPPRESS_TOKEN_ID, dtype=int
        )
        ...
        return (
            assistant_to_target_input_ids.to(self._assistant_model_device),
    ...
    def _get_suppress_input_ids(self) -> list[int]:
        ...
        """
        return torch.where(
            self._assistant_to_target_input_ids == self.SUPPRESS_TOKEN_ID
        )[0]
    ...
    def get_target_ids(
        ...
        if num_new_tokens == 0:
            ...
            transformed_slice = self._assistant_to_target_input_ids[
                assistant_candidate_ids[0, -num_new_tokens:]
            ]
    ...
    def get_target_logits(
        ...
        assistant_indices_mask = (
            self._assistant_to_target_input_ids != self.SUPPRESS_TOKEN_ID
        )
        # Exclude invalid indices
        target_logits_supported_indices = self._assistant_to_target_input_ids[
            assistant_indices_mask
        ]
        valid_assistant_logits = assistant_logits[
            ..., : self._assistant_to_target_input_ids.shape[0]
        ]

        target_logits[..., target_logits_supported_indices] = valid_assistant_logits[
            ..., assistant_indices_mask
        ]
...
class UniversalSpeculativeDecodingGenerator(
    ...
    def get_candidates(
        ...
        assistant_input_ids, num_added_tokens = self._prepare_assistant_input_ids(
            target_input_ids
        )
        ...
        self._update_past_and_masks(
            assistant_input_ids, num_added_tokens=num_added_tokens
        )
        generation_args = self._prepare_generation_args(
            assistant_input_ids, min_new_tokens, max_new_tokens
        )
        ...
        self._prev_assistant_ids, assistant_candidate_logits = (
            self._generate_candidates(generation_args)
        )
        ...
        target_candidate_logits = self._atm_translator.get_target_logits(
            assistant_candidate_logits
        )

        return target_candidate_ids, target_candidate_logits

    def _update_past_and_masks(
        self, assistant_input_ids: torch.LongTensor, num_added_tokens: int = 1
    ) -> bool:
    ...
    def _prepare_assistant_input_ids(
        self, target_input_ids: torch.LongTensor
    ) -> torch.LongTensor:
        """
        ...
        if self._target_seq_len_with_candidates > 0:
            ...
            assistant_new_ids = self._atm_translator.target_to_assistant_input_ids.get(
                target_new_ids[0].item()
            )
        if assistant_new_ids is None:
            ...
            assistant_new_ids = torch.tensor(
                [[assistant_new_ids]], device=self.assistant_model.device
            )
        ...
        if self._prev_assistant_ids is None:
            ...
            if tokens_to_remove > 0:
                self._prev_assistant_ids = self._prev_assistant_ids[
                    :, :-tokens_to_remove
                ]
            assistant_input_ids = torch.cat(
                [self._prev_assistant_ids, assistant_new_ids], dim=-1
            )
...
class EarlyExitCandidateGenerator(AssistedCandidateGenerator):
    ...
    def get_candidates(
        ...
        base_model = getattr(
            self.assistant_model, self.assistant_model.base_model_prefix
        )
...
def _prepare_attention_mask(
    ...
    if "cross_attention_mask" in model_kwargs:
        ...
        if mask_length_diff < 0:
            ...
            model_kwargs["image_attention_mask"] = torch.cat(
                [cross_mask, new_mask], dim=1
            )
