...
needs_svml = unittest.skipUnless(
    config.USING_SVML, "SVML tests need SVML to be present"
)
...
numpy_funcs = [
    f for f, v in svml_funcs.items() if "<ufunc" in [str(p).split(" ")[0] for p in v]
]
other_funcs = [
    f for f, v in svml_funcs.items() if "<built-in" in [str(p).split(" ")[0] for p in v]
]
...
class TestSVMLGeneration(TestCase):
    ...
    def _inject_test(cls, dtype, mode, vlen, flags):
        ...
        @staticmethod
        def run_template():
            fn, contains, avoids = combo_svml_usecase(
                dtype, mode, vlen, flags["fastmath"], flags["name"]
            )
            # look for specific patterns in the asm for a given target
            with (
                override_env_config("NUMBA_CPU_NAME", vlen2cpu[vlen]),
    ...
    def autogenerate(cls):
        flag_list = [
            {"fastmath": False, "error_model": "numpy", "name": "usecase"},
            {"fastmath": True, "error_model": "numpy", "name": "fastmath_usecase"},
        ]
...
class TestSVML(TestCase):
    ...
    @TestCase.run_test_in_subprocess(
        envvars={"NUMBA_DISABLE_INTEL_SVML": "1", **_skylake_axv512_envvars}
    )
