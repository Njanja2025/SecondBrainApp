...
class NotebookProgressBar:
    ...
    def __init__(
        ...
        if "VSCODE_PID" in os.environ:
            self.update_every = (
                0.5  # Adjusted for smooth updated as html rending is slow on VS Code
            )
            # This is the only adjustment required to optimize training html rending

    def update(
        self, value: int, force_update: bool = False, comment: Optional[str] = None
    ):
        """
        ...
        if self.last_value is None:
            ...
            if (self.average_time_per_item is None) or (
                self.average_time_per_item == 0
            ):
                self.wait_for = 1
            else:
                self.wait_for = max(
...
class NotebookProgressCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        self.first_column = (
            "Epoch" if args.eval_strategy == IntervalStrategy.EPOCH else "Step"
        )
