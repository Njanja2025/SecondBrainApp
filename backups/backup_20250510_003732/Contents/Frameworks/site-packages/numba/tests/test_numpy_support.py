...
class TestFromDtype(TestCase):
    ...
    def test_number_types(self):
        ...
        for letter in "hHiIlLqQfdFD":
            self.assertRaises(
                NumbaNotImplementedError, f, np.dtype(foreign_align + letter)
            )
...
class TestUFuncs(TestCase):
    ...
    def test_ufunc_find_matching_loop(self):
        ...
        def check(ufunc, input_types, sigs, output_types=()):
            ...
            *sigs* for *ufunc*, *input_types* and optional *output_types*.
            ...
            self.assertIn(
                ...
                "one of %s, got %s" % (input_types, output_types, sigs, loop.ufunc_sig),
            )
        ...
        check_exact(
            np_add,
            (types.NPDatetime("s"), types.NPTimedelta("")),
            "Mm->M",
            output_types=(types.NPDatetime("s"),),
        )
        check_exact(
            np_add,
            (types.NPDatetime("ns"), types.NPTimedelta("")),
            "Mm->M",
            output_types=(types.NPDatetime("ns"),),
        )
        check_exact(
            np_add,
            (types.NPTimedelta(""), types.NPDatetime("s")),
            "mM->M",
            output_types=(types.NPDatetime("s"),),
        )
        check_exact(
            np_add,
            (types.NPTimedelta(""), types.NPDatetime("ns")),
            "mM->M",
            output_types=(types.NPDatetime("ns"),),
        )
        check_exact(
