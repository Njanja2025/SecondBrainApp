...
from scipy.stats._axis_nan_policy import (
    _masked_arrays_2_sentinel_arrays,
    SmallSampleWarning,
    too_small_nd_omit,
...
def combine_pvalues_weighted(*args, **kwargs):
    return stats.combine_pvalues(
        args[0], *args[2:], weights=args[1], method="stouffer", **kwargs
    )
...
axis_nan_policy_cases = [
    ...
    (
        stats.fligner,
        tuple(),
        {"center": "trimmed", "proportiontocut": 0.01},
        2,
        2,
        False,
        None,
...
@pytest.mark.parametrize(
    ("hypotest", "args", "kwds", "n_samples", "n_outputs", "paired", "unpacker"),
    axis_nan_policy_cases,
)
...
def _axis_nan_policy_test(
    ...
    if (
        nan_policy == "omit"
        and data_generator in {"all_nans", "mixed"}
        and hypotest not in too_small_special_case_funcs
    ):
...
@pytest.mark.parametrize(("nan_policy"), ("propagate", "omit", "raise"))
@pytest.mark.parametrize(
    ("data_generator"), ("all_nans", "all_finite", "mixed", "empty")
)
    ...
    for samples in small_data_generator(n_samples, n_dims):
        ...
        for axis in range(-max_axis, max_axis):
            ...
            try:
                ...
                concat = stats._stats_py._broadcast_concatenate(
                    samples, axis, paired=paired
                )
                with np.testing.suppress_warnings() as sup:
                    sup.filter(RuntimeWarning, "Mean of empty slice.")
                    sup.filter(RuntimeWarning, "invalid value encountered")
                ...
                assert not _check_arrays_broadcastable(
                    samples, None if paired else axis
                )
...
@pytest.mark.parametrize(
    ("hypotest", "args", "kwds", "n_samples", "n_outputs", "paired", "unpacker", "rng"),
    paired_non_broadcastable_cases(),
)
def test_non_broadcastable(
    hypotest, args, kwds, n_samples, n_outputs, paired, unpacker, rng, axis
):
    ...
    while get_samples:
        samples = [
            rng.random(size=rng.integers(2, 100, size=2)) for i in range(n_samples)
        ]
