...
class TestManager:
    ...
    def run_tests(self, config_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results["summary"]["failed"] = len(test_results.failures) + len(
                test_results.errors
            )
            ...
            for test in test_results.testsRun:
                if test not in [f[0] for f in test_results.failures] and test not in [
                    e[0] for e in test_results.errors
                ]:
                    results["test_cases"].append(
                        {"name": test._testMethodName, "status": "passed"}
                    )
            ...
            logger.error(
                f"Failed to run tests for configuration {config_name}: {str(e)}"
            )
    ...
    def add_test_case(self, config_name: str, test_case: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to add test case to configuration {config_name}: {str(e)}"
            )
            return False
    ...
    def remove_test_case(self, config_name: str, test_case_name: str) -> bool:
        ...
        try:
            ...
            config.test_cases = [
                tc for tc in config.test_cases if tc.get("name") != test_case_name
            ]
            self._save_configs()
            ...
            logger.error(
                f"Failed to remove test case from configuration {config_name}: {str(e)}"
            )
            return False
...
if __name__ == "__main__":
    ...
    config = TestConfig(
        name="basic_tests",
        test_cases=[
            {"name": "test_addition", "test": lambda: assertEqual(1 + 1, 2)},
            {"name": "test_subtraction", "test": lambda: assertEqual(2 - 1, 1)},
        ],
    )
