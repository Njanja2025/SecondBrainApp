...
def _check_cuda_version(compiler_name: str, compiler_version: TorchVersion) -> None:
    ...
    if cuda_ver != torch_cuda_version:
        ...
        if cuda_ver.major != torch_cuda_version.major:
            raise RuntimeError(
                CUDA_MISMATCH_MESSAGE.format(cuda_str_version, torch.version.cuda)
            )
    ...
    if cuda_str_version not in cuda_compiler_bounds:
        warnings.warn(
            f"There are no {compiler_name} version bounds defined for CUDA version {cuda_str_version}"
        )
    else:
...
def _write_ninja_file_and_build_library(
    ...
    extra_ldflags = _prepare_ldflags(
        extra_ldflags or [], with_cuda, verbose, is_standalone
    )
    build_file_path = os.path.join(build_directory, "build.ninja")
    if verbose:
        print(f"Emitting ninja build file {build_file_path}...", file=sys.stderr)
...
def _prepare_ldflags(extra_ldflags, with_cuda, verbose, is_standalone):
    ...
    if with_cuda:
        ...
        if IS_WINDOWS:
            ...
            extra_ldflags.append(
                "-lamdhip64" if ROCM_VERSION >= (3, 5) else "-lhip_hcc"
            )
