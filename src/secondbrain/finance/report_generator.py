...
class IncomeSummaryPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0, 10, f"Date: {datetime.now().strftime('%Y-%m-%d')}", ln=True, align="C"
        )
        self.ln(10)
...
class IncomeReport(FPDF):
    ...
    def _validate_directories(self):
        ...
        try:
            self.chart_dir.mkdir(parents=True, exist_ok=True)
            (Path.home() / ".secondbrain" / "reports").mkdir(
    ...
    def _validate_income_data(
        self, data: Union[Dict[str, float], List[Dict[str, str]]]
    ) -> List[IncomeData]:
        """
                ...
                for entry in data:
                    if not all(k in entry for k in ["Source", "Revenue"]):
                        raise DataValidationError(
                            "Missing required fields in income data"
                        )
                    ...
                    if not re.match(r"^\d+(\.\d{1,2})?$", revenue_str):
                        raise DataValidationError(
                            f"Invalid revenue format: {revenue_str}"
                        )
    ...
    def _create_chart(self, data: List[IncomeData], chart_type: str) -> Path:
        ...
        try:
            ...
            # Save chart
            chart_path = (
                self.chart_dir / f"chart_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            )
    ...
    def add_daily_summary(
        self, data: List[Dict[str, str]], title: str = "Daily Income Summary"
    ):
        """
    ...
    def _add_summary_statistics(self, data: List[IncomeData]):
        ...
        self.cell(
            0,
            10,
            f"Average Revenue per Source: {self._format_currency(avg_revenue)}",
            ln=1,
        )
        self.cell(
            0, 10, f"Highest Revenue Source: {self._format_currency(max_revenue)}", ln=1
        )
        self.cell(
            0, 10, f"Lowest Revenue Source: {self._format_currency(min_revenue)}", ln=1
        )
...
def generate_income_report(
    ...
    try:
        ...
        with open(
            data_dir / f"income_data_{datetime.now().strftime('%Y%m%d')}.json", "w"
        ) as f:
            json.dump(
                {
