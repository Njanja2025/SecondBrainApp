...
class IntegrationMonitor:
    ...
    def _save_history(self):
        ...
        try:
            ...
            with open(sync_log_path, 'w') as f:
                json.dump([asdict(status) for status in self.sync_statuses], f, indent=2)
    ...
    def log_sync_status(self, service: str, operation: str, status: str,
                       items_processed: int, duration: float, error: Optional[str] = None):
        ...
        if error:
            ...
            logger.info(f"Sync completed: {service} {operation} - {items_processed} items")
    ...
    def get_sync_stats(self, service: Optional[str] = None,
                      time_window: Optional[timedelta] = None) -> Dict[str, Any]:
    ...
    def cleanup_old_logs(self, max_age_days: int = 30):
        ...
        # Clean up API calls
        self.api_calls = [call for call in self.api_calls 
                         if datetime.fromisoformat(call.timestamp) > cutoff]
        
        # Clean up sync statuses
        self.sync_statuses = [sync for sync in self.sync_statuses 
                             if datetime.fromisoformat(sync.timestamp) > cutoff]
