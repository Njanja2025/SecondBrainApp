...
class DNSManager:
    ...
    def __init__(
        self,
        config_path: str = "config/dns_config.json",
        cache_dir: str = "cache/dns"
    ):
    ...
    def _check_dns_cache(
        ...
        if (
            ...
            and (datetime.now().timestamp() - self.last_check[cache_key]) < self.check_interval
        ):
            return self.dns_cache[cache_key] == ip
            
        return False
        
    def _update_dns_cache(
        self,
        domain: str,
        subdomain: str,
        ip: str
