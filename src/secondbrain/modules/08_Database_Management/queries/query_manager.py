...
class QueryManager:
    ...
    def __init__(self, db_url: str = "sqlite:///secondbrain.db", config_dir: str = "config/queries"):
        ...
        try:
            ...
            if config_file.exists():
                with open(config_file, 'r') as f:
                    self.configs = {name: QueryConfig(**config)
    ...
    def create_config(self, config: QueryConfig) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to create query configuration {config.name}: {str(e)}")
            return False
    ...
    def execute_query(self, config_name: str, params: Dict[str, Any] = None) -> Union[List[Dict[str, Any]], int, bool]:
        """Execute a query.
    ...
    def _build_query(self, config: QueryConfig, params: Dict[str, Any] = None) -> expression:
            ...
            if config.query_type == "select":
                ...
                if config.joins:
                    for join in config.joins:
                        join_table = Table(join["table"], self.metadata, autoload_with=self.engine)
                ...
                if config.conditions:
                    ...
                    for condition in config.conditions:
                        ...
                        value = params.get(condition.get("param", field)) if params else condition["value"]
    ...
    def clear_cache(self, config_name: str = None):
        ...
        try:
            ...
            logger.info(f"Cleared cache for {config_name if config_name else 'all configurations'}")
