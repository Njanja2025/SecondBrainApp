...
class VoiceProcessor:
                ...
                with tempfile.NamedTemporaryFile(delete=True, suffix=".wav") as f:
                    ...
                    if text:
                        ...
                        await self.process_command(
                            text, None
                        )  # Replace None with agent reference
    ...
    async def process_command(self, command_text: str, agent: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                and (current_time - self.last_command_time).total_seconds()
                < self.command_cooldown
            ):
                return {
                    "status": "error",
                    "error": "Please wait before issuing another command",
                }
                ...
                for pattern in handler.patterns:
                    ...
                    if match:
                        ...
                        return await handler.handler(
                            command_text, agent, match.groups()
                        )
    ...
    async def respond_with_voice(self, response_text: str):
        ...
        try:
            self.response_history.append(
                {"timestamp": datetime.now(), "response": response_text}
            )
    ...
    async def _handle_empty_command(self) -> Dict[str, Any]:
        ...
        await self.respond_with_voice(
            "Please provide a command. Say 'help' for available commands."
        )
    ...
    async def _handle_help(
        self, command_text: str, agent: Any, groups: tuple
    ) -> Dict[str, Any]:
        """Handle help command."""
    ...
    async def _handle_contract_creation(
        ...
        try:
            ...
            if not re.match(r"^[a-zA-Z]\w*$", contract_name):
                raise ValueError(
                    "Invalid contract name. Use only letters, numbers, and underscores."
                )
    ...
    async def _handle_contract_deployment(
        self, command_text: str, agent: Any
    ) -> Dict[str, Any]:
    ...
    async def _handle_status_query(self, agent: Any) -> Dict[str, Any]:
        ...
        try:
            status = {
                "blockchain_connected": hasattr(agent, "blockchain")
                and agent.blockchain.network_manager.is_connected(),
                "memory_size": len(agent.memory.log) if hasattr(agent, "memory") else 0,
                "command_history": len(self.command_history),
            }

            status_msg = (
                f"Status: Connected to blockchain: {status['blockchain_connected']}, "
            )
    ...
    def get_command_history(self, limit: Optional[int] = None) -> List[Dict]:
        ...
        history = sorted(
            self.command_history, key=lambda x: x["timestamp"], reverse=True
        )
    ...
    def get_response_history(self, limit: Optional[int] = None) -> List[Dict]:
        ...
        history = sorted(
            self.response_history, key=lambda x: x["timestamp"], reverse=True
        )
        return history[:limit] if limit else history
    ...
    def clear_history(self, older_than: Optional[datetime] = None):
        ...
        if older_than:
            self.command_history = [
                cmd for cmd in self.command_history if cmd["timestamp"] > older_than
            ]
            self.response_history = [
                resp for resp in self.response_history if resp["timestamp"] > older_than
            ]
        else:
            self.command_history = []
            self.response_history = []
    ...
    async def _handle_system_optimization(
        self, command_text: str, agent: Any, groups: tuple
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            process_result = await self.phantom_mcp.improve_system(
                "process_optimization"
            )
            learning_result = await self.phantom_mcp.improve_system(
    ...
    async def _handle_pattern_analysis(
        ...
        try:
            status = self.phantom_mcp.get_system_status()
            evolution = self.phantom_mcp.evolution_history[
                -5:
    ...
    async def _handle_backup_creation(
        ...
        try:
            ...
            if result["status"] == "success":
                await self.respond_with_voice(
                    f"Backup created successfully. ID: {result['backup_id']}"
                )
            else:
                await self.respond_with_voice(
                    f"Failed to create backup: {result['message']}"
                )
            return result
        except Exception as e:
    ...
    async def _handle_backup_restore(
        ...
        try:
            ...
            if result["status"] == "success":
                await self.respond_with_voice(
                    f"System restored from backup {backup_id}"
                )
            else:
                await self.respond_with_voice(
                    f"Failed to restore backup: {result['message']}"
                )
            return result
        except Exception as e:
