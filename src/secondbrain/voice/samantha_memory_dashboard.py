...
class MemoryDashboard:
    ...
    def _setup_memory_tab(self):
        ...
        ttk.Button(filter_frame, text="Apply Filter", command=self._apply_filter).pack(
            side=tk.LEFT
        )
    ...
    def _setup_stats_tab(self):
        ...
        for i, stat in enumerate(stats):
            ttk.Label(stats_frame, text=f"{stat}:").grid(
    ...
    def _setup_emotion_tab(self):
        ...
        # Add emotion history
        self.emotion_history = ttk.Treeview(
            emotion_frame, columns=("Time", "Emotion", "Intensity")
        )
    ...
    def _filter_data(self, data: Any, filter_text: str) -> Any:
        ...
        if isinstance(data, dict):
            return {
                k: self._filter_data(v, filter_text)
                for k, v in data.items()
                if filter_text in str(k).lower() or filter_text in str(v).lower()
            }
        elif isinstance(data, list):
            return [item for item in data if filter_text in str(item).lower()]
        else:
            return data
