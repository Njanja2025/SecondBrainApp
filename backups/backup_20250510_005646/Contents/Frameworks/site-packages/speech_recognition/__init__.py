...
class Recognizer(AudioSource):
    ...
    def snowboy_wait_for_hot_word(
        ...
        detector = snowboydetect.SnowboyDetect(
            resource_filename=os.path.join(
                snowboy_location, "resources", "common.res"
            ).encode(),
    ...
    def listen(
        ...
        """
        result = self._listen(
            source, timeout, phrase_time_limit, snowboy_configuration, stream
        )
        if not stream:
            for a in result:
                return a
    ...
    def recognize_azure(
        ...
        access_token, expire_time = getattr(
            self, "azure_cached_access_token", None
        ...
        try:
            ...
            allow_caching = (
                False  # don't allow caching, since monotonic time isn't available
            )
        if (
            expire_time is None or monotonic() > expire_time
        ...
        if show_all:
            return result
        if (
            "RecognitionStatus" not in result
            or result["RecognitionStatus"] != "Success"
            or "NBest" not in result
        ):
            raise UnknownValueError()
    ...
    def recognize_bing(self, audio_data, key, language="en-US", show_all=False):
        ...
        access_token, expire_time = getattr(
            self, "bing_cached_access_token", None
        ...
        if (
            ...
            if allow_caching:
                ...
                self.bing_cached_access_token_expiry = (
                    start_time + 600
    ...
    def recognize_houndify(self, audio_data, client_id, client_key, show_all=False):
        ...
        wav_data = audio_data.get_wav_data(
            convert_rate=(
                None if audio_data.sample_rate in [8000, 16000] else 16000
        ...
        request = Request(
            ...
            headers={
                ...
                "Hound-Client-Authentication": "{};{};{}".format(
                    client_id, request_time, request_signature
                ),
            },
        )
        try:
        ...
        if "Disambiguation" not in result or result["Disambiguation"] is None:
            raise UnknownValueError()
        return (
            result["Disambiguation"]["ChoiceData"][0]["Transcription"],
            result["Disambiguation"]["ChoiceData"][0]["ConfidenceScore"],
        )

    def recognize_amazon(
        self,
        audio_data,
        bucket_name=None,
        access_key_id=None,
        ...
        if check_existing:
            ...
            try:
                ...
                if exc.response["Error"][
                    "Code"
                ] == "BadRequestException" and "The requested job couldn't be found" in str(
                    exc
                ):
            ...
            if (
                job["TranscriptionJobStatus"] in ["COMPLETED"]
                and "TranscriptFileUri" in job["Transcript"]
            ):
    ...
    def recognize_ibm(self, audio_data, key, language="en-US", show_all=False):
        ...
        flac_data = audio_data.get_flac_data(
            convert_rate=(
                None if audio_data.sample_rate >= 16000 else 16000
            ),  # audio samples should be at least 16 kHz
            convert_width=(
                None if audio_data.sample_width >= 2 else 2
