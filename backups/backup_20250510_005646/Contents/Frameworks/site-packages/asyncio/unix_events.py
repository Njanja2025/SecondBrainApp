...
class _UnixSelectorEventLoop(selector_events.BaseSelectorEventLoop):
    ...
    def create_unix_connection(
        self, protocol_factory, path, *, ssl=None, sock=None, server_hostname=None
    ):
    ...
    def __repr__(self):
        ...
        if self._pipe is not None:
            polling = selector_events._test_selector_event(
                self._loop._selector, self._fileno, selectors.EVENT_READ
            )
            if polling:
                info.append("polling")
            else:
...
class _UnixWritePipeTransport(transports._FlowControlMixin, transports.WriteTransport):
    ...
    def __repr__(self):
        ...
        if self._pipe is not None:
            polling = selector_events._test_selector_event(
                self._loop._selector, self._fileno, selectors.EVENT_WRITE
            )
            if polling:
                info.append("polling")
            else:
