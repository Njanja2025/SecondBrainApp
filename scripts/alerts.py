...
class AlertManager:
    ...
    def can_send_alert(self, alert_type: str, resource: str) -> bool:
        ...
        cooldown = (
            ...
            .get(alert_type, {})
            .get("cooldown_minutes", self.config["cooldown_minutes"])
        )

        for alert in reversed(self.history["alerts"]):
            if (
                alert["type"] == alert_type
                and alert["resource"] == resource
                and (now - datetime.fromisoformat(alert["timestamp"]))
        ...
        while retries < self.max_retries:
            try:
                ...
                if retries < self.max_retries:
                    logger.warning(
                        f"Network error while sending email (attempt {retries}): {e}"
                    )
                    time.sleep(self.retry_delay)
                else:
                    logger.error(
                        f"Failed to send email after {self.max_retries} attempts: {e}"
                    )
                    return False
    ...
    def process_alerts(self, system_stats: Dict) -> List[Dict]:
        ...
        if (
            ...
            >= self.config["alert_levels"]["critical"]["threshold"]
        ):
            ...
            system_stats["cpu_percent"]
            >= self.config["alert_levels"]["warning"]["threshold"]
        ):
