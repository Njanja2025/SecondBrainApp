...
def get_triple_parts(triple: str):
    ...
    """
    with (
        ffi.OutputString() as arch,
        ffi.OutputString() as vendor,
        ffi.OutputString() as os,
        ffi.OutputString() as env,
    ):
...
class TargetData(ffi.ObjectRef):
    ...
    def get_pointee_abi_size(self, ty):
        ...
        if opaque_pointers_enabled:
            raise RuntimeError("Cannot get pointee type in opaque pointer " "mode.")
...
ffi.lib.LLVMPY_ABIAlignmentOfType.argtypes = [ffi.LLVMTargetDataRef, ffi.LLVMTypeRef]
