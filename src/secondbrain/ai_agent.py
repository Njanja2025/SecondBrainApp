...
class AIAgent:
    ...
    async def start(self):
        ...
        try:
            ...
            await self.startup_manager.register_component(
                "phantom_mcp", self.phantom_mcp
            )
            await self.startup_manager.register_component("phantom_core", self.phantom)
            await self.startup_manager.register_component(
                "security", self.security, ["phantom_mcp"]
            )
            await self.startup_manager.register_component("wallet", self.wallet)
            await self.startup_manager.register_component(
                "blockchain", self.blockchain, ["wallet"]
            )
            await self.startup_manager.register_component(
                "voice_enhancer", self.voice_enhancer
            )
            await self.startup_manager.register_component(
                "voice_processor", self.voice_processor, ["voice_enhancer"]
            )
            await self.startup_manager.register_component(
                "recommendation_engine", self.recommendation_engine, ["memory"]
            )
            await self.startup_manager.register_component(
                "context_manager", self.context_manager, ["memory"]
            )
                ...
                if audio_data is not None:
                    ...
                    if command:
                        ...
                        self.context_manager.push_context(
                            "voice_command",
                            {"command": command, "audio_metrics": metrics},
                        )

                        # Get recommendation
                        context = self.context_manager.get_current_context()
                        recommendation = (
                            await self.recommendation_engine.get_recommendation(
                                context.data
                            )
                        )

                        # Process command with recommendation
                        await self.process_command_with_recommendation(
    ...
    async def process_command_with_recommendation(
        ...
        try:
            ...
            self.context_manager.update_current_context(
                {"response": response, "recommendation_used": recommendation}
            )
    ...
    async def handle_security_threat(self, scan_result: Dict[str, Any]):
        ...
        try:
            ...
            self.phantom.log_event(
                "Security Threat", f"Detected threat level {threat_level}", "WARNING"
            )
