...
def create_input(
    ...
    def map_arg(arg):
        ...
        if isinstance(arg, (torch.Size, dont_convert)):
            ...
            return conjugate(
                maybe_non_contig(torch.randn(*arg, dtype=dtype, device=device))
        ...
        # double check casting
            ...
            if arg.is_complex() != dtype.is_complex:
                raise RuntimeError(
                    "User provided tensor is real for a test that runs with complex dtype, ",
                    "which is not supported for now",
                )
            # NOTE: We do clone() after detach() here because we need to be able to change size/storage of v afterwards
            v = conjugate(maybe_non_contig(arg)).detach().to(device=device).clone()
            v.requires_grad = requires_grad and (
                v.is_floating_point() or v.is_complex()
            )
            return v
        elif callable(arg):
...
nn_functional_tests = [
    ...
    ("max_unpool1d", torch.tensor([[[2.0, 4]]]), (torch.tensor([[[1, 3]]]), 2, 2, 0)),
    (
        "max_unpool2d",
        torch.tensor([[[[2.0, 4]]]]),
        (torch.tensor([[[[1, 3]]]]), 2, 2, 0),
    ),
    (
        "max_unpool3d",
        torch.tensor([[[[[2.0, 4]]]]]),
    ...
    ("threshold", (S, S, S), (0.1, 2.0), "", (True,)),
    ("threshold", (S, S, S), (0.1, 2.0, True), "inplace"),
    ("relu", (S, S, S), (), "", (True,)),
    ("relu", (S, S, S), (), "inplace"),
    (
        "glu",
    ...
    (
        ...
        "with_weight_and_bias_inference",
        (True, "aten::_batch_norm_impl_index"),
    ),
    (
        "batch_norm",
    ...
    (
        ...
        "with_only_weight_inference",
        (True, "aten::_batch_norm_impl_index"),
    ),
    (
        "instance_norm",
        (S, S, S),
        (non_differentiable(torch.zeros(S)), non_differentiable(torch.ones(S))),
    ),
    (
        "layer_norm",
        (S, S, S, S),
    ...
    (
        "layer_norm",
        (S, S, S, S),
        ([5], non_differentiable(torch.rand(S)), non_differentiable(torch.rand(S))),
    ...
    (
        "nll_loss",
        F.log_softmax(torch.randn(3, 5), dim=0),
        (torch.tensor([1, 0, 4]),),
        "",
    ),
    (
    ...
    (
        "grid_sample",
        (S, S, S, S),
        (non_differentiable(torch.rand(S, S, S, 2)),),
    ),
    (
        "gumbel_softmax",
        (S, S),
        (2.0,),
        "",
        (True, ["aten::softmax", "aten::add", "aten::div"], ["aten::neg"]),
    ),
    (
        "gumbel_softmax",
        (S, S),
    ...
    (
        ...
        (S, S),
        (
            non_differentiable(torch.randint(S, (S,), dtype=torch.int64)),
            1,
            1.0,
            non_differentiable(torch.randn(S)),
        ),
    ...
    (
        ...
        torch.randn(3, 2).sigmoid(),
        (non_differentiable(torch.rand(3, 2)), non_differentiable(torch.randn(3, 2))),
    ),
    (
        "binary_cross_entropy",
        torch.randn(3, 2).sigmoid(),
    ...
    (
        "interpolate",
        torch.randn(S, M, M, M, M),
        (None, 2.0, "trilinear", None, False),
        "trilinear_5d_with_scale_not_recompute_scale_factor",
    ),
    (
        "interpolate",
        torch.randn(S, M, M, M, M),
        (4, None, "trilinear", None, False),
...
class SplitInputs:
    ...
    def __init__(self, args, kwargs):
        ...
        self.kwarg_types = {
            k: "t" if self._is_tensor_input(v) else "s" for k, v in kwargs.items()
        }
        ...
        self.tensor_kwargs = {
            k: v for k, v in kwargs.items() if self._is_tensor_input(v)
        }
        self.nontensor_kwargs = {
            k: v for k, v in kwargs.items() if not self._is_tensor_input(v)
        }
        self.all_tensors = [
            *self.tensor_args,
            *[v for k, v in self.tensor_kwargs.items()],
        ]
...
def partial_apply_nontensors(fn, args, kwargs):
    ...
    def new_fn(*tensors_):
        ...
        full_args = [
            args[i] if s == "s" else next(tensors)
            for i, s in enumerate(inputs.arg_types)
        ]
        full_kwargs = {
            k: kwargs[k] if s == "s" else next(tensors)
            for k, s in inputs.kwarg_types.items()
        }
    ...
    def traced_fn(*inputs, **kwargs):
        ...
        if not cache_traced_fn or not hasattr(traced_fn, "traced"):
            traced = torch.jit.trace(
                fn_tensors, split_inputs.all_tensors, check_trace=False
            )
...
def get_nn_functional_compiled_fn_and_inputs(
    name, self_size, args, variant_name="", *extra_args
):
    ...
    with torch._jit_internal._disable_emit_hooks():
        script_fn, inputs = gen_script_fn_and_args(
            name, "nn_functional", *f_args_variable
        )
