...
class ContractManager:
            ...
            if contracts_file.exists():
                ...
                for name, data in stored_contracts.items():
                    contract = self.web3.eth.contract(
                        address=data["address"], abi=data["abi"]
                    )
    ...
    def load_contract(
        self, name: str, address: str, abi: List[Dict[str, Any]]
    ) -> Dict[str, str]:
    ...
    def list_contracts(self) -> List[Dict[str, str]]:
        ...
        return [
            {"name": name, "address": contract.address}
            for name, contract in self.contracts.items()
        ]
    ...
    async def send_transaction(self, name: str, method: str, *args, **kwargs) -> str:
        ...
        try:
            ...
            tx_hash = await contract_method(*args).transact(
                {**kwargs, "gas": int(gas_estimate * 1.2)}  # Add 20% buffer
            )
