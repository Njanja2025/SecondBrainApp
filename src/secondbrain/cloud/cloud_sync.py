...
class CloudSync:
    ...
    def __init__(
        self, config_path: str = "config/cloud_sync.json", backup_dir: str = "backups"
    ):
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                self._dropbox_client.files_upload(
                    f.read(), backup_path, mode=mode, mute=True
                )
            ...
            if retain_versions:
                await self._cleanup_dropbox_versions(
                    Path(backup_path).parent, self.config["dropbox"]["retention_days"]
                )
        ...
        try:
            ...
            if retain_versions and folder_id:
                await self._cleanup_drive_versions(
                    folder_id, self.config["google_drive"]["retention_days"]
                )
        ...
        try:
            ...
            if retain_versions:
                ...
                await self._cleanup_s3_versions(
                    bucket, prefix, self.config["aws"]["retention_days"]
                )
    ...
    async def _cleanup_dropbox_versions(self, folder_path: str, retention_days: int):
    ...
    async def _cleanup_drive_versions(self, folder_id: str, retention_days: int):
        ...
        try:
            ...
            results = (
                self._drive_service.files()
                .list(
                    q=f"'{folder_id}' in parents", fields="files(id, name, createdTime)"
                )
                .execute()
            )
    ...
    async def _cleanup_s3_versions(self, bucket: str, prefix: str, retention_days: int):
