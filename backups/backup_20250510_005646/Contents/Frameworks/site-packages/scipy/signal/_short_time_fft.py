...
def _calc_dual_canonical_window(win: np.ndarray, hop: int) -> np.ndarray:
    ...
    if hop > len(win):
        raise ValueError(
            f"{hop=} is larger than window length of {len(win)}"
            + " => STFT not invertible!"
        )
    if issubclass(win.dtype.type, np.integer):
        raise ValueError(
            "Parameter 'win' cannot be of integer type, but "
...
class ShortTimeFFT:
    ...
    def mfft(self, n_: int):
        ...
        if not (n_ >= self.m_num):
            raise ValueError(
                f"Attribute mfft={n_} needs to be at least the "
    ...
    def phase_shift(self, v: int | None):
        ...
        if not isinstance(v, int):
            raise ValueError(
                f"phase_shift={v} has the unit samples. Hence "
                + "it needs to be an int or it may be None!"
            )
        if not (-self.mfft < v < self.mfft):
            raise ValueError(
                "-mfft < phase_shift < mfft does not hold "
    ...
    def stft(
        ...
        *,
        ...
        """
        return self.stft_detrend(
            x, None, p0, p1, k_offset=k_offset, padding=padding, axis=axis
        )
    ...
    def stft_detrend(
        ...
        x: np.ndarray,
        detr: (
            Callable[[np.ndarray], np.ndarray] | Literal["linear", "constant"] | None
        ),  # noqa: E501
        p0: int | None = None,
        p1: int | None = None,
        *,
        ...
        if self.onesided_fft and np.iscomplexobj(x):
            raise ValueError(
                f"Complex-valued `x` not allowed for {self.fft_mode=}'! "
    ...
    def spectrogram(
        ...
        *,
        ...
        :class:`scipy.signal.ShortTimeFFT`: Class this method belongs to.
        """
        Sx = self.stft_detrend(
            x, detr, p0, p1, k_offset=k_offset, padding=padding, axis=axis
        )
        if y is None or y is x:  # do spectrogram:
            return Sx.real**2 + Sx.imag**2
        # Cross-spectrogram:
        Sy = self.stft_detrend(
            y, detr, p0, p1, k_offset=k_offset, padding=padding, axis=axis
        )
        return Sx * Sy.conj()
    ...
    def istft(
        ...
        *,
        ...
        if not (self.k_min <= k0 < k1 <= k_max):
            raise ValueError(
                f"({self.k_min=}) <= ({k0=}) < ({k1=}) <= " + f"({k_max=}) is false!"
            )
        if not (num_pts := k1 - k0) >= n_min:
            raise ValueError(
                f"({k1=}) - ({k0=}) = {num_pts} has to be at "
                + f"least the half the window length {n_min}!"
            )

        q0 = (
            k0 // self.hop + self.p_min
            if k0 >= 0  # p_min always <= 0
            else k0 // self.hop
        )
        ...
        x = np.zeros(
            S.shape[:-2] + (n_pts,), dtype=float if self.onesided_fft else complex
        )
    ...
    def extent(
        ...
        if self.onesided_fft:
            ...
            raise ValueError(
                f"Attribute fft_mode={self.fft_mode} must be "
