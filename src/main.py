...
class SystemCheck:
    ...
    def check_system_requirements() -> Dict[str, Any]:
        ...
        if platform.python_version_tuple() < tuple(requirements["min_python_version"].split(".")):
            status["passed"] = False
            status["issues"].append(f"Python version must be >= {requirements['min_python_version']}")
        
        # Check CPU cores
        if checks["cpu_cores"] < requirements["min_cpu_cores"]:
            status["passed"] = False
            status["issues"].append(f"Need at least {requirements['min_cpu_cores']} CPU cores")
        
        # Check memory
        if checks["memory_available"] < requirements["min_memory_gb"]:
            status["passed"] = False
            status["issues"].append(f"Need at least {requirements['min_memory_gb']}GB available memory")
        
        # Check disk space
        if checks["disk_space"] < requirements["min_disk_space_gb"]:
            status["passed"] = False
            status["issues"].append(f"Need at least {requirements['min_disk_space_gb']}GB free disk space")
    ...
    async def monitor_system(self):
        ...
        while True:
            try:
                ...
                self.phantom.log_event(
                    "Monitor Error",
                    f"Error monitoring system: {str(e)}",
                    "ERROR"
                )
