...
class BackupLogReport:
    ...
    def __init__(self, report_dir: str = "phantom_reports", email_config: Optional[Dict] = None):
        """
    ...
    def log_event(self, event_type: str, message: str, status: str = "INFO", details: Dict = None):
        """
    ...
    def generate_report(self) -> str:
        ...
        try:
            ...
            for entry in self.entries:
                ...
                """)
                if entry['details']:
                    pdf.multi_cell(0, 10, f"Details: {json.dumps(entry['details'], indent=2)}")
                pdf.ln(5)
            
            # Save report
            report_path = self.report_dir / f"cloud_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    ...
    def email_report(self) -> bool:
        ...
        if not self.latest_report_path or not self.email_config:
            logger.error("Cannot send email: No report generated or email not configured")
            return False
        
        try:
            msg = MIMEMultipart()
            msg['Subject'] = f'SecondBrain Cloud Operations Report - {datetime.now().strftime("%Y-%m-%d %H:%M")}'
