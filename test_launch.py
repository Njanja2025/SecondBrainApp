...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("launch_test.log"), logging.StreamHandler()],
)
...
class TestVoiceAssistantLaunch(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        logger.info(
            f"Completed test: {self._testMethodName} (Duration: {duration:.2f}s)"
        )
    ...
    def test_1_initialization(self):
        ...
        self.assertIsNotNone(
            self.assistant.engine.getProperty("rate"), "Speech rate is None"
        )
        self.assertIsNotNone(
            self.assistant.engine.getProperty("volume"), "Volume is None"
        )

        # Test recognizer properties
        self.assertIsNotNone(
            self.assistant.recognizer.energy_threshold, "Energy threshold is None"
        )
        self.assertIsNotNone(
            self.assistant.recognizer.pause_threshold, "Pause threshold is None"
        )
    ...
    def test_2_voice_commands(self):
        ...
        for cmd, description in commands_to_test.items():
            ...
            self.assertIsInstance(
                response, str, f"Response for {cmd} command is not a string"
            )
    ...
    def test_4_system_monitor_plugin(self):
        ...
        self.assertIn(
            "system", self.assistant.plugins, "System monitor plugin not registered"
        )
    ...
    def test_5_error_handling(self):
        ...
        for cmd, args in invalid_args:
            ...
            self.assertIn(
                "Error", response, f"Error message not found in response for {cmd}"
            )
