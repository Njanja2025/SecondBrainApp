...
class DataClassBuilder(ExtClassBuilder):
    ...
    def finalize(self, ir: ClassIR) -> None:
        ...
        dataclass_type_val = self.builder.load_str(
            dataclass_type(self.cdef) or "unknown"
        )
        self.builder.call_c(
            dataclass_sleight_of_hand,
            [
    ...
    if new_style_type_args:
        bases = [
            make_generic_base_class(
                builder, cdef.fullname, new_style_type_args, cdef.line
            )
        ]
    else:
        bases = []
...
def gen_glue_ne_method(builder: IRBuilder, cls: ClassIR, line: int) -> None:
    ...
    with builder.enter_method(cls, "__ne__", eq_sig.ret_type):
        ...
        can_return_not_implemented = is_subtype(
            not_implemented_op.type, eq_sig.ret_type
        )
        ...
        if not strict_typing or can_return_not_implemented:
            ...
            rettype = (
                bool_rprimitive if return_bool and strict_typing else object_rprimitive
            )
