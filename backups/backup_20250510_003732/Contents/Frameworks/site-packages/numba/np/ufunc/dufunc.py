...
class UfuncAtIterator:
    ...
    def _prepare(self, context, builder):
        ...
        if self.need_advanced_indexing():
            indices = cgutils.unpack_tuple(builder, indices, count=len(indices_ty))
        ...
        self.indexer = FancyIndexer(context, builder, a_ty, a, index_types, indices)
    ...
    def _load_val(self, context, builder, loop_indices, array, array_ty):
        ...
        ptr = cgutils.get_item_pointer2(
            context, builder, data, shapes, strides, array_ty.layout, loop_indices
        )
    ...
    def _call_ufunc(self, context, builder, loop_indices):
        ...
        res = context.call_internal(builder, cres.fndesc, cres.signature, args)
...
class DUFuncLowerer(UfuncLowererBase):
    ...
    def __init__(self, dufunc):
        ...
        super().__init__(dufunc, make_dufunc_kernel, npyimpl.numpy_ufunc_kernel)
...
class DUFunc(serialize.ReduceMixin, _internal._DUFunc, UfuncBase):
    ...
    def _compile_for_args(self, *args, **kws):
        ...
        if kws:
            ...
            if kws:
                raise TypeError(
                    "unexpected keyword arguments to ufunc: %s"
    ...
    def _compile_for_argtys(self, argtys, return_type=None):
        ...
        for k, cres in self._dispatcher.overloads.items():
            if argtys == k.args:
                msg = (
                    "Compilation requested for previously compiled argument"
    ...
    def _install_ufunc_at(self, template) -> None:
        ...
        def ol_at(ufunc, a, indices, b=None):
            warnings.warn(
                "ufunc.at feature is experimental",
            ...
            b_array = isinstance(
                b, (types.Array, types.Sequence, types.List, types.Tuple)
            )
            ...
            def impl_cast(ufunc, a, indices, b=None):
                if b_none:
                    ...
                    return ufunc.at(a, np.asarray(indices), np.asarray(b))
    ...
    def _install_ufunc_reduce(self, template) -> None:
        ...
        def ol_reduce(ufunc, array, axis=0, dtype=None, initial=None):
            ...
            warnings.warn(
                "ufunc.reduce feature is experimental",
                category=errors.NumbaExperimentalFeatureWarning,
            )
            ...
            axis_int_tuple = isinstance(axis, types.UniTuple) and isinstance(
                axis.dtype, types.Integer
            )
            ...
            if not self.reorderable and axis_int_tuple and len(axis) > 1:
                msg = (
                    f"reduction operation '{ufunc_name}' is not "
            ...
            def compute_flat_idx(typingctx, strides, itemsize, idx, axis):
                ...
                def codegen(context, builder, sig, args):
                    ...
                    for i in range(len_idx):
                        block, flat_idx = gen_block(
                            builder, i, f"axis_{i}", switch_end, args
                        )
                ...
                if initial is None and identity is None:
                    for idx, val in np.ndenumerate(array):
                        if idx[axis] == 0:
                            ...
                            flat_pos = compute_flat_idx(
                                r.strides, r.itemsize, idx, axis
                            )
            ...
            elif axis == 0 or isinstance(
                axis, (types.Integer, types.Omitted, types.IntegerLiteral)
            ):
    ...
    def at(self, a, indices, b=None):
        ...
        ewise_types = tuple(
            arg.dtype if isinstance(arg, types.Array) else arg for arg in argtys
        )
    ...
    def _install_type(self, typingctx=None):
        ...
        _ty_cls = type(
            "DUFuncTyping_" + self.ufunc.__name__,
            (AbstractTemplate,),
        ...
        if sig is None:
            ...
            if self._frozen:
                raise errors.NumbaTypeError(
                    "cannot call %s with types %s" % (self, argtys)
                )
