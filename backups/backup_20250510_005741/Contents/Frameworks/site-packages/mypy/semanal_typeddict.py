        ...
        while isinstance(typ, (RequiredType, ReadOnlyType)):
            ...
            if isinstance(typ, ReadOnlyType):
                if context is not None and seen_readonly:
                    self.fail(
                        '"ReadOnly[]" type cannot be nested',
                        context,
                        code=codes.VALID_TYPE,
                    )
                readonly = True
    ...
    def check_typeddict(
        ...
        if not ok:
            ...
            types = [  # unwrap Required[T] or ReadOnly[T] to just T
                t.item if isinstance(t, (RequiredType, ReadOnlyType)) else t
                for t in types
            ]
        ...
        for field_name_expr, field_type_expr in dict_items:
            ...
            try:
                ...
                self.fail_typeddict_arg(
                    "Use dict literal for nested TypedDict", field_type_expr
                )
                return [], [], False
            analyzed = self.api.anal_type(
                type,
