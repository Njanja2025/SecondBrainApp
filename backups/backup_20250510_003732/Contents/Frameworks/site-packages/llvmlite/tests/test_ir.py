...
class TestFunction(TestBase):
    ...
    proto = (
        ...
        else """i32 @"my_func"(i32 %".1", i32 %".2", double %".3", i32* %".4")"""
    )
    ...
    def test_function_section_meta(self):
        ...
        func.set_metadata("dbg", module.add_metadata([]))
        asm = self.descr(func).strip()
        self.assertEqual(asm, f'declare {self.proto} section "a_section" !dbg !0')
        # Check pickling
        self.assert_pickle_correctly(func)
    ...
    def test_function_attr_meta(self):
        ...
        func.set_metadata("dbg", module.add_metadata([]))
        asm = self.descr(func).strip()
        self.assertEqual(asm, f"declare {self.proto} alwaysinline !dbg !0")
        # Check pickling
        self.assert_pickle_correctly(func)
    ...
    def test_declare_intrinsics(self):
        ...
        if opaque_pointers_enabled:
            self.check_descr(
                self.descr(memset).strip(),
                """\
                declare void @"llvm.memset.p0i8.i32"(ptr %".1", i8 %".2", i32 %".3", i1 %".4")""",
            )  # noqa E501
            self.check_descr(
                self.descr(memcpy).strip(),
                """\
                declare void @"llvm.memcpy.p0i8.p0i8.i32"(ptr %".1", ptr %".2", i32 %".3", i1 %".4")""",
            )  # noqa E501
        else:
            self.check_descr(
                self.descr(memset).strip(),
...
class TestIR(TestBase):
    ...
    def test_debug_info_2(self):
        ...
        self.assert_ir_line(
            '!2 = distinct !DIFile(directory: "bar", filename: ' '"foo")', strmod
        )
        self.assert_valid_ir(mod)
    ...
    def test_debug_info_gvar(self):
        ...
        di_float = mod.add_debug_info(
            "DIBasicType",
            {"name": "float", "size": 32, "encoding": ir.DIToken("DW_ATE_float")},
        )
        di_gvar = mod.add_debug_info(
            "DIGlobalVariableExpression",
            {
                "expr": mod.add_debug_info("DIExpression", {}),
                "var": mod.add_debug_info(
                    "DIGlobalVariable",
                    {"name": gvar.name, "type": di_float, "isDefinition": True},
                    is_distinct=True,
                ),
            },
        )
        ...
        strmod = str(mod)
        self.assert_ir_line(
            "!0 = !DIBasicType(encoding: DW_ATE_float, " 'name: "float", size: 32)',
            strmod,
        )
        self.assert_ir_line("!1 = !DIExpression()", strmod)
        self.assert_ir_line(
            "!2 = distinct !DIGlobalVariable(isDefinition: "
            'true, name: "gvar", type: !0)',
            strmod,
        )
        self.assert_ir_line(
            "!3 = !DIGlobalVariableExpression(expr: !1, " "var: !2)", strmod
        )
        self.assert_ir_line(
            '@"gvar" = global float 0x4045000000000000, ' "!dbg !3", strmod
        )
...
class TestGlobalValues(TestBase):
    ...
    def test_globals_access(self):
        ...
        if opaque_pointers_enabled:
            self.assertEqual(
                repr(globdouble), "<ir.GlobalVariable 'globdouble' of type 'ptr'>"
            )
        else:
            self.assertEqual(
                repr(globdouble), "<ir.GlobalVariable 'globdouble' of type 'double*'>"
            )
...
class TestBuildInstructions(TestBase):
    ...
    def test_binops(self):
        ...
        with self.assertRaises(ValueError) as cm:
            builder.add(a, ff)
        self.assertEqual(
            str(cm.exception), "Operands must be the same type, got (i32, double)"
        )
        self.assertFalse(block.is_terminated)
        self.check_block(
            block,
...
my_block:
    ...
    def test_float_comparisons(self):
        ...
        builder.fcmp_unordered(
            "olt", a, b, "y", flags=["nnan", "ninf", "nsz", "arcp", "fast"]
        )
        self.assertFalse(block.is_terminated)
        self.check_block(
            block,
    ...
    def test_mem_ops(self):
        ...
        if opaque_pointers_enabled:
            self.assertEqual(
                str(cm.exception), "cannot store i32 to ptr: mismatching types"
            )
            self.check_block(
                block,
                """\
            ...
            self.assertEqual(
                str(cm.exception), "cannot store i32 to double*: mismatching types"
            )
            self.check_block(
                block,
                """\
    ...
    def test_extract_insert_value(self):
        ...
        c_inner = ir.Constant(
            tp_inner, (ir.Constant(int32, 4), ir.Constant(int1, True))
        )
        # Flat structure
        c = builder.extract_value(c_inner, 0, name="c")  # noqa F841
        d = builder.insert_value(c_inner, a, 0, name="d")  # noqa F841
        e = builder.insert_value(
            d, ir.Constant(int1, False), 1, name="e"
        )  # noqa F841 E501
    ...
    def test_call_attributes(self):
        ...
        builder.call(
            ...
            arg_attrs={0: ("sret", "noalias"), 2: "noalias"},
        )
        # FIXME: Remove `else' once TP are no longer supported.
        if opaque_pointers_enabled:
            self.check_block_regex(
                block,
                """\
            my_block:
                %"retval" = alloca i32
    ...
    def test_landingpad(self):
        ...
        int_typeinfo = ir.GlobalVariable(
            builder.function.module, int8.as_pointer(), "_ZTIi"
        )
        int_typeinfo.global_constant = True
        lp.add_clause(ir.CatchClause(int_typeinfo))
        lp.add_clause(
            ir.FilterClause(
                ir.Constant(ir.ArrayType(int_typeinfo.type, 1), [int_typeinfo])
            )
        )
        builder.resume(lp)
        # FIXME: Remove `else' once TP are no longer supported.
    ...
    def test_arg_attributes(self):
        def gen_code(attr_name):
            fnty = ir.FunctionType(
                ir.IntType(32), [ir.IntType(32).as_pointer(), ir.IntType(32)]
            )
            module = ir.Module()
...
class TestBuilderMisc(TestBase):
    ...
    def test_if_then_long_label(self):
        ...
        self.check_func_body(
            ...
            """.format(
                full_label=full_label, label=full_label[:25] + ".."
            ),
        )
...
class TestTypes(TestBase):
    ...
    def test_str(self):
        ...
        self.assertEqual(str(ir.FunctionType(int1, (flt, dbl))), "i1 (float, double)")
        self.assertEqual(str(ir.FunctionType(int1, (), var_arg=True)), "i1 (...)")
        self.assertEqual(
            str(ir.FunctionType(int1, (flt,), var_arg=True)), "i1 (float, ...)"
        )
        self.assertEqual(
        ...
        # FIXME: Remove `else' once TP are no longer supported.
        if opaque_pointers_enabled:
            self.assertEqual(str(ir.ArrayType(ir.PointerType(int1), 5)), "[5 x ptr]")
        ...
        self.assertEqual(str(ir.LiteralStructType((int1,), packed=True)), "<{i1}>")
        self.assertEqual(
            str(ir.LiteralStructType((int1, flt), packed=True)), "<{i1, float}>"
        )
...
class TestConstant(TestBase):
    ...
    def test_structs(self):
        ...
        c = ir.Constant(
            st1, (ir.Constant(ir.FloatType(), 1.5), ir.Constant(int1, True))
        )
        self.assertEqual(str(c), "{float, i1} {float 0x3ff8000000000000, i1 true}")
        c = ir.Constant.literal_struct(
            (ir.Constant(ir.FloatType(), 1.5), ir.Constant(int1, True))
        )
        self.assertEqual(c.type, st1)
        self.assertEqual(str(c), "{float, i1} {float 0x3ff8000000000000, i1 true}")
        c = ir.Constant.literal_struct(
            (ir.Constant(ir.FloatType(), 1.5), ir.Constant(int1, ir.Undefined))
        )
        self.assertEqual(c.type, st1)
    ...
    def test_gep(self):
        ...
        if opaque_pointers_enabled:
            self.assertEqual(
                str(c), 'getelementptr ({float, i1}, ptr @"myconstant", i32 0, i32 1)'
            )  # noqa E501
        else:
            self.assertEqual(
                str(c),
    ...
    def test_flt_binops(self):
        ...
        for fn, irop in oracle.items():
            actual = str(fn(two))
            expected = irop + (
                " (float 0x3ff0000000000000," " float 0x4000000000000000)"
            )
            self.assertEqual(actual, expected)
