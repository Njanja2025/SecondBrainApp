        ...
        if self.map_inputs_layer is None:
            ...
            if self.enable_pronunciation:
                if input_pronunciation_ids is None:
                    input_pronunciation_ids = torch.zeros(
                        input_shape, dtype=torch.long, device=device
                    )
                embedding_pronunciation = self.pronunciation_embed(
                    input_pronunciation_ids
        ...
        **kwargs,
        ...
        if labels_input_ids is not None:
            ...
            if attack_input_ids is not None:
                ...
                labels_pooled_output_norm = torch.nn.functional.normalize(
                    labels_pooled_output, dim=-1
                )
                attack_pooled_output_norm = torch.nn.functional.normalize(
                    attack_pooled_output, dim=-1
                )

                sim_matrix = torch.matmul(
                    pooled_output_norm, attack_pooled_output_norm.T
                )  # batch_size * hidden_dim
                sim_matrix_target = torch.matmul(
                    labels_pooled_output_norm, attack_pooled_output_norm.T
                )
                batch_labels = torch.tensor(list(range(batch_size)), device=device)
                contrastive_loss = (
                    loss_fct(
                        100 * sim_matrix.view(batch_size, -1), batch_labels.view(-1)
                    )
                    + loss_fct(
    ...
    def prepare_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        if input_pronunciation_ids is not None:
            input_pronunciation_ids = torch.cat(
...
class RoCBertForMultipleChoice(RoCBertPreTrainedModel):
    ...
    def forward(
        ...
        input_shape_ids = (
            input_shape_ids.view(-1, input_shape_ids.size(-1))
            if input_shape_ids is not None
            else None
        )
        input_pronunciation_ids = (
