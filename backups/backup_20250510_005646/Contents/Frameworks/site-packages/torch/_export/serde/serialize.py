...
class GraphModuleSerializer:
    ...
    def handle_call_function(self, node: torch.fx.Node):
        ...
        if node.target in _SYM_INT_OPS:
            ...
            elif isinstance(meta_val, (list, tuple)) and all(
                isinstance(v, torch.Tensor) for v in meta_val
            ):
...
class GraphModuleDeserializer:
    ...
    def deserialize_node(self, serialized_node: Node, target: Callable) -> None:
        if target.__module__ == "_operator":  # TODO(zhxchen17) Follow up on this.
            ...
            assert len(serialized_node.outputs) == 1 and serialized_node.outputs[
                0
            ].type in (
                "as_tensors",
                "as_tensor",
    ...
    def deserialize(
        ...
        return GraphModuleDeserializer.Result(
            graph_module=torch._export.exported_program._create_graph_module_for_export(
    ...
    def deserialize_outputs(self, serialized_node: Node, fx_node: torch.fx.Node):
        ...
        if _is_single_tensor_return(fx_node.target):
            ...
            return
        elif len(returns) == 1 and isinstance(
    ...
    def deserialize_multiple_outputs(
        ...
        def generate_getitem(
            meta_val, fx_node: torch.fx.Node, arg: TensorArgument, idx: int
        ):
            name = arg.name
...
class ExportedProgramDeserializer:
    ...
    def deserialize_range_constraints(
        ...
        for k, v in symbol_name_to_range.items():
            if symbol := symbol_name_to_symbol.get(k):
                ...
                log.warning(
                    f"Symbol {k} did not appear in the graph that was deserialized"
                )  # noqa: G004
    ...
    def deserialize(
        ...
        model_opset_version: Optional[Dict[str, int]] = (
            serialized_artifact.exported_program.opset_version
        )
