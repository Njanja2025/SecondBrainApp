...
class IntegrationLogger:
    ...
    def log_event(self, service: str, event_type: str, status: str,
        ...
        try:
            # Generate event ID
            event_id = f"{service}_{event_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            # Log event
            log_message = f"Integration event: {event_id} - {service} - {event_type} - {status}"
            if error:
    ...
    def get_events(self, service: Optional[str] = None,
        ...
        try:
            ...
            service_dirs = [self.log_dir / service] if service else self.log_dir.iterdir()
            ...
            for service_dir in service_dirs:
                ...
                for event_file in service_dir.glob("*.json"):
                    try:
                        ...
                        logger.error(f"Failed to process event file {event_file}: {str(e)}")
    ...
    def get_service_stats(self, service: str,
                         start_time: Optional[datetime] = None,
        ...
        """
        try:
            events = self.get_events(service=service, start_time=start_time, end_time=end_time)
