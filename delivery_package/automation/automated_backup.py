...
class AutomatedBackup:
    ...
    def create_backup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log success
            self.phantom.log_event(
                "Backup", f"Successfully created backup: {timestamp}", "INFO"
            )
        ...
        for cloud, path in self.cloud_paths.items():
            if path:
                try:
                    ...
                    self.phantom.log_event(
                        "Cloud Sync", f"Failed to sync to {cloud}: {str(e)}", "ERROR"
                    )
    ...
    def cleanup_old_backups(self, max_backups: int = 7):
        ...
        if len(backups) > max_backups:
            for backup in backups[:-max_backups]:
                ...
                self.phantom.log_event(
                    "Backup Cleanup", f"Removed old backup: {backup.name}", "INFO"
                )
