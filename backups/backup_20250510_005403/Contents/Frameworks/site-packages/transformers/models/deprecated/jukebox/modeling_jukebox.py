...
class JukeboxBottleneckBlock(nn.Module):
    ...
    def update_codebook(self, hidden_states, latent_states):
        ...
        with torch.no_grad():
            ...
            dk = torch.linalg.norm(self.codebook - old_codebook) / np.sqrt(
    ...
    def preprocess(self, hidden_states):
        ...
        if hidden_states.shape[-1] == self.codebook_width:
            prenorm = torch.linalg.norm(
                hidden_states - torch.mean(hidden_states)
            ) / np.sqrt(np.prod(hidden_states.shape))
            ...
            prenorm = (
                torch.linalg.norm(x1 - torch.mean(x1)) / np.sqrt(np.prod(x1.shape))
