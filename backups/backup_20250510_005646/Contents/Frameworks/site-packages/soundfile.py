...
try:  # packaged lib (in _soundfile_data which should be on python path)
    if _sys.platform == "darwin":
        ...
        if _architecture()[0] == "64bit":
            ...
            raise OSError(
                "no packaged library for Windows {} {}".format(
                    _architecture(), _machine()
                )
            )
    ...
    try:  # system-wide libsndfile:
        ...
        if _sys.platform == "darwin" and _machine() == "arm64":
            _hbrew_path = (
                "/opt/homebrew/lib/"
                if _os.path.isdir("/opt/homebrew/lib/")
                else "/usr/local/lib/"
            )
...
class SoundFile(object):
    ...
    def __repr__(self):
        ...
        compression_setting += (
            ...
            if self.bitrate_mode is not None
            else ""
        )
        return (
    ...
    def _check_dtype(self, dtype):
        ...
        try:
            ...
            raise ValueError(
                "dtype must be one of {0!r} and not {1!r}".format(
    ...
    def _array_io(self, action, array, frames):
        ...
        if array.ndim not in (1, 2):
            raise ValueError(
                "Invalid shape: {0!r} ({1})".format(
                    array.shape,
                    (
                        "0 dimensions not supported"
                        if array.ndim < 1
                        else "too many dimensions"
                    ),
                )
            )
        ...
        if array_channels != self.channels:
            raise ValueError(
                "Invalid shape: {0!r} (Expected {1} channels, got {2})".format(
                    array.shape, self.channels, array_channels
                )
            )
    ...
    def _set_bitrate_mode(self, bitrate_mode):
        ...
        err = _snd.sf_command(
            self._file,
            _snd.SFC_SET_BITRATE_MODE,
            pointer_bitrate_mode,
            _ffi.sizeof(pointer_bitrate_mode),
        )
        if err != _snd.SF_TRUE:
            err = _snd.sf_error(self._file)
    ...
    def _set_compression_level(self, compression_level):
        ...
        err = _snd.sf_command(
            self._file,
            _snd.SFC_SET_COMPRESSION_LEVEL,
            pointer_compression_level,
            _ffi.sizeof(pointer_compression_level),
        )
        if err != _snd.SF_TRUE:
            err = _snd.sf_error(self._file)
            raise LibsndfileError(
                err, f"Error set compression level {compression_level}"
            )
