...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/security.log"), logging.StreamHandler()],
)
...
class SecurityManager:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            default_config = {
                "encryption": {"algorithm": "AES-256", "key_rotation_days": 30},
                "access_control": {
    ...
    def generate_token(self, user_id: str, permissions: List[str]) -> str:
        ...
        payload = {"user_id": user_id, "permissions": permissions, "exp": expiry}
    ...
    def _check_security_metrics(self):
        ...
        if (
            failed_logins
            >= self.config["monitoring"]["alert_thresholds"]["failed_logins_per_hour"]
        ):
            logger.warning(
                f"High number of failed login attempts: {failed_logins} in the last hour"
            )
        ...
        if (
            api_requests
            >= self.config["monitoring"]["alert_thresholds"]["api_requests_per_minute"]
        ):
    ...
    def check_rate_limit(self, key: str, limit: int, window: int) -> bool:
        ...
        with self.rate_limit_lock:
            ...
            self.rate_limits[key] = [
                ts for ts in self.rate_limits[key] if ts > now - window
            ]
