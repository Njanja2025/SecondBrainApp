...
class PatternChecker(PatternVisitor[PatternType]):
    ...
    def visit_sequence_pattern(self, o: SequencePattern) -> PatternType:
        ...
        if isinstance(current_type, TupleType):
            ...
            if unpack_index is None:
                ...
                for it in inner_types:
                    ...
                    # analysis instead.
                    if isinstance(it, UnpackType) and isinstance(
                        it.type, TypeVarTupleType
                    ):
        ...
        if isinstance(current_type, TupleType) and unpack_index is None:
            ...
            if isinstance(current_type, TypeVarType):
                new_bound = self.narrow_sequence_child(
                    current_type.upper_bound, new_inner_type, o
                )
    ...
    def contract_starred_pattern_types(
        ...
        if unpack_index is not None:
            ...
            prefix, middle, suffix = split_with_prefix_and_suffix(
                tuple(
                    [
                        UnpackType(unpacked) if isinstance(t, UnpackType) else t
                        for t in types
                    ]
                ),
                star_pos,
        ...
        if original_unpack:
            ...
            for i, t in enumerate(types):
                if i != star_pos:
                    ...
                    res.append(
                        UnpackType(self.chk.named_generic_type("builtins.tuple", [t]))
                    )
            return res
        new_types = types[:star_pos]
    ...
    def narrow_sequence_child(
        self, outer_type: Type, inner_type: Type, ctx: Context
    ) -> Type:
        ...
        if isinstance(type_info, TypeInfo):
            ...
            for tv in type_info.defn.type_vars:
                if isinstance(tv, TypeVarTupleType):
                    args.append(
                        UnpackType(
                            self.chk.named_generic_type("builtins.tuple", [any_type])
                        )
                    )
                else:
