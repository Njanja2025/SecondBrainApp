...
def validate_hpu_device(location):
    ...
    if not hpu.is_available():
        raise RuntimeError(
            "Attempting to deserialize object on a HPU "
    ...
    if device >= device_count:
        raise RuntimeError(
            "Attempting to deserialize object on HPU device "
...
def _privateuse1_deserialize(obj, location):
    ...
    if location.startswith(backend_name):
        if not hasattr(obj, backend_name):
            raise RuntimeError(
                f"Attempting to load the storages to the {backend_name.upper()} device "
