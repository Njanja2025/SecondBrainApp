...
def test_module(module_name: str) -> Iterator[Error]:
    ...
    try:
        ...
        yield Error(
            [module_name],
            f"failed to import.{note} {type(e).__name__}: {e}",
            stub,
            MISSING,
        )
        return
...
def verify_mypyfile(
    ...
    if hasattr(runtime, "__all__"):
        ...
        if "__all__" in stub.names:
            ...
            yield Error(
                object_path + ["__all__"], "is not present in stub", MISSING, runtime
            )
    else:
        ...
        if signature:
            ...
            for arg in signature.parameters.values():
                ...
                if arg.kind == inspect.Parameter.POSITIONAL_ONLY:
                    ...
                    arg_kinds.append(
                        nodes.ARG_NAMED if no_default else nodes.ARG_NAMED_OPT
                    )
...
ANNOYING_STDLIB_MODULES: Final = frozenset(
