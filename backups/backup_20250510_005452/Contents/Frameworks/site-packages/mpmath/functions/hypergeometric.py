...
def _check_need_perturb(ctx, terms, prec, discard_known_zeros):
    ...
    for term_index, term in enumerate(terms):
        ...
        if (
            discard_known_zeros
            and pole_count[1] > pole_count[0] + pole_count[2]
    ...
    try:
        while 1:
            ...
            if verbose:
                ...
                print("prec =", ctx.prec)
                print("hextra", hextra)
            perturb, recompute, extraprec, discard = _check_need_perturb(
                ctx, terms, orig, discard_known_zeros
            )
            ctx.prec += extraprec
            if perturb:
                if "hmag" in kwargs:
            ...
            if precision_ok and (not perturb) or ctx.isnan(cancellation):
                ...
                if perturbed_reference_value is None:
                    ...
                    ctx.mag(sumvalue - perturbed_reference_value)
    ...
    if magz >= 7 and not (ctx.isint(a) and ctx.re(a) <= 0):
        ...
        try:
            try:
                ...
                if (
                    ctx._is_real_type(a)
                    and ctx._is_real_type(b)
                    and ctx._is_real_type(z)
                ):
                    v = ctx._re(v)
                return +v
            except ctx.NoConvergence:
...
def _hyp2f1(ctx, a_s, b_s, z, **kwargs):
    ...
    if z == 1:
        ...
        zerodiv = (
            ctx.isint(c)
            and c <= 0
            and not ((ctx.isint(a) and c <= a <= 0) or (ctx.isint(b) and c <= b <= 0))
        )
    ...
    if (
        absz <= 0.8
        or (ctx.isint(a) and a <= 0 and a >= -1000)
        or (ctx.isint(b) and b <= 0 and b >= -1000)
    ):
...
def _hypq1fq(ctx, p, q, a_s, b_s, z, **kwargs):
    ...
    if absz < 1.1 and ctx._re(z) <= 1:
        ...
        def log_diffs(k0):
            b2 = b_s + [1]
            yield sum(ctx.loggamma(a + k0) for a in a_s) - sum(
                ctx.loggamma(b + k0) for b in b2
            ) + k0 * ctx.log(z)
            i = 0
            while 1:
                v = sum(ctx.psi(i, a + k0) for a in a_s) - sum(
                    ctx.psi(i, b + k0) for b in b2
                )
                if i == 0:
                    v += ctx.log(z)
                yield v
...
def _hyp2f2(ctx, a_s, b_s, z, **kwargs):
    ...
    if can_use_asymptotic:
        ...
        try:
            try:
                ...
                v = ctx.hypercomb(
                    h, [a1, a2, b1, b2], force_series=True, maxterms=4 * ctx.prec
                )
    ...
    return ctx.hypsum(
        2, 2, (a1type, a2type, b1type, b2type), [a1, a2, b1, b2], z, **kwargs
    )
...
def _hyp1f2(ctx, a_s, b_s, z, **kwargs):
    ...
    can_use_asymptotic = (
        (not kwargs.get("force_series"))
        and (ctx.mag(absz) > 19)
        and (ctx.sqrt(absz) > 1.5 * orig)
    )  # and \
        ...
        try:
            try:
                ...
                def h(a1, b1, b2):
                    ...
                    c[0] = ctx.one
                    c[1] = 2 * (
                        ctx.mpq_1_4 * (3 * a1 + b1 + b2 - 2) * (a1 - b1 - b2)
                        + b1 * b2
                        - ctx.mpq_3_16
                    )
                    c[2] = 2 * (
                        b1 * b2
                        + ctx.mpq_1_4 * (a1 - b1 - b2) * (3 * a1 + b1 + b2 - 2)
                        - ctx.mpq_3_16
                    ) ** 2 + ctx.mpq_1_16 * (
                        -16 * (2 * a1 - 3) * b1 * b2
                        + 4 * (a1 - b1 - b2) * (-8 * a1**2 + 11 * a1 + b1 + b2 - 2)
                        - 3
                    )
                    s1 = 0
                    s2 = 0
                    ...
                    while 1:
                        ...
                        k += 1
                    S = (
                        ctx.expj(ctx.pi * X + 2 * ctx.sqrt(-z)) * s1
                ...
                v = ctx.hypercomb(
                    h, [a1, b1, b2], force_series=True, maxterms=4 * ctx.prec
                )
...
def _hyp2f3(ctx, a_s, b_s, z, **kwargs):
            ...
            try:
                ...
                def h(a1, a2, b1, b2, b3):
                    ...
                    c[1] = 2 * (
                        B
                        - A
                        + ctx.mpq_1_4 * (3 * A2 + B3 - 2) * (A2 - B3)
                        - ctx.mpq_3_16
                    )
                    c[2] = ctx.mpq_1_2 * c[1] ** 2 + ctx.mpq_1_16 * (
                        -16 * (2 * A2 - 3) * (B - A)
                        + 32 * R
                        + 4 * (-8 * A2**2 + 11 * A2 + 8 * A + B3 - 2) * (A2 - B3)
                        - 3
                    )
                    s1 = 0
                    s2 = 0
                    ...
                    while 1:
                        if k not in c:
                            ...
                            uu3 = (
                                5 * (k - 1) ** 2
                                + 2 * (-10 * X + A2 - 3 * B3 + 3) * (k - 1)
                                + 2 * c[1]
                            )
                            c[k] = (
                                ctx.one
                                / (2 * k)
                                * (uu1 * c[k - 3] - uu2 * c[k - 2] + uu3 * c[k - 1])
                            )
                    ...
                    T2 = (
                        ...
                        [-a1],
                        [b1, b2, b3, a2 - a1],
                        [a2, b1 - a1, b2 - a1, b3 - a1],
                        [a1, a1 - b1 + 1, a1 - b2 + 1, a1 - b3 + 1],
                        [a1 - a2 + 1],
                        1 / z,
                    )
                    T3 = (
                        ...
                        [-a2],
                        [b1, b2, b3, a1 - a2],
                        [a1, b1 - a2, b2 - a2, b3 - a2],
                        [a2, a2 - b1 + 1, a2 - b2 + 1, a2 - b3 + 1],
                        [-a1 + a2 + 1],
                        1 / z,
                    )
                    return T1, T2, T3
                ...
                v = ctx.hypercomb(
                    h, [a1, a2, b1, b2, b3], force_series=True, maxterms=4 * ctx.prec
                )
    ...
    return ctx.hypsum(
        2,
        3,
        (a1type, a2type, b1type, b2type, b3type),
        [a1, a2, b1, b2, b3],
        z,
        **kwargs,
    )
...
def appellf2(ctx, a, b1, b2, c1, c2, x, y, **kwargs):
    # TODO: continuation
    return ctx.hyper2d(
        {"m+n": [a], "m": [b1], "n": [b2]}, {"m": [c1], "n": [c2]}, x, y, **kwargs
    )
    ...
    try:
        ...
        while 1:
            ...
            for a in a_2n_sub_m:
                ...
                outer_b.append(a - m - 1)
            inner = ctx.hyper(
                inner_a, inner_b, inner_sign * y, zeroprec=ctx.prec, **kwargs
            )
