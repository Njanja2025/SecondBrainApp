...
def _constrain_symbol_range(
    ...
    log.debug(
        "_constrain_symbol_range %s [%s, %s] [%s, %s]",
        s,
        compiler_min,
        compiler_max,
        runtime_min,
...
def _constrain_range_for_size(a, min: Optional[int] = None, max: Optional[int] = None):
    ...
    if max <= 2:
        raise ValueError(
            f"Maximum value to constrain_as_size must be greater than 2, but was {max}"
        )
    ...
    if isinstance(a.node.expr, sympy.Integer):
        if not (min <= int(a.node.expr) <= max):
            raise ValueRangeError(
                f"Invalid value {int(a.node.expr)} for range [{min}:{max}]"
            )
        return
    assert isinstance(a.node.expr, sympy.Symbol), "constraining non-Symbols NYI"
...
def _assert_bound_is_rational(expr: sympy.Expr, bound: ValueRanges):
    ...
    """
    assert (
        bound.lower.is_rational
        or bound.lower.is_Boolean
        or not bound.lower.is_finite
        or expr.has(sympy.Pow)
    ), (bound, expr)
    assert (
        bound.upper.is_rational
        or bound.upper.is_Boolean
        or not bound.upper.is_finite
        or expr.has(sympy.Pow)
    ), (bound, expr)
...
def cast_symbool_to_symint_guardless(symbool: torch.SymBool) -> torch.SymInt:
    ...
    return symbool.node.shape_env.create_symintnode(
        int_sym, hint=int(symbool.node.require_hint())
    )
...
class DimConstraints:
    ...
    def __init__(
        self, symbol_to_source, var_to_val, marked_dynamic, source_name_to_debug_name
    ):
        # We try to solve systems of inequalities with 1 free variable.
        self._univariate_inequalities: Dict[sympy.Symbol, Set[sympy.Expr]] = (
            defaultdict(set)
        )
    ...
    def solve(self, disable_congruences=True, disable_equivalences=True):
                ...
                if s not in self._substitutions or not sympy.checksol(
                    ...
                    if disable_congruences:
                        ...
                        self._dynamic_results.add(
                            self._dcp.doprint(sympy.Eq(congruence, 0))
                        )
            ...
            if disable_equivalences and not isinstance(expr, sympy.Symbol):
                for s in expr.free_symbols:
                    ...
                    self._dynamic_results = {
                        r for r in self._dynamic_results if sexpr not in r
                    }
        ...
        for source, expr in self._symbolic_equivalences:
            self._dynamic_results.add(
                f"{self._dcp.print_source(source)} == {self._dcp.doprint(expr)}"
            )
    ...
    def prettify_results(
        ...
        if self._dcp.source_name_to_debug_name:
            ...
            if forced_specializations:
                debug_names.update(
                    k.split(" = ")[0] for k in forced_specializations.keys()
                )
                buf += (
            ...
            if constraint_violation_error:
                match = re.search(
                    r"Constraints violated \((.*)\)", constraint_violation_error.args[0]
                )
        ...
        signature = original_signature.replace(
...
class ShapeEnv:
    ...
    def _init(
        ...
        *,
        ...
        self.var_to_guards: Dict[
            sympy.Symbol, Tuple[Optional[ShapeGuard], Optional[ShapeGuard]]
        ] = {}
    ...
    def create_fx_call_function(
        ...
        if self._translation_validation_enabled and node_key not in self.fx_node_cache:
            ...
            node = self.fx_node_cache[node_key] = self.graph.call_function(
                lifted_op, args
            )
    ...
    def _produce_dyn_sizes(
        ...
        return self._produce_dyn_sizes_from_int_tuple(
            tuple(ex.size()), source, symbolic_context
        )
    ...
    def _create_symbolic_sizes_strides_storage_offset(
        ...
        *,
        ...
        dynamic_strides_offset = (
            DimDynamic.STATIC
            if all(r == DimDynamic.STATIC for r in dynamic_dims)
    ...
    def create_unbacked_symint(self):
        symbol: sympy.Symbol = sympy.Symbol(
            f"i{next(self.unbacked_symint_counter)}", integer=True
        )
        self.counter["create_unbacked_symbol"] += 1
        self.var_to_stack[symbol] = CapturedTraceback.extract(skip=1)
        ...
        log.info(
            "create_unbacked_symbol %s [%s, %s]%s (%s)",
            symbol,
            vr.lower,
            vr.upper,
            maybe_user_loc,
    ...
    def create_symbol(
        ...
        if val in (0, 1) and specialize_zero_one:
            ...
            sympy_expr = sympy.Symbol(
                f"s{len(self.var_to_val)}", positive=positive, integer=True
            )
            ...
            if isinstance(val, int):
                ...
                self.var_to_val[sympy_expr] = SingletonInt(
                    val.node.singleton_int(), coeff=val.node.singleton_coeff()
                )
    ...
    def render_range_for_constraint_violation(self, source, c):
        if isinstance(c, StrictMinMaxConstraint):
            ...
            if upper >= default.upper:
                upper = None
            c_render = (
                f"{self.debug_name(source)} = {source.name()} in the specified range"
            )
    ...
    def produce_guards(
        ...
        *,
        ...
        constraint_inputs: Optional[
            InputList[Union[DimConstraint, Optional[DimList[DimConstraint]]]]
        ] = None,
        ...
        if constraint_inputs is None:
            constraint_inputs = [
                [None] * t.dim() if isinstance(t, Tensorlike) else None
            ...
            if isinstance(val, SymInt):
                ...
                if isinstance(s, sympy.Symbol):
                    ...
                    if constraint_violated:
                        ...
                        var_with_range = self.render_range_for_constraint_violation(
                            source, constraint
                        )
                        msg = (
                ...
                if constraint_violated:
                    ...
                    record_constraint_violation(
                        constraint.warn_only, self.debug_name(source), msg
                    )
        ...
        for t, source, constraint in zip(placeholders, sources, constraint_inputs):
            ...
            if is_traceable_wrapper_subclass(t):
                ...
                inner_sources_and_tensors = [
                    (AttrSource(source, attr), getattr(t, attr)) for attr in attrs
                ]
                if t.is_nested:
            ...
            for src, curr_t in sources_and_tensors:
                ...
                if not t.is_nested:
                    for i, ss in enumerate(curr_t.stride()):
                        track_symint(
                            TensorPropertySource(src, TensorProperty.STRIDE, i), ss
                        )
                    track_symint(
        ...
        if not _simplified:
            for source, expr in input_guards:
                ...
                if (
                    ...
                    record_constraint_violation(
                        equalities_inputs.warn_only, self.debug_name(source), msg
                    )
                ...
                if not is_trivial and len(expr.free_symbols) == 1:
                    ...
                    for c in constraints:
                        if isinstance(c, StrictMinMaxConstraint):
                            ...
                            record_constraint_violation(
                                c.warn_only, self.debug_name(source), msg
                            )
    ...
    def get_nontrivial_guards(self):
        return [
            ...
            for guard in self.guards
            if self._maybe_evaluate_static(guard.expr) is None
        ]
    ...
    def format_guards(self, verbose=False):
        ...
        return "\n".join(
            f" - {guard.expr}{format_tb(guard.stack)}" for guard in self.guards
        )
    ...
    def _maybe_evaluate_static(
        ...
        *,
        ...
        for idx, k in enumerate(symbols):
            ...
            # SymInt
            if vr.lower < (-sys.maxsize - 1) // 2 or (
    ...
    def _set_replacement(self, a: "sympy.Symbol", expr: "sympy.Expr") -> None:
        ...
        """
        if config.print_specializations and isinstance(
            expr, (sympy.Integer, sympy.Float)
        ):
            ...
            if a not in self.replacements or expr != self.replacements[a]:
                self.log.warning(
                    "Specializing %s to %s", self.var_to_sources[a][0].name(), expr
                )
    ...
    @lru_cache(256)
    def _maybe_guard_eq(
        self, expr: Union["sympy.Eq", "sympy.Ne"], concrete_bool: bool
    ) -> None:
        """
                ...
                if r is not None and r[1] == 0:
                    ...
                    if (
                        ...
                        if (
                            ...
                            self.runtime_var_to_range[i1] = (
                                SymPyValueRangeAnalysis.truediv(
    ...
    def evaluate_expr(
        self, orig_expr: "sympy.Expr", hint=None, fx_node=None, expect_rational=True
    ):
        """
        ...
        if (
            ...
            if concrete_val is sympy.true:
                ...
                eql, _ = self.create_fx_call_function(
        ...
        try:
            ...
            static_expr = self._maybe_evaluate_static(
                expr, expect_rational=expect_rational
            )
            ...
            if not (expr.free_symbols <= self.var_to_val.keys()):
                ...
                if not (new_expr.free_symbols <= self.var_to_val.keys()):
                    raise self._make_data_dependent_error(
                        expr.xreplace(self.var_to_val), expr
                    )
                expr = new_expr
    ...
    def defer_runtime_assert(self, orig_expr: "sympy.Expr", msg, fx_node=None):
        ...
        if not self._suppress_guards_tls():
            ...
            cands = sorted(
                [s for s in expr.free_symbols if s.name.startswith("i")],
