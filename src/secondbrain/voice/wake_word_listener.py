...
class WakeWordListener:
    ...
    def __init__(
        self, wake_words: List[str] = None, threshold: float = 0.5, timeout: int = 5
    ):
        """
            ...
            try:
                with self.microphone as source:
                    ...
                    audio = self.recognizer.listen(
                        source, timeout=self.timeout, phrase_time_limit=5
                    )

                    # Process audio in separate thread to avoid blocking
                    threading.Thread(
                        target=self._process_audio, args=(audio, callback), daemon=True
                    ).start()
