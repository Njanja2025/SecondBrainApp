...
MANIFEST_FILE = os.environ.get('MANIFEST_FILE', f'{ARTIFACT_DIR}/SecondBrain-2025-manifest.json')
...
def generate_signed_url(artifact, expires_in=300):
    ...
    sig = hmac.new(SIGNED_URL_SECRET.encode(), data.encode(), hashlib.sha256).hexdigest()
    return url_for('api_download_signed', artifact=artifact, expires=expires, sig=sig, _external=True)

def verify_signed_url(artifact, expires, sig):
    if int(expires) < int(time.time()):
        return False
    data = f"{artifact}:{expires}"
    expected = hmac.new(SIGNED_URL_SECRET.encode(), data.encode(), hashlib.sha256).hexdigest()
...
@app.route('/')
def index():
    return "<h1>SecondBrain Artifact Server</h1><ul>" + ''.join(f"<li><a href='/api/download/{f}'>{f}</a> ({download_counts.get(f,0)} downloads)</li>" for f in os.listdir(ARTIFACT_DIR) if not f.startswith('.')) + "</ul>"
