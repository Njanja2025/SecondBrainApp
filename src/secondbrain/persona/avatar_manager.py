...
class AvatarManager:
    ...
    def display_avatar_expression(self, image_path: str):
        ...
        try:
            ...
            print(
                f"Avatar expression changed to: {image_path}"
            )  # Fallback console output

    async def animate_expression_change(
        self, from_tone: str, to_tone: str, duration: float = 0.5
    ):
        """
            ...
            for frame in range(frames):
                ...
                current_state = {
                    "expression": self._interpolate_expression(
                        from_state["expression"], to_state["expression"], t
                    ),
                    "eye_state": self._interpolate_expression(
                        from_state["eye_state"], to_state["eye_state"], t
                    ),
                    "mouth_curve": from_state["mouth_curve"]
                    + (to_state["mouth_curve"] - from_state["mouth_curve"]) * t,
                }
        ...
        try:
            ...
            for expression in ["calm", "excited", "attentive", "neutral"]:
                ...
                if base_path.exists():
                    ...
                    self.animation_frames[f"{expression}_blink"] = ImageTk.PhotoImage(
                        blink_img
                    )

                    # Create speaking frames
                    speak_img = self._create_speak_frame(img)
                    self.animation_frames[f"{expression}_speak"] = ImageTk.PhotoImage(
                        speak_img
                    )
