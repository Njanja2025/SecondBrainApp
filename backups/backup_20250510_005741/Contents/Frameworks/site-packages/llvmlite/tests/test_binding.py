...
riscv_asm_ilp32d = ["fcvt.d.s\tft0, fa0", "fadd.d\tfa0, ft0, fa1", "ret"]
...
issue_632_text = "48c1e2204809c24889d048c1c03d4831d048b90120000480001070480fafc8"
...
class TestDependencies(BaseTest):
    ...
    @unittest.skipUnless(
        os.environ.get("LLVMLITE_DIST_TEST"), "Distribution-specific test"
    )
    def test_linux(self):
        ...
        env["LANG"] = "C"
        p = subprocess.Popen(
            ["objdump", "-p", lib_path], stdout=subprocess.PIPE, env=env
        )
        out, _ = p.communicate()
...
class TestRISCVABI(BaseTest):
    ...
    def break_up_asm(self, asm):
        ...
        for line in asm.splitlines():
            s_line = line.strip()
            if not (
                s_line.startswith(".") or s_line.startswith("fpadd") or s_line == ""
            ):
    ...
    def test_rv32d_ilp32(self):
        ...
        self.assertEqual(
            self.break_up_asm(target.emit_assembly(llmod)), riscv_asm_ilp32
        )
    ...
    def test_rv32d_ilp32f(self):
        ...
        self.assertEqual(
            self.break_up_asm(target.emit_assembly(llmod)), riscv_asm_ilp32f
        )
    ...
    def test_rv32d_ilp32d(self):
        ...
        self.assertEqual(
            self.break_up_asm(target.emit_assembly(llmod)), riscv_asm_ilp32d
        )
...
class TestModuleRef(BaseTest):
    ...
    def test_get_struct_type(self):
        ...
        self.assertIsNotNone(
            re.match(
                r"%struct\.glob_type(\.[\d]+)? = type { i64, \[2 x i64\] }", str(st_ty)
            )
        )
    ...
    def test_global_variables(self):
        ...
        self.assertEqual(
            [g.name for g in globs], ["glob", "glob_b", "glob_f", "glob_struct"]
        )
    ...
    def test_structs(self):
        ...
        self.assertIsNotNone(re.match(r"struct\.glob_type(\.[\d]+)?", structs[0].name))
        self.assertIsNotNone(
            re.match(
    ...
    def test_as_bitcode(self):
        ...
        bitcode_magic = b"BC"
        self.assertTrue(
            bc.startswith(bitcode_magic) or bc.startswith(bitcode_wrapper_magic)
        )
...
class TestOrcLLJIT(BaseTest):
    ...
    def jit(
        ...
        lljit = llvm.create_lljit_compiler(
            target_machine, use_jit_link=False, suppress_errors=suppress_errors
        )
        builder = llvm.JITLibraryBuilder()
        if add_process:
            builder.add_current_process()
        rt = (
            builder.add_ir(asm.format(triple=llvm.get_default_triple()))
    ...
    def test_global_ctors_dtors(self):
        ...
        builder = llvm.JITLibraryBuilder()
        rt = (
            builder.add_ir(asm_ext_ctors.format(triple=llvm.get_default_triple()))
...
class TestValueRef(BaseTest):
    ...
    def test_type_name(self):
        ...
        self.assertIsNotNone(
            re.match(r"struct\.glob_type(\.[\d]+)?", st.global_value_type.name)
        )
    ...
    def test_type_printing_struct(self):
        ...
        if opaque_pointers_enabled:
            ...
            self.assertIsNotNone(
                re.match(r"%struct\.glob_type(\.[\d]+)?\*", str(st.type))
            )
        self.assertIsNotNone(
            re.match(
                r"%struct\.glob_type(\.[\d]+)? = type { i64, \[2 x i64\] }",
    ...
    def test_value_kind(self):
        mod = self.module()
        self.assertEqual(
            mod.get_global_variable("glob").value_kind, llvm.ValueKind.global_variable
        )
        func = mod.get_function("sum")
        ...
        self.assertEqual(
            list(iasm_inst.operands)[0].value_kind, llvm.ValueKind.inline_asm
        )
...
class TestTypeRef(BaseTest):
    ...
    def test_str(self):
        ...
        self.assertEqual(
            str(glob_struct_type), "%struct.glob_type = type { i64, [2 x i64] }"
        )
    ...
    def test_vararg_function(self):
        ...
        if opaque_pointers_enabled:
            ...
            self.assertIn(
                "Type i32 (i32, i32)* is not a function", str(raises.exception)
            )
    ...
    def test_void_typeref_as_ir(self):
        ...
        self.assertEqual(fnty.return_type, gvty.as_ir(ir.global_context).return_type)
    ...
    def test_global_typeref_as_ir(self):
        ...
        def maker_struct():
            ...
            yield ir.LiteralStructType([ir.FloatType(), ir.IntType(64)], packed=True)

        makers["struct"] = maker_struct

        # Ensure that number of supported TypeKind matches number of makers
        self.assertEqual(
            {x.name for x in _TypeKindToIRType.keys()},
    ...
    def _check_typeref_as_ir_for_wrappers(self, asm, target_symbol):
        ...
        self.assertEqual(
            new_mod.get_function(declfn.name).global_value_type,
...
class TestTypeParsing(BaseTest):
    ...
    def test_literal_struct(self):
        ...
        with self.check_parsing() as mod:
            const = ir.Constant.literal_struct(
                [ir.IntType(32)(1), ir.IntType(32)(2)], packed=True
            )
