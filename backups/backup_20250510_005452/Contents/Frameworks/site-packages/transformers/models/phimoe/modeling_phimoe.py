...
def sparsemixer(scores, jitter_eps, training, top_k=2):
    ...
    if training:
        ...
        mask_for_one = torch.add(0.3333, mask_for_one, alpha=0.6667).type_as(
            masked_gates
        )
    ...
    if training:
        ...
        # 1 -> 1.0 & 0 -> 1./3: lambda x: (x + 0.5) / 1.5
        mask_for_one_top2 = torch.add(0.3333, mask_for_one_top2, alpha=0.6667).type_as(
            masked_gates_top2
        )
...
class PhimoeDecoderLayer(nn.Module):
    def __init__(self, config: PhimoeConfig, layer_idx: int):
        ...
        self.self_attn = PHIMOE_ATTENTION_CLASSES[config._attn_implementation](
            config, layer_idx
        )
...
class PhimoeModel(PhimoePreTrainedModel):
    ...
    def forward(
        ...
        position_embeddings = self.rotary_emb(
            hidden_states, seq_len=cache_position[-1] + 1
        )
...
class PhimoeForCausalLM(PhimoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.lm_head = nn.Linear(
            config.hidden_size, config.vocab_size, bias=self.config.lm_head_bias
        )
