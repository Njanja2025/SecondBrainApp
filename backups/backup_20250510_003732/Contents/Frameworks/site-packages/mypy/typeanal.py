...
class TypeAnalyser(SyntheticTypeVisitor[Type], TypeAnalyzerPluginInterface):
        ...
        if sym is not None:
            ...
            if isinstance(sym.node, ParamSpecExpr):
                ...
                if param_spec_name is not None and not self.allow_param_spec_literals:
                    self.fail(
                        "ParamSpec components are not allowed here",
                        t,
                        code=codes.VALID_TYPE,
                    )
                    return AnyType(TypeOfAny.from_error)
            ...
            if (
                ...
                if self.not_declared_in_type_params(t.name):
                    if self.python_3_12_type_alias:
                        msg = (
                            message_registry.TYPE_PARAMETERS_SHOULD_BE_DECLARED.format(
                                f'"{t.name}"'
                            )
                        )
                    else:
    ...
    def try_analyze_special_unbound_type(
        ...
        if fullname == "builtins.None":
            ...
            if len(t.args) != 1:
                self.fail(
                    f"{type_str} must have exactly one type argument",
                    t,
                    code=codes.VALID_TYPE,
                )
            item = self.anal_type(t.args[0])
            ...
            if bad_item_name:
                self.fail(
                    f'{type_str} can\'t contain "{bad_item_name}"',
                    t,
                    code=codes.VALID_TYPE,
                )
            ...
            return self.anal_type(
                t.args[0],
                allow_typed_dict_special_forms=self.allow_typed_dict_special_forms,
            )
            ...
            if len(t.args) != 1:
                ...
                return AnyType(TypeOfAny.from_error)
            return RequiredType(
                self.anal_type(t.args[0], allow_typed_dict_special_forms=True),
                required=True,
            )
        ...
        if (
            ...
            for imp in self.cur_mod_node.imports:
                if isinstance(imp, ImportFrom) and any(
                    info.name == n[0] for n in imp.names
                ):
                    break
            else:
                warn = (
                    self.note if self.options.report_deprecated_as_note else self.fail
                )
    ...
    def visit_callable_type(
        ...
        with self.tvar_scope_frame(namespace=namespace):
            ...
            for kind, ut in zip(arg_kinds, t.arg_types):
                if kind == ARG_STAR:
                    param_spec_with_args, at = self.anal_star_arg_type(
                        ut, kind, nested=nested
                    )
                elif kind == ARG_STAR2:
                    param_spec_with_kwargs, at = self.anal_star_arg_type(
                        ut, kind, nested=nested
                    )
                else:
                    if param_spec_with_args:
                        ...
                        self.fail(
                            "Arguments not allowed after ParamSpec.args",
                            t,
                            code=codes.VALID_TYPE,
                        )
            ...
            if (
                not param_spec_invalid
                and param_spec_with_args != param_spec_with_kwargs
            ):
            ...
            if param_spec_invalid:
                ...
                if ARG_STAR2 in arg_kinds:
                    arg_types[arg_kinds.index(ARG_STAR2)] = AnyType(
    ...
    def anal_star_arg_type(
        self, t: Type, kind: ArgKind, nested: bool
    ) -> tuple[str | None, Type]:
    ...
    def visit_typeddict_type(self, t: TypedDictType) -> Type:
            ...
            for typ in t.extra_items_from:
                ...
                if not isinstance(p_analyzed, TypedDictType):
                    if not isinstance(p_analyzed, (AnyType, PlaceholderType)):
                        self.fail(
                            "Can only merge-in other TypedDict",
                            t,
                            code=codes.VALID_TYPE,
                        )
                    continue
        ...
        return TypedDictType(
            items, required_keys, readonly_keys, fallback, t.line, t.column
        )
    ...
    def visit_union_type(self, t: UnionType) -> Type:
        ...
        return UnionType(
            self.anal_array(t.items), t.line, uses_pep604_syntax=t.uses_pep604_syntax
        )
        ...
        *,
        ...
        if (
            ...
            if analyzed.prefix.arg_types:
                ...
                self.fail(
                    INVALID_PARAM_SPEC_LOCATION.format(
                        format_type(analyzed, self.options)
                    ),
                    t,
                    code=codes.VALID_TYPE,
                )
...
def make_optional_type(t: Type) -> Type:
    ...
    if isinstance(t, ProperType) and isinstance(t, NoneType):
        ...
        items = [
            item for item in t.items if not isinstance(get_proper_type(item), NoneType)
        ]
