...
class ExpressionChecker(ExpressionVisitor[Type]):
        ...
        if (
            ...
            for typ in mypy.checker.flatten(e.args[1]):
                ...
                if (
                    ...
                    and not (
                        isinstance(
                            union_target := get_proper_type(node.node.target), UnionType
                        )
    ...
    def check_op_reversible(
        ...
        variants = [
            (na, op, obj, arg) for (na, op, obj, arg) in variants_raw if op is not None
        ]
    ...
    def visit_index_with_type(
        ...
        if isinstance(left_type, TypeVarType) and not self.has_member(
            ...
            and left_type.type.fullname == "typing._SpecialForm"
        ):
        ...
        if isinstance(tp, Overloaded):
            for it in tp.items:
                ...
                has_type_var_tuple = any(
                    isinstance(v, TypeVarTupleType) for v in type_vars
                )
                if (
                    len(args) < min_arg_count or len(args) > len(type_vars)
                ) and not has_type_var_tuple:
    ...
    def visit_conditional_expr(
        ...
        if has_uninhabited_component(res) and not isinstance(
            ...
            if (
                not isinstance(p_alt, Instance)
                or p_alt.type.fullname != "builtins.object"
            ):
                res = alternative
        return res
    ...
    def is_valid_var_arg(self, typ: Type) -> bool:
        ...
        typ = get_proper_type(typ)
        return isinstance(
            typ, (TupleType, AnyType, ParamSpecType, UnpackType)
        ) or is_subtype(
            typ,
            self.chk.named_generic_type(
