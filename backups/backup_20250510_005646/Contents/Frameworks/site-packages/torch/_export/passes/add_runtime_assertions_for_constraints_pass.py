...
class _AddRuntimeAssertionsForInlineConstraintsPass(_ExportPassBase):
    def __init__(
        ...
        self.equality_constraints: List[Tuple[InputDim, InputDim]] = (
    ...
    def _insert_assert_async(self, operator, lower, upper, assert_msg):
        ...
        """
        self.counter += 1
        cmp = super().call_operator(
            operator, (lower, upper), {}, self._create_dummy_node_metadata()
        )
        cmp_tensor = super().call_operator(
        ...
        def add_assertions(val):
            ...
            if isinstance(val, (torch.SymInt, torch.SymFloat, torch.SymBool)):
                ...
                if isinstance(symbol, sympy.Symbol) and symbol.name.startswith("i"):
                    ...
                    assert_msg = (
                        f" is outside of inline constraint [{min_val}, {max_val}]."
                    )
                    call_backs.append(
                        partial(
                            self._assert_range_constraint, lower=min_val, upper=max_val
                        )
                    )
                ...
                for i, sym in enumerate(val.shape):
                    ...
                    for cb, msg in zip(cbs, msgs):
                        ...
                        def sym_size_cb(proxy, assert_msg, dim):
                            dim_proxy = super(
                                _AddRuntimeAssertionsForInlineConstraintsPass, self
                            ).call_operator(
...
class _AddRuntimeAssertionsForConstraintsPass(
    ...
    def _insert_specialized_shape_assert_inplace(
        ...
        with graph.inserting_after(eq_node):
            tensor_eq_node = graph.call_function(
                torch.ops.aten.scalar_tensor.default, (eq_node,)
            )
        with graph.inserting_after(tensor_eq_node):
            _ = graph.call_function(
                torch.ops.aten._assert_async.msg, (tensor_eq_node, assert_msg)
            )
    ...
    def _insert_equality_assert_inplace(
        ...
        for input_dim, other_input_dim in self.equality_constraints:
            ...
            self._insert_assert_async_inplace(
                graph, operator.eq, (dim_node, other_dim_node), assert_msg
            )
