...
class TestTimedeltaArithmetic(TestCase):
    ...
    def test_hash(self):
        ...
        DT_CASES = (
            ...
            ("2014-02-16",),
            ("2014-03-01",),
            ("2000-01-01T01:02:03.002Z",),
            ("2000-01-01T01:02:03Z",),
            ("NaT",),
        )

        for case, typ in zip(
            TD_CASES + DT_CASES, (TD,) * len(TD_CASES) + (DT,) * len(TD_CASES)
        ):
            check(typ(*case))
...
class TestTimedeltaArithmeticNoPython(TestTimedeltaArithmetic):
    ...
    def test_int_cast(self):
        ...
        with self.assertRaises(
            TypingError,
            msg=(
                "Only datetime64[ns] can be " + "converted, but got " + "datetime64[y]"
            ),
        ):
            f(DT("2014"))
...
class TestDatetimeArrayOps(TestCase):
    ...
    def _test_add_sub_td_no_match(self, operation):
        ...
        expected = re.escape(
            (
                f"ufunc '{fname}' is not supported between "
