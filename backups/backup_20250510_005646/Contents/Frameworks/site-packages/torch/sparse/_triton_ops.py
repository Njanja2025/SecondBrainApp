...
def bsr_dense_mm_meta(
    M, K, N, Ms, Ks, GROUP_SIZE_ROW=None, num_warps=None, num_stages=None, **extra
):
...
def bsr_dense_addmm_meta(
    ...
    **extra,
    ...
    if {SPLIT_N, num_warps, num_stages, GROUP_SIZE_ROW} == {None}:
        ...
        meta = get_meta(
            "bsr_dense_addmm",
            (M, K, N, Ms, Ks, beta == 0, beta == 1, alpha == 1),
            device_name,
            version=(0, dtype, 0.5),
        )
        if meta is not None:
    ...
    *,
    ...
    if meta is None:
        meta = bsr_dense_addmm_meta(
            M, K, N, blocksize[0], blocksize[1], beta, alpha, dtype=out.dtype
        )
...
if has_triton():
    ...
    def bsr_dense_mm(
        ...
        *,
        ...
        if (
            enable_bsr_scatter_mm
            and max(blocksize) == 16
            and bsr.dense_dim() == 0
            and bsr.ndim == 2
        ):
            dtype = bsr.dtype
            ...
            if (
                ...
                dtype == torch.float32
                and (m >= 1024 or (m == 512 and n >= 512) or (m == 256 and n >= 2048))
            ):
        ...
        _run_dense_rowspace_kernel(
            blocksize, values, crow_indices, col_indices, dense, out, max_grid, meta
        )
