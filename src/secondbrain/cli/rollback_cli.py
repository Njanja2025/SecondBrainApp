...
def list_backups(args):
    ...
    for backup in backups:
        timestamp = datetime.strptime(backup["timestamp"], "%Y%m%d_%H%M%S").strftime(
            "%Y-%m-%d %H:%M:%S"
        )
...
def main():
    ...
    create_parser.add_argument("--notes", help="Notes for the backup")
    create_parser.set_defaults(func=create_backup)

    # Restore backup command
    restore_parser = subparsers.add_parser("restore", help="Restore a backup version")
    ...
    cleanup_parser.add_argument("--keep", type=int, help="Number of backups to keep")
