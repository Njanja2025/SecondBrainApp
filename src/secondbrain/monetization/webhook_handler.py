...
def log_stripe_event(event_type, customer_email=None, note=""):
    ...
    with open("logs/stripe_payments.log", "a") as log_file:
        ...
        log_file.write(
            f"[{timestamp}] {event_type} - {customer_email or 'unknown'} {note}\n"
        )
...
def stripe_webhook():
    ...
    try:
        ...
        if event["type"] == "checkout.session.completed":
            ...
            # Log event
            log_stripe_event(
                "subscription.deleted", email, f"Subscription ID: {subscription.id}"
            )
            ...
            # Log event
            log_stripe_event(
                "subscription.updated", email, f"New status: {subscription.status}"
            )
