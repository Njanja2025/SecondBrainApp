...
def _labeled_extract(
    salt: bytes, label: bytes, ikm: bytes, suite_id: bytes, hashmod: ModuleType
):
...
def _labeled_expand(
    prk: bytes, label: bytes, info: bytes, L: int, suite_id: bytes, hashmod: ModuleType
):
    labeled_info = struct.pack(">H", L) + b"HPKE-v1" + suite_id + label + info
    return _HKDF_expand(prk, labeled_info, L, hashmod)


def _extract_and_expand(
    dh: bytes, kem_context: bytes, suite_id: bytes, hashmod: ModuleType
):
    ...
    shared_secret = _labeled_expand(
        eae_prk, b"shared_secret", kem_context, Nsecret, suite_id, hashmod
    )
...
class HPKE_Cipher:
    ...
    def __init__(
        ...
        try:
            ...
            raise ValueError(
                "Curve {} is not supported by HPKE".format(self._curve)
            ) from ke
        ...
        if self._encrypt:
            ...
            shared_secret, self.enc = self._encap(
                receiver_key, self._kem_id, self._hashmod, sender_key
            )
        else:
            # SetupBaseR (decryption)
            if enc is None:
                raise ValueError("Parameter 'enc' required when unsealing")
            shared_secret = self._decap(
                enc, receiver_key, self._kem_id, self._hashmod, sender_key
            )

        self._sequence = 0
        self._max_sequence = (1 << (8 * self._Nn)) - 1

        self._key, self._base_nonce, self._export_secret = self._key_schedule(
            shared_secret, info, *psk_pair
        )
    ...
    def _encap(
        ...
        shared_secret = key_agreement(
            eph_priv=eph_key, static_pub=receiver_key, kdf=kdf, **extra_param
        )
    ...
    def _decap(
        ...
        shared_secret = key_agreement(
            eph_pub=pkE, static_priv=receiver_key, kdf=kdf, **extra_param
        )
        return shared_secret
    ...
    def _key_schedule(
        self, shared_secret: bytes, info: bytes, psk_id: bytes, psk: bytes
    ):

        suite_id = b"HPKE" + struct.pack(
            ">HHH", self._kem_id, self._kdf_id, self._aead_id
        )

        psk_id_hash = _labeled_extract(
            b"", b"psk_id_hash", psk_id, suite_id, self._hashmod
        )
        ...
        secret = _labeled_extract(
            shared_secret, b"secret", psk, suite_id, self._hashmod
        )

        key = _labeled_expand(
            secret, b"key", key_schedule_context, self._Nk, suite_id, self._hashmod
        )
    ...
    def unseal(self, ciphertext: bytes, auth_data: Optional[bytes] = None):
        ...
        try:
            pt = cipher.decrypt_and_verify(
                ciphertext[: -self._Nt], ciphertext[-self._Nt :]
            )
        except ValueError:
            if self._sequence == 1:
                raise ValueError(
                    "Incorrect HPKE keys/parameters or invalid message (wrong MAC tag)"
                )
...
def new(
    ...
    * a shared secret (e.g., a symmetric key derived from a password)
    ...
    if sender_key:
        count_private_keys = int(receiver_key.has_private()) + int(
            sender_key.has_private()
        )
        if count_private_keys != 1:
            raise ValueError("Exactly 1 private key required")
        if sender_key.curve != curve:
            raise ValueError(
                "Sender key uses {} but recipient key {}".format(
                    sender_key.curve, curve
                )
            )
