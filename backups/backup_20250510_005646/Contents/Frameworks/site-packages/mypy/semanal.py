...
SCOPE_ANNOTATION: Final = (
    4  # Annotation scopes for type parameters and aliases (PEP 695)
)
...
class SemanticAnalyzer(
        ...
        with scope.module_scope(self.cur_mod_id):
            ...
            if active_type:
                ...
                if "." in enclosing_fullname:
                    enclosing_node = self.lookup_fully_qualified_or_none(
                        enclosing_fullname
                    )
    ...
    def update_function_type_variables(
        ...
        with self.tvar_scope_frame(self.tvar_scope.method_frame(fullname)):
            ...
            if defn.type_args:
                ...
                declared_fullnames = {
                    self.qualified_name(p.name) for p in defn.type_args
                }
            ...
            if isinstance(item, Decorator):
                for d in item.decorators:
                    if deprecation and refers_to_fullname(d, OVERLOAD_NAMES):
                        self.msg.note(
                            "@overload should be placed before @deprecated", d
                        )
            ...
            if type_param.values:
                for value in type_param.values:
                    ...
                    if has_type_vars(analyzed):
                        self.fail(
                            message_registry.TYPE_VAR_GENERIC_CONSTRAINT_TYPE, context
                        )
    ...
    def check_type_alias_bases(self, bases: list[Expression]) -> None:
        for base in bases:
            ...
            if (
                ...
                self.fail(
                    'Type alias defined using "type" statement not valid as base class',
                    base,
                )
    ...
    def clean_up_bases_and_infer_type_variables(
            ...
            if result is not None:
                ...
                if declared_tvars:
                    if defn.type_args:
                        if is_protocol:
                            self.fail(
                                'No arguments expected for "Protocol" base class',
                                context,
                            )
                        else:
        ...
        if declared_tvars:
            if (
                len(remove_dups(declared_tvars)) < len(declared_tvars)
                and not defn.type_args
            ):
                self.fail(
    ...
    def analyze_unbound_tvar_impl(
        ...
        is_typealias_param: bool = False,
    ) -> tuple[str, TypeVarLikeExpr] | None:
        assert (
            not is_unpacked or not is_typealias_param
        ...
        if (
            (is_unpacked or is_typealias_param)
            and sym
            and isinstance(sym.node, TypeVarTupleExpr)
        ):
            if sym.fullname and not self.tvar_scope.allow_binding(sym.fullname):
                # It's bound by our type variable scope
                return None
    ...
    def can_possibly_be_typevarlike_declaration(self, s: AssignmentStmt) -> bool:
        ...
        if len(s.lvalues) != 1 or not isinstance(s.lvalues[0], NameExpr):
            return False
        if not isinstance(s.rvalue, CallExpr) or not isinstance(
    ...
    def analyze_alias(
        ...
        namespace = self.qualified_name(name)
        alias_type_vars = (
            found_type_vars if declared_type_vars is None else declared_type_vars
        )
        with self.tvar_scope_frame(self.tvar_scope.class_frame(namespace)):
    ...
    def check_and_set_up_type_alias(self, s: AssignmentStmt) -> bool:
        ...
        if self.check_type_alias_type_call(s.rvalue, name=lvalue.name):
            ...
            pep_695 = True
            type_params, all_type_params_names = self.analyze_type_alias_type_params(
                s.rvalue
            )
        else:
            rvalue = s.rvalue
        ...
        if (
            not pep_613
            and not pep_695
            and isinstance(rvalue, RefExpr)
            and non_global_scope
        ):
        ...
        if self.is_none_alias(rvalue):
            ...
            tag = self.track_incomplete_refs()
            res, alias_tvars, depends_on, qualified_tvars, empty_tuple_index = (
                self.analyze_alias(
                    lvalue.name,
                    rvalue,
                    allow_placeholder=True,
    ...
    def check_type_alias_type_call(
        self, rvalue: Expression, *, name: str
    ) -> TypeGuard[CallExpr]:
        ...
        for tp_expr in type_params:
            ...
            if tvar:
                ...
                if sym and isinstance(sym.node, TypeVarLikeExpr):
                    all_declared_tvar_names.append(
                        sym.node.name
                    ...
                    if sym and sym.fullname in (
                        "typing.Unpack",
                        "typing_extensions.Unpack",
                    ):
                        self.note(
                            "Don't Unpack type variables in type_params",
                            base,
                            code=codes.TYPE_VAR,
                        )
                continue
    ...
    def check_typevar_default(self, default: Type, context: Context) -> Type:
        ...
        if isinstance(typ, AnyType) and typ.is_from_error:
            self.fail(
                message_registry.TYPEVAR_ARG_MUST_BE_TYPE.format("TypeVar", "default"),
                context,
            )
        return default
        ...
        if isinstance(typ, Parameters):
            for i, arg_type in enumerate(typ.arg_types):
                ...
                if isinstance(arg_ptype, AnyType) and arg_ptype.is_from_error:
                    self.fail(
                        f"Argument {i} of ParamSpec default must be a type", context
                    )
        elif (
    ...
    def visit_for_stmt(self, s: ForStmt) -> None:
        ...
        self.analyze_lvalue(
            s.index, explicit_type=s.index_type is not None, is_index_var=True
        )
        if s.index_type:
    ...
    def analyze_try_stmt(self, s: TryStmt, visitor: NodeVisitor[None]) -> None:
        ...
        for type, var, handler in zip(s.types, s.vars, s.handlers):
            ...
            with self.inside_except_star_block_set(
                self.inside_except_star_block or s.is_star
            ):
                ...
                for table, scope_type in zip(
                    ...
                    if table is not None and name in table:
                        if scope_type == SCOPE_ANNOTATION:
                            self.fail(
                                f'nonlocal binding not allowed for type parameter "{name}"',
                                d,
                            )
                        break
                else:
    ...
    def analyze_type_application_args(self, expr: IndexExpr) -> list[Type] | None:
        ...
        if has_param_spec and num_args == 1 and types:
            ...
            if not (
                single_any
                or any(isinstance(t, (Parameters, ParamSpecType)) for t in types)
            ):
    ...
    def enter(
        ...
        self.scope_stack.append(
            SCOPE_FUNC if not is_comprehension else SCOPE_COMPREHENSION
        )
