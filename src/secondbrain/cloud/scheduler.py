...
class BackupScheduler:
    ...
    def _load_config(self) -> Dict:
        ...
        try:
            ...
            return {
                "backup_schedule": {
                    "memory": {"frequency": "weekly", "day": "sunday", "time": "00:00"},
                    "voice_logs": {"frequency": "daily", "time": "23:00"},
                    "emotional_logs": {"frequency": "daily", "time": "23:30"},
                    "gui_logs": {"frequency": "daily", "time": "23:45"},
                },
    ...
    async def start(self):
        ...
        try:
            ...
            self.tasks.append(asyncio.create_task(self._run_dns_checks()))
        ...
        try:
            ...
            if os.getenv("DROPBOX_ACCESS_TOKEN"):
                await self.cloud_sync.upload_to_dropbox(
                    str(memory_path), os.getenv("DROPBOX_ACCESS_TOKEN")
                )
    ...
    async def _backup_emotional_logs(self):
        ...
        try:
            ...
            if not emotional_logs_dir.exists():
                logger.warning(
                    f"Emotional logs directory not found: {emotional_logs_dir}"
                )
                return
            ...
            for domain_config in self.dns_manager.config.get("domains", []):
                ...
                for subdomain in domain_config["subdomains"]:
                    ...
                    if not await self.dns_manager.verify_dns(
                        domain, subdomain, current_ip
                    ):
                        # Update if verification fails
                        await self.dns_manager.update_namecheap_dns(
                            api_user, api_key, domain, subdomain, current_ip
                        )
