    ...
    async def _handle_failure(self, exception: Exception):
        ...
        async with self._lock:
            ...
            if (
                ...
                self.state = CircuitState.OPEN
                logger.warning(
                    "Circuit breaker returned to OPEN state after failed recovery attempt"
                )
            ...
            async with asyncio.timeout(self.timeout):
                async with self.semaphore:
                    ...
                    if (
                        ...
                        > self.metrics["max_concurrent_reached"]
                    ):
                        self.metrics["max_concurrent_reached"] = self.metrics[
                            "current_concurrent"
                        ]
