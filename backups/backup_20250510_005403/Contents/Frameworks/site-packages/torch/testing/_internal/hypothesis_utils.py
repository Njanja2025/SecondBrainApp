...
def _floats_wrapper(*args, **kwargs):
    if "width" in kwargs and hypothesis.version.__version_info__ < (3, 67, 0):
        ...
        no_nan_and_inf = (
            ("allow_nan" in kwargs and not kwargs["allow_nan"])
            or "allow_nan" not in kwargs
        ) and (
            ("allow_infinity" in kwargs and not kwargs["allow_infinity"])
            or "allow_infinity" not in kwargs
        )
        min_and_max_not_specified = (
            len(args) == 0 and "min_value" not in kwargs and "max_value" not in kwargs
        )
...
@st.composite
def array_shapes(
    draw, min_dims=1, max_dims=None, min_side=1, max_side=None, max_numel=None
):
