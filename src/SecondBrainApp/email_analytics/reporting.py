...
class EmailReportManager:
    ...
    def generate_report(self, data: Dict[str, Any], template: ReportTemplate,
                       output_path: str) -> str:
            ...
            for chart_config in template.charts:
                if chart_config.type == ChartType.LINE:
                    fig = self.visualizer.create_time_series(
                        data.get(chart_config.name, pd.DataFrame()),
                        chart_config
                    )
                    ...
                    fig = self.visualizer.create_pie_chart(
                        data.get(chart_config.name, pd.Series()),
                        chart_config
                    )
                ...
                if fig:
                    chart_path = self.visualizer.save_chart(
                        fig,
                        f"{template.name}_{chart_config.name}",
                        format='png'
                    )
    ...
    def generate_summary(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            summary["overview"] = {
                ...
                "total_threads": len(set(e.get("thread_id") for e in data.get("emails", []))),
                "date_range": {
    ...
    def generate_dashboard(self, data: Dict[str, Any], template: ReportTemplate,
                         output_path: str) -> str:
        ...
        try:
            ...
            # Update layout
            fig.update_layout(
                title_text="Email Analytics Dashboard",
                height=800,
                showlegend=True
            )
