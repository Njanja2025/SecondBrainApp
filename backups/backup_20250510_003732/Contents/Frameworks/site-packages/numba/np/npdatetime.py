...
def timedelta_times_number(context, builder, sig, args):
    res = _timedelta_times_number(
        context, builder, args[0], sig.args[0], args[1], sig.args[1], sig.return_type
    )
    return impl_ret_untracked(context, builder, sig.return_type, res)
...
def number_times_timedelta(context, builder, sig, args):
    res = _timedelta_times_number(
        context, builder, args[1], sig.args[1], args[0], sig.args[0], sig.return_type
    )
    return impl_ret_untracked(context, builder, sig.return_type, res)
...
timedelta_eq_timedelta_impl = _create_timedelta_comparison_impl("==", cgutils.false_bit)
timedelta_ne_timedelta_impl = _create_timedelta_comparison_impl("!=", cgutils.true_bit)
...
lower_builtin(npdatetime_helpers.datetime_minimum, types.NPDatetime, types.NPDatetime)(
    datetime_minimum_impl
)
lower_builtin(
    npdatetime_helpers.datetime_minimum, types.NPTimedelta, types.NPTimedelta
)(datetime_minimum_impl)
lower_builtin(npdatetime_helpers.datetime_maximum, types.NPDatetime, types.NPDatetime)(
