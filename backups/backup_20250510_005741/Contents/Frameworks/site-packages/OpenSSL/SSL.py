...
OP_NETSCAPE_REUSE_CIPHER_CHANGE_BUG: int = _lib.SSL_OP_NETSCAPE_REUSE_CIPHER_CHANGE_BUG
...
OP_NETSCAPE_DEMO_CIPHER_CHANGE_BUG: int = _lib.SSL_OP_NETSCAPE_DEMO_CIPHER_CHANGE_BUG
...
def _asFileDescriptor(obj: Any) -> int:
    ...
    if not isinstance(fd, int):
        ...
        raise ValueError(f"file descriptor cannot be a negative integer ({fd:i})")
...
class Context:
    ...
    _methods: typing.ClassVar[dict[int, tuple[Callable[[], Any], int | None]]] = {
    ...
    def __init__(self, method: int) -> None:
        ...
        self._cookie_generate_helper: _CookieGenerateCallbackHelper | None = None
    ...
    def _wrap_callback(self, callback: _PassphraseCallback[_T]) -> _PassphraseHelper:
        @wraps(callback)
        def wrapper(size: int, verify: bool, userdata: Any) -> bytes:
            return callback(size, verify, self._passphrase_userdata)

        return _PassphraseHelper(FILETYPE_PEM, wrapper, more_args=True, truncate=True)
    ...
    def set_default_verify_paths(self) -> None:
        ...
        *   manylinux cryptography wheels will work on most common Linux
        ...
        file_env_var = _ffi.string(_lib.X509_get_default_cert_file_env()).decode(
            "ascii"
        )
    ...
    def set_session_id(self, buf: bytes) -> None:
        ...
        _openssl_assert(
            _lib.SSL_CTX_set_session_id_context(self._context, buf, len(buf)) == 1
        )
    ...
    def set_verify(self, mode: int, callback: _VerifyCallback | None = None) -> None:
        """
    ...
    def set_client_ca_list(self, certificate_authorities: Sequence[X509Name]) -> None:
        """
    ...
    def add_client_ca(self, certificate_authority: X509 | x509.Certificate) -> None:
        """
        ...
        if not isinstance(certificate_authority, X509):
            certificate_authority = X509.from_cryptography(certificate_authority)
        else:
            warnings.warn(
                (
                    "Passing pyOpenSSL X509 objects is deprecated. You "
    ...
    def set_cookie_generate_callback(self, callback: _CookieGenerateCallback) -> None:
    ...
    def set_cookie_verify_callback(self, callback: _CookieVerifyCallback) -> None:
...
class Connection:
    _reverse_mapping: typing.MutableMapping[Any, Connection] = WeakValueDictionary()
