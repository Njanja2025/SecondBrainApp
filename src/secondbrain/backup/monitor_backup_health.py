...
class BackupHealthMonitor:
    ...
    def check_backup_health(self, days=7):
        ...
        try:
            ...
            self.record_backup_health(backup_file, {
    ...
    def send_health_report(self, report):
        ...
        try:
            ...
            msg = MIMEMultipart()
            msg['Subject'] = f"SecondBrain Backup Health Report - {datetime.now().strftime('%Y-%m-%d')}"
            ...
            # Send email
            with smtplib.SMTP(self.cloud_config.get('smtp_server', 'smtp.gmail.com'), 
                            self.cloud_config.get('smtp_port', 587)) as server:
                server.starttls()
                server.login(
