...
class TestUFuncs(BasicUFuncTest, TestCase):
    ...
    def test_negative_ufunc(self):
        ...
        self.basic_ufunc_test(
            np.negative, int_output_type=types.int64, skip_inputs=skip_inputs
        )
    ...
    def bool_additional_inputs(self):
        return [
            (np.array([True, False], dtype=np.bool_), types.Array(types.bool_, 1, "C")),
        ]
    ...
    def binary_ufunc_mixed_types_test(self, ufunc):
        ...
        for vals in itertools.product(inputs1, inputs2, output_types):
            ...
            if ufunc_name == "divide" and (
                input2_type == types.Array(types.uint32, 1, "C")
                or input2_type == types.Array(types.uint64, 1, "C")
            ):
                continue
            ...
            if (
                isinstance(input1_type, types.Array)
                or isinstance(input2_type, types.Array)
            ) and not isinstance(output_type, types.Array):
                continue
...
class TestArrayOperators(BaseUFuncTest, TestCase):
    ...
    def inplace_op_test(
        self, operator, lhs_values, rhs_values, lhs_dtypes, rhs_dtypes, precise=True
    ):
    ...
    def inplace_bitwise_op_test(self, operator, lhs_values, rhs_values):
        ...
        self.inplace_op_test(
            operator, lhs_values, rhs_values, (np.bool_,), (np.bool_, np.bool_)
        )
    ...
    def test_inplace_pow(self):
        ...
        self.inplace_float_op_test(
            operator.ipow, [-1, 1.5, 3], [-5, 2, 2.5], precise=False
        )

    def test_inplace_and(self):
        self.inplace_bitwise_op_test("&=", [0, 1, 2, 3, 51], [0, 13, 16, 42, 255])
        self.inplace_bitwise_op_test(
            operator.iand, [0, 1, 2, 3, 51], [0, 13, 16, 42, 255]
        )

    def test_inplace_or(self):
        self.inplace_bitwise_op_test("|=", [0, 1, 2, 3, 51], [0, 13, 16, 42, 255])
        self.inplace_bitwise_op_test(
            operator.ior, [0, 1, 2, 3, 51], [0, 13, 16, 42, 255]
        )

    def test_inplace_xor(self):
        self.inplace_bitwise_op_test("^=", [0, 1, 2, 3, 51], [0, 13, 16, 42, 255])
        self.inplace_bitwise_op_test(
            operator.ixor, [0, 1, 2, 3, 51], [0, 13, 16, 42, 255]
        )
...
class TestScalarUFuncs(TestCase):
    ...
    def test_scalar_binary_mixed_ufunc(self):
        ...
        self.run_ufunc(
            _func, itertools.product(tys, tys), itertools.product(vals, vals)
        )
...
class _LoopTypesTester(TestCase):
    ...
    def _arg_for_type(self, a_letter_type, index=0):
        ...
        if a_letter_type in "BHILQ":
            ...
            if sys.platform != "win32":
                ...
                negzero = 0.0 - 1.0j
            return np.array(
                [negzero, 1.5 + 1.5j, 1j * float("nan"), 0j], dtype=a_letter_type
            )
        else:
    ...
    def _check_ufunc_with_dtypes(self, fn, ufunc, dtypes):
        ...
        c_args = [
            self._arg_for_type(t, index=index).repeat(2)
    ...
    def _inject_test(cls, ufunc, loop):
        ...
        setattr(
            cls,
            "test_{0}_{1}".format(ufunc.__name__, loop.replace("->", "_")),
            test_template,
        )
...
class TestUfuncWriteInput(TestCase):
    def test_write_input_arg(self):
        ...
        np.testing.assert_array_equal(
            np.array([True, False, False, False, True], dtype=np.bool_), out.any(axis=0)
        )
