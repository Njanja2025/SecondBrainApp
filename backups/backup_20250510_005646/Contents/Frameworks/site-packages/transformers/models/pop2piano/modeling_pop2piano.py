...
try:
    ...
    logger.warning(
        "Discovered apex but it failed to load, falling back to Pop2PianoLayerNorm"
    )
    pass
...
class Pop2PianoLayerFF(nn.Module):
    def __init__(self, config: Pop2PianoConfig):
        ...
        self.layer_norm = Pop2PianoLayerNorm(
...
class Pop2PianoLayerCrossAttention(nn.Module):
    def __init__(self, config, layer_idx: Optional[int] = None):
        super().__init__()
        self.EncDecAttention = Pop2PianoAttention(
...
class Pop2PianoStack(Pop2PianoPreTrainedModel):
    ...
    def __init__(self, config, embed_tokens=None):
        ...
        self.final_layer_norm = Pop2PianoLayerNorm(
            config.d_model, eps=config.layer_norm_epsilon
        )
        self.dropout = nn.Dropout(config.dropout_rate)
...
class Pop2PianoConcatEmbeddingToMel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.embedding = nn.Embedding(
            num_embeddings=config.composer_vocab_size, embedding_dim=config.d_model
        )
...
class Pop2PianoForConditionalGeneration(Pop2PianoPreTrainedModel, GenerationMixin):
    ...
    def get_mel_conditioner_outputs(
        ...
        if attention_mask is not None:
            ...
            attention_mask = torch.concatenate(
                [attention_mask[:, 0].view(-1, 1), attention_mask], axis=1
            )
    ...
    def forward(
        ...
        if inputs_embeds is not None and input_features is not None:
            raise ValueError(
                "Both `inputs_embeds` and `input_features` received! Please provide only one of them"
            )
    ...
    def generate(
        ...
        **kwargs,
        ...
        if (
            len(generation_config.composer_to_feature_token)
            != self.config.composer_vocab_size
        ):
            raise ValueError(
